...
console.log(
  "🔐 RECAPTCHA ENV:",
  process.env.RECAPTCHA_SECRET_KEY ? "ADA" : "TIDAK ADA",
);
...
app.get("/api/layers/:tableName/geojson", async (req, res) => {
  ...
  try {
    ...
    if (infraColsWhitelist[tableName]) {
      selectedCols = infraColsWhitelist[tableName].filter((c) =>
        availableColNames.includes(c),
      );
    } else {
    ...
    if (
      tableName === "tutupan_lahan" ||
      tableName === "penutupan_lahan_2024" ||
      tableName === "pl2024"
    ) {
      needsMapping = true;
...
// ============================================================
// SIMITI GIS - LAYER OVERLAY CATALOG
// REAL DATABASE CATALOG
// ============================================================
console.log("🔥 LAYER OVERLAY ROUTE REGISTERED");
app.get("/api/layer-overlay/catalog", async (req, res) => {
  console.log("🔥🔥🔥 REQUEST MASUK /api/layer-overlay/catalog");
  ...
  try {
    console.log("==============================================");
    console.log("📡 GET /api/layer-overlay/catalog");
    console.log("📡 Loading real GIS layer catalog...");
    console.log("==============================================");

    // ----------------------------------------------------------
    // Ambil seluruh tabel spatial yang benar-benar mempunyai
    // kolom geometry dari PostgreSQL/PostGIS.
    // ----------------------------------------------------------
    ...
    const result = await db.query(`
      SELECT
        ...
        (
          SELECT COUNT(*)
          FROM information_schema.columns gc
          WHERE gc.table_schema = c.table_schema
            AND gc.table_name = c.table_name
            AND gc.column_name IN ('geom', 'geometry', 'the_geom')
        ) AS geometry_column_count
      ...
      WHERE c.table_schema = DATABASE()
        ...
        AND EXISTS (
          ...
          WHERE g.table_schema = c.table_schema
            ...
            AND g.column_name IN ('geom', 'geometry', 'the_geom')
        )

      ORDER BY c.table_name
    `);

    console.log(`📊 Spatial tables detected: ${result.rows.length}`);

    // ----------------------------------------------------------
    // Jangan masukkan tabel sistem / internal.
    // ----------------------------------------------------------
    ...
    const spatialTables = result.rows.filter(
      (row) => !excludedTables.has(row.table_name),
    );

    // ----------------------------------------------------------
    // Helper icon berdasarkan nama tabel.
    // Frontend tetap bisa menggunakan icon dari metadata.
    // ----------------------------------------------------------
    ...
    const getCategory = (tableName) => {
      ...
      if (
        ...
        name.includes("limpasan") ||
        name.includes("longsor") ||
        name.includes("erosi") ||
        name.includes("karhutla") ||
        name.includes("kebakaran") ||
        name.includes("kekeringan") ||
        name.includes("abrasi") ||
        name.includes("rawan")
      ) {
        return {
          id: "risiko",
      ...
      if (
        ...
        name.includes("adaptasi") ||
        name.includes("rekomendasi") ||
    ...
    for (const row of spatialTables) {
      const category = getCategory(row.table_name);
      ...
      try {
        ...
        featureCount = countResult.rows[0]?.count ?? 0;
      } catch (countError) {
        console.warn(
          `⚠️ Failed counting ${row.table_name}:`,
          countError.message,
        );
      }
      ...
      categoryMap.get(category.id).children.push({
        ...
        title: formatTitle(row.table_name),
        icon: getIcon(row.table_name),
        checked: false,
        featureCount,
    ...
    const totalLayers = categories.reduce(
    ...
    console.log(`✅ Catalog ready: ${totalLayers} layers`);

    console.log(`✅ Total spatial records: ${totalFeatures}`);
    ...
    console.error("❌ /api/layer-overlay/catalog ERROR:", error);

    res.status(500).json({
      success: false,
      message: "Gagal mengambil katalog layer GIS.",
      error: error.message,
    });
  } finally {
    db.release();
...
// ============================================================
// NASA FIRMS / VIIRS NOAA-21 (ADDITIVE ROUTE)
// Tidak mengubah route/fungsi lama.
// Frontend memanggil endpoint lokal ini agar API key tidak
// perlu dikirim langsung dari browser dan menghindari CORS.
// ============================================================
app.get("/api/firms/noaa21", async (req, res) => {
  const firmsKey =
    process.env.FIRMS_MAP_KEY || process.env.NASA_FIRMS_MAP_KEY || "";

  if (!firmsKey) {
    return res.status(500).json({
      success: false,
      message:
        "FIRMS API key belum dikonfigurasi. Tambahkan FIRMS_MAP_KEY di .env backend.",
    });
  }

const url =
  `https://firms.modaps.eosdis.nasa.gov/mapserver/wfs/SouthEast_Asia/` +
  `${encodeURIComponent(firmsKey)}/?` +
  new URLSearchParams({
    SERVICE: "WFS",
    REQUEST: "GetFeature",
    VERSION: "2.0.0",
    TYPENAME: "ms:fires_noaa21_24hrs",
    STARTINDEX: "0",
    COUNT: "10000",
    ...
    if (!response.ok) {
      ...
      console.error(
        `❌ NASA FIRMS NOAA-21 HTTP ${response.status}:`,
        body.slice(0, 500),
      );

      return res.status(response.status).json({
        success: false,
        message: `NASA FIRMS HTTP ${response.status}`,
      });
    }
    ...
    const hotspots = lines.slice(1).flatMap((line) => {
    ...
    console.error("❌ /api/firms/noaa21 ERROR:", error);

    return res.status(500).json({
      success: false,
      message: "Gagal mengambil data NASA FIRMS VIIRS NOAA-21.",
      error: error?.message || String(error),
    });
  }
});
// ============================================================
// ADDITIVE: NASA FIRMS / VIIRS NOAA-21 24 HOURS
// Tidak mengubah endpoint/fungsi existing.
// ============================================================
...
app.get("/api/firms/noaa21", async (req, res) => {
  ...
  try {
    ...
    const url =
      `https://firms.modaps.eosdis.nasa.gov/mapserver/wfs/SouthEast_Asia/` +
      `${encodeURIComponent(FIRMS_MAP_KEY)}/?${params.toString()}`;

    console.log("🔥 FIRMS NOAA-21 WFS request");
    ...
    if (!response.ok) {
      console.error("NASA FIRMS WFS ERROR:", response.status, raw);

      return res.status(response.status).json({
        success: false,
        message: `NASA FIRMS WFS HTTP ${response.status}`,
    ...
    try {
      ...
      console.error("NASA FIRMS returned non-JSON:", raw.slice(0, 1000));

      return res.status(502).json({
        success: false,
        message: "NASA FIRMS tidak mengembalikan JSON yang valid.",
        details: raw.slice(0, 1000),
      });
    }
    ...
    console.error("❌ FIRMS NOAA-21 ERROR:", error);

    return res.status(500).json({
      success: false,
      message: error?.message || "Gagal mengambil data NASA FIRMS.",
    });
  }
});

// ============================================================
// ADDITIVE: NASA FIRMS NOAA-21 GetFeatureInfo proxy
// Tidak mengubah fungsi existing.
// ============================================================
app.get("/api/firms/noaa21/info", async (req, res) => {
  const FIRMS_MAP_KEY = process.env.FIRMS_MAP_KEY || "";

  if (!FIRMS_MAP_KEY) {
    return res.status(500).json({
      success: false,
      message: "FIRMS_MAP_KEY belum dikonfigurasi di backend.",
    });
  }

  try {
    const { bbox, width, height, x, y } = req.query;

    if (!bbox || !width || !height || x === undefined || y === undefined) {
      return res.status(400).json({
        success: false,
        message: "Parameter bbox, width, height, x, y wajib diisi.",
      });
    }
    ...
    console.log("🔥 FIRMS GetFeatureInfo proxy");
    ...
    console.log("NASA FIRMS STATUS:", response.status);
    console.log("NASA FIRMS BODY:", raw);
    let data = null;
    ...
    try {
      ...
      return res.status(502).json({
        success: false,
        message: "NASA FIRMS mengembalikan response bukan JSON.",
    ...
    console.error("❌ FIRMS GetFeatureInfo proxy error:", error);

    return res.status(500).json({
      success: false,
      message: error?.message || "Gagal mengambil detail hotspot FIRMS.",
    });
  }
});
// 404 handler

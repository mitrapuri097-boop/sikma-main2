...
// ============================================================
// AREA SEARCH — Opsi A (MySQL-safe)
//
// Prinsip:
// 1) Jangan memakai geom_valid / ST_Union() karena schema GIS saat ini
//    memakai kolom `geom` dan beberapa tabel boundary dapat kosong.
// 2) Prioritaskan tabel spatial lama jika memang berisi data.
// 3) Jika tabel spatial kosong, pakai tabel master_* untuk nama wilayah.
//    Geometry boleh null; frontend akan geocode hasilnya dan flyTo lokasi.
// 4) Jangan memakai PostgreSQL $1/$2 di endpoint ini — gunakan ? langsung
//    supaya mysql2.execute() menerima parameter dengan aman.
// ============================================================
const AREA_SEARCH_CONFIG = {
  provinsi: {
    spatialTable: "provinsi",
    spatialNameColumns: ["provinsi", "nama_provinsi", "name"],
    masterTables: ["master_provinsi", "provinsi_dummy"],
    masterNameColumns: ["nama_provinsi", "provinsi", "name"],
    output: (row, name) => ({
      label: name,
      provinsi: name,
      level: "provinsi",
      geom: row.geom_json ? parseJsonSafe(row.geom_json) : null,
    }),
  },
  kabupaten: {
    spatialTable: "kab_kota",
    spatialNameColumns: ["kab_kota", "nama_kabupaten", "kabupaten", "name"],
    masterTables: ["master_kabupaten"],
    masterNameColumns: ["nama_kabupaten", "kab_kota", "kabupaten", "name"],
    output: (row, name) => ({
      label: row.provinsi ? `${name}, ${row.provinsi}` : name,
      kab_kota: name,
      nama_kabupaten: name,
  ...
  kecamatan: {
    spatialTable: "kecamatan",
    spatialNameColumns: ["kecamatan", "nama_kecamatan", "name"],
    masterTables: ["master_kecamatan"],
    masterNameColumns: ["nama_kecamatan", "kecamatan", "name"],
    output: (row, name) => ({
      label: row.kab_kota ? `${name}, ${row.kab_kota}` : name,
      kecamatan: name,
      nama_kecamatan: name,
  ...
  kelurahan: {
    spatialTable: "kel_desa",
    spatialNameColumns: ["kel_desa", "nama_kelurahan", "kelurahan", "desa", "name"],
    masterTables: ["master_desa", "master_kelurahan"],
    masterNameColumns: ["nama_desa", "nama_kelurahan", "kel_desa", "desa", "kelurahan", "name"],
    output: (row, name) => ({
      label: row.kecamatan ? `${name}, ${row.kecamatan}` : name,
      kel_desa: name,
      nama_kelurahan: name,
...
function firstExistingColumn(columns, candidates) {
  ...
  for (const candidate of candidates) {
    const hit = lookup.get(String(candidate).toLowerCase());
    if (hit) return hit;
  }
  return null;
}
...
app.get("/api/areas/search", async (req, res) => {
  ...
  try {
    ...
    const config = AREA_SEARCH_CONFIG[level];
    if (!config) {
      return res.status(400).json({
        success: false,
        error: "Invalid level",
        message: "Level harus provinsi, kabupaten, kecamatan, atau kelurahan.",
      });
    }
    ...
    // ----------------------------------------------------------
    // A. Cari geometry dari tabel spatial yang benar-benar ada.
    // ----------------------------------------------------------
    if (await tableExists(client, config.spatialTable)) {
      const columns = await getTableColumns(client, config.spatialTable);
      const nameColumn = firstExistingColumn(columns, config.spatialNameColumns);
      const geomColumn = firstExistingColumn(columns, ["geom", "geometry"]);

      if (nameColumn && geomColumn) {
        // Ambil kolom konteks jika tersedia, tanpa mengasumsikan schema tertentu.
        const contextCandidates = ["provinsi", "kab_kota", "kecamatan", "kel_desa"];
        const contextColumns = contextCandidates
          .filter((c) => columns.some((actual) => actual.toLowerCase() === c.toLowerCase()))
          .map((c) => columns.find((actual) => actual.toLowerCase() === c.toLowerCase()));

        const selectContext = contextColumns.map((c) => `\`${c}\``).join(", ");
        const contextPrefix = selectContext ? `, ${selectContext}` : "";

        const sql = `
          SELECT \`${nameColumn}\` AS matched_name
                 ${contextPrefix},
                 ST_AsGeoJSON(\`${geomColumn}\`) AS geom_json
            FROM \`${config.spatialTable}\`
           WHERE \`${nameColumn}\` IS NOT NULL
             AND LOWER(\`${nameColumn}\`) LIKE ?
             AND \`${geomColumn}\` IS NOT NULL
           ORDER BY \`${nameColumn}\`
           LIMIT 50
        `;

        const spatialResult = await client.query(sql, [searchPattern]);
        results = spatialResult.rows.map((row) => {
          const name = String(row.matched_name || "").trim();
          return config.output(row, name);
        }).filter((r) => r.label);

        console.log(`🗺️ [SEARCH] Spatial ${config.spatialTable}: ${results.length} results`);
      } else {
        console.log(`ℹ️ [SEARCH] ${config.spatialTable}: name/geom column tidak lengkap`);
      }
    } else {
      console.log(`ℹ️ [SEARCH] Spatial table ${config.spatialTable} tidak ada`);
    }

    // ----------------------------------------------------------
    // B. Kalau spatial kosong, cari master_*.
    //    Ini sengaja TANPA geometry karena tabel master memang hanya
    //    menyimpan identitas wilayah pada schema yang sekarang.
    // ----------------------------------------------------------
    if (results.length === 0) {
      for (const masterTable of config.masterTables) {
        if (!(await tableExists(client, masterTable))) continue;

        const columns = await getTableColumns(client, masterTable);
        const nameColumn = firstExistingColumn(columns, config.masterNameColumns);
        ...
        const masterResult = await client.query(sql, [searchPattern]);
        results = masterResult.rows.map((row) => {
          const name = String(row.matched_name || "").trim();
          return config.output(row, name);
        }).filter((r) => r.label);

        console.log(`📚 [SEARCH] Master ${masterTable}: ${results.length} results`);
    ...
    console.log(`📍 [SEARCH] Returning ${results.length} grouped results`);

...
app.get("/api/location-proximity", async (req, res) => {
  ...
  try {
    ...
    //   3. DISTANCE terdekat
    //
    // Tidak lagi mengambil "1 feature terdekat per layer".
    //
    // Score langsung JOIN ke risk_class_score sehingga urutan
    // spatial + scoring dilakukan oleh PostgreSQL/PostGIS.
    // ========================================================
    ...
    const threatResults = await Promise.all(
      riskLayers.map(async (definition) => {
        try {
          ...
          // POSTGIS SRID NORMALIZATION
          //
          // Beberapa layer GIS lama tersimpan sebagai SRID 0
          // walaupun koordinatnya sebenarnya sudah longitude/latitude
          // WGS84. Point GPS selalu dibuat sebagai EPSG:4326.
          //
          // Jangan mengubah geometry di database. Normalisasi hanya
          // dilakukan di query:
          ...
          // Fast path: spatial bbox first for SRID 0 / EPSG:4326.
          // The raw geometry remains indexable; expensive geography distance
          // is evaluated only for nearby candidates.
          const nearbyQuery = `
            SELECT
              ${classColumn} AS risk_class,
              scoring.score AS risk_score,
              ST_Covers(${normalizedGeometry}, ${point}) AS inside,
              ROUND(
                ST_Distance(
            ...
            WHERE ${geometryColumn} IS NOT NULL
              AND ST_SRID(${geometryColumn}) IN (0, 4326)
              AND ${geometryColumn} && ST_Expand(
                ST_SetSRID(ST_MakePoint($2, $1), 4326),
                5
              )
            ORDER BY
              ST_Covers(${normalizedGeometry}, ${point}) DESC,
              scoring.score DESC NULLS LAST,
              ${normalizedGeometry} <-> ${point} ASC
            LIMIT $3
          `;

          let result = await pool.query(nearbyQuery, [
            latitude,
            longitude,
            threatLimit,
          ]);

          // Fallback untuk layer yang tidak punya feature pada bbox 5 derajat
          // atau menggunakan SRID selain 0/4326.
          if (!result.rows.length) {
            const fallbackQuery = `
              SELECT
                ${classColumn} AS risk_class,
          ...
          return (result.rows || []).map((row) => {
            ...
            return {
              ...
              // Score individual feature berdasarkan risk_class_score.
              score,

              // level tidak dipaksakan menjadi status karena
              // risk_status_rule pada /api/location-assessment
              // adalah rule untuk TOTAL SCORE seluruh parameter.
              level: null,

              distanceMeters: Boolean(row.inside)
                ? 0
    ...
    const threats = threatResults
      .flat()
      .sort((a, b) => {
        // 1. Polygon yang mengandung GPS selalu di depan.
        if (Boolean(a.inside) !== Boolean(b.inside)) {
          return Boolean(b.inside) - Boolean(a.inside);
        }

        // 2. Score lebih tinggi lebih prioritas.
        ...
        // 3. Distance lebih dekat lebih prioritas.
    ...
    try {
      ...
      console.log(
        `✅ [LOCATION PROXIMITY] Mitigations: ${mitigations.length}`,
      );
    } catch (error) {
      console.warn(
    ...
    try {
      const incidentResult = await pool.query(
        `
          SELECT
            ...
            -- disaster_type is read safely through JSON so an older
            -- kejadian schema without that physical column does not
            -- break the entire proximity response.
            to_jsonb(k) ->> 'disaster_type' AS disaster_type,

            to_jsonb(k) ->> 'incident_date' AS incident_date,
            k.latitude,
            k.longitude,
            ...
            ROUND(
              ST_Distance(
                (CASE
                  WHEN ST_SRID(k.geom) = 0
                    THEN ST_SetSRID(k.geom, 4326)
                  WHEN ST_SRID(k.geom) = 4326
                    THEN k.geom
                  ELSE ST_Transform(k.geom, 4326)
                END)::geography,
                ${point}::geography
              )
            )::bigint AS distance_meters

          FROM public.kejadian k
      ...
      console.log(
        `✅ [LOCATION PROXIMITY] Incidents: ${incidents.length}`,
      );
    } catch (error) {
      console.warn(
        "⚠️ [LOCATION PROXIMITY] Incident query skipped:",
        error.message,
      );
    ...
    console.log(
      `🚀 [LOCATION PROXIMITY] TOTAL: ${processingTimeMs} ms`,
    );
    ...
    return res.json({
      ...
      summary: {
        ...
        nearestThreatDistanceMeters:
          threats[0]?.distanceMeters ?? null,

        nearestMitigationDistanceMeters:
          mitigations[0]?.distanceMeters ?? null,

        nearestIncidentDistanceMeters:
          incidents[0]?.distanceMeters ?? null,

...
const BNPB_GPS_IDENTIFY_LAYERS = [
  {
    ...
    url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_RISIKO_BANJIR_JBTBPJ/MapServer/0",
  },
  {
    key: "longsor",
    label: "Longsor",
    category: "Risiko",
    serviceType: "MapServer",
    url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_RISIKO_TANAHLONGSOR_JBTBPJ/MapServer/0",
  },
  {
    key: "banjir_bandang",
    ...
    category: "Risiko",
    serviceType: "MapServer",
    url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_RISIKO_BANJIRBANDANG_JBTBPJ/MapServer/0",
  },
  {
    key: "kekeringan",
    label: "Kekeringan",
...
const isBnpbSafeClass = (value) => {
  const normalized = normalizeBnpbRisk(value);
  return normalized.score === 1;
};

const queryBnpbPoint = async (layer, latitude, longitude) => {
  const x = Number(longitude);
  const y = Number(latitude);

  if (layer.serviceType === "ImageServer") {
    const params = new URLSearchParams({
      f: "json",
    ...
    const response = await fetch(`${layer.url}/getSamples?${params.toString()}`, {
      headers: { Accept: "application/json" },
    });

    if (!response.ok) throw new Error(`BNPB HTTP ${response.status}`);
    const json = await response.json();
    if (json?.error) throw new Error(json.error.message || "BNPB ImageServer error");

    const sample = Array.isArray(json?.samples) ? json.samples[0] : null;
    const rawValue = sample?.value ?? sample?.attributes?.value ?? null;
    const normalized = normalizeBnpbRisk(rawValue);

    return {
      ...layer,
      inside: rawValue !== null && rawValue !== undefined,
      available: true,
  ...
  const serviceMatch = String(layer.url).match(/^(.*\/MapServer)\/(\d+)$/i);
  if (!serviceMatch) throw new Error(`URL MapServer tidak valid: ${layer.url}`);

  const serviceUrl = serviceMatch[1];
  const layerId = serviceMatch[2];
  const delta = 0.01;
  ...
  const params = new URLSearchParams({
    ...
    layers: `all:${layerId}`,
    tolerance: "2",
    mapExtent: `${x - delta},${y - delta},${x + delta},${y + delta}`,
    imageDisplay: "1000,1000,96",
    returnGeometry: "false",
  });

  const response = await fetch(`${serviceUrl}/identify?${params.toString()}`, {
    headers: { Accept: "application/json" },
  });
...
const identifyBnpbAtPoint = async (latitude, longitude) => {
  return Promise.all(
    BNPB_GPS_IDENTIFY_LAYERS.map(async (layer) => {
      try {
        return await queryBnpbPoint(layer, latitude, longitude);
      } catch (error) {
        return {
          ...layer,
          inside: false,
...
app.get("/api/location-bnpb", async (req, res) => {
  ...
  try {
    ...
    // ==========================================================
    // 5 PARAMETER MY LOKASI
    // GPS -> BNPB InaRISK -> klasifikasi -> skor 1..3
    // ==========================================================
    const riskFactors = disasterLayers.map((item) => ({
      ...
      score: item.score,
      source: item.url,
      available: item.available === true && item.score !== null,
      inside: item.inside === true,
      rawValue: item.value,
    }));

    const completeRisk = riskFactors.length === 5 && riskFactors.every(
      (item) => item.available && Number.isFinite(Number(item.score)),
    );
    ...
    const apiRiskIndex =
      myLokasiScore == null
        ? null
        : Math.round(((myLokasiScore - 5) / 10) * 100);
    ...
    return res.json({
      ...
      risk: {
        ...
        factors: riskFactors,
        methodology: "GPS -> BNPB InaRISK -> klasifikasi ancaman -> scoring 1-3 -> total 5-15 -> status Aman/Siaga/Waspada/Rawan.",
      },

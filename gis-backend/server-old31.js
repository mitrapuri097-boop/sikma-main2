...
const BNPB_GPS_IDENTIFY_LAYERS = [
  ...
  },
  {
    ...
    label: "Bahaya Banjir",
    category: "Bahaya",
    url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_BAHAYA_BANJIR_JBTBPJ/MapServer/0",
  },
  {
    key: "bahaya_banjir_bandang",
    label: "Bahaya Banjir Bandang",
    category: "Bahaya",
    url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_BAHAYA_BANJIRBANDANG_JBTBPJ/MapServer/0",
  },
  {
    key: "bahaya_longsor",
    label: "Bahaya Longsor",
    category: "Bahaya",
    url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_BAHAYA_TANAHLONGSOR_JBTBPJ/MapServer/0",
  },
];
...
const classifyBnpbAttributes = (attributes = {}) => {
  ...
  const source = preferred || entries.find(([, value]) =>
    /sangat tinggi|tinggi|sedang|rendah|aman|bahaya|rawan/i.test(String(value)),
  );

  return {
    field: source ? String(source[0]) : null,
    value: source ? String(source[1]).trim() : null,
    normalized: source ? normalizeBnpbClass(source[1]) : "",
  };
};
...
const isBnpbSafeClass = (value) => {
  ...
  if (v.includes("sedang") || v.includes("waspada") || v.includes("siaga")) {
    return false;
  }

  if (v.includes("rendah") || v.includes("aman") || v === "0" || v === "1") {
    return true;
  }
...
const queryBnpbPoint = async (layer, latitude, longitude) => {
  ...
  if (!response.ok) {
    throw new Error(`BNPB HTTP ${response.status}`);
  }

  const json = await response.json();

  if (json?.error) {
    throw new Error(json.error.message || "BNPB query error");
  }
  ...
  const classified = classifyBnpbAttributes(feature.attributes);

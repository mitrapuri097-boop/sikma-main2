...
const dbConfig = {
  ...
  database: process.env.DB_NAME || process.env.MYSQL_DATABASE || "gis_data",
...
const normalizeSql = (sql) => {
  ...
  q = q.replace(/::(?:geography|geometry|integer|int|json|numeric|text|regclass)\b/g, "");
  ...
  q = q.replace(/ST_SetSRID\(ST_MakePoint\(([^,()]+),\s*([^()]+)\),\s*4326\)/gi, "ST_SRID(POINT($1, $2), 4326)");
  q = q.replace(/ST_SetSRID\(([^()]+),\s*4326\)/gi, "ST_SRID($1, 4326)");

  // A few PostgreSQL concatenations used in active SQL.
  q = q.replace(/'Tutupan Lahan ' \|\| tl\.pl2024_id/g, "CONCAT('Tutupan Lahan ', tl.pl2024_id)");
  q = q.replace(/'Penutupan Lahan ' \|\| pl\.pl2024_id/g, "CONCAT('Penutupan Lahan ', pl.pl2024_id)");
...
const getReturningInfo = (sql) => {
  ...
  const baseSql = sql.slice(0, match.index).trim().replace(/;\\s*$/, "");
...
const compatQuery = async (connection, rawSql, params = []) => {
  ...
  if (returningInfo) {
    const tableMatch = sql.match(/\b(?:INSERT\s+INTO|UPDATE|DELETE\s+FROM)\s+`?([A-Za-z0-9_]+)`?/i);
    ...
    if (tableName) {
      ...
      if (idValue !== null && idValue !== undefined) {
        const selectCols = returningInfo.returning === "*" ? "*" : returningInfo.returning;
...
const pool = {
  ...
  connect: async () => new MySqlClientCompat(await mysqlPool.getConnection(), true),
};

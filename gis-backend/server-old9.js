...
// Endpoint untuk search area administrasi.
// IMPORTANT: this project uses MySQL. Do not use PostgreSQL aggregate spatial
// functions such as ST_Union() here. MySQL's ST_Union() is a binary spatial
// function, so the old query caused HTTP 500 for every search.
app.get("/api/areas/search", async (req, res) => {
  ...
  try {
    ...
    const searchPattern = `%${String(query).trim().toLowerCase()}%`;
    ...
    const config = {
      ...
      kabupaten: {
        ...
        groupKey: (r) => `${r.kab_kota}|||${r.provinsi}`,
        makeItem: (r) => ({
          label: `${r.kab_kota}, ${r.provinsi}`,
          kab_kota: r.kab_kota,
          provinsi: r.provinsi,
          level: "kabupaten",
        }),
      ...
      kecamatan: {
        table: "kecamatan",
        name: "kecamatan",
        select: "kecamatan, kab_kota, provinsi",
        groupKey: (r) => `${r.kecamatan}|||${r.kab_kota}|||${r.provinsi}`,
        makeItem: (r) => ({
          label: `${r.kecamatan}, ${r.kab_kota}, ${r.provinsi}`,
      ...
      kelurahan: {
        table: "kel_desa",
        name: "kel_desa",
        select: "kel_desa, kecamatan, kab_kota, provinsi",
        groupKey: (r) =>
          `${r.kel_desa}|||${r.kecamatan}|||${r.kab_kota}|||${r.provinsi}`,
        makeItem: (r) => ({
          label: `${r.kel_desa}, ${r.kecamatan}, ${r.kab_kota}, ${r.provinsi}`,
          kel_desa: r.kel_desa,
          kecamatan: r.kecamatan,
          kab_kota: r.kab_kota,
          provinsi: r.provinsi,
    ...
    const cfg = config[normalizedLevel];
    if (!cfg) {
      return res.status(400).json({
        error:
    ...
    console.log(`🗺️  [SEARCH] Searching ${normalizedLevel} for`, searchPattern);

    // Fetch individual geometries. We combine them into a GeoJSON
    // GeometryCollection in Node.js instead of calling PostgreSQL's ST_Union.
    // This works with the MySQL spatial schema currently used by SIMITIGASI.
    const sql = `
      SELECT ${cfg.select}, ST_AsGeoJSON(geom_valid) AS geom_json
      FROM ${cfg.table}
      WHERE LOWER(${cfg.name}) LIKE ?
        AND geom_valid IS NOT NULL
      ORDER BY ${cfg.name}
      LIMIT 1000
    `;

    const result = await client.query(sql, [searchPattern]);
    console.log(`✅ [SEARCH] Found ${result.rows.length} geometry rows`);

    const grouped = new Map();

    for (const row of result.rows) {
      const key = cfg.groupKey(row);
      let item = grouped.get(key);

      if (!item) {
        item = { ...cfg.makeItem(row), geometries: [] };
      ...
      if (row.geom_json) {
        try {
          ...
          console.warn("⚠️ [SEARCH] Invalid GeoJSON skipped:", geometryError.message);
        }
      }
    }
    ...
    console.log(`📍 [SEARCH] Returning ${response.length} grouped results`);
    return res.json(response);
  } catch (error) {
    console.error("❌ [SEARCH] Error:", error);
    return res.status(500).json({
      error: "Failed to search areas: " + (error?.message || String(error)),
    });
  } finally {
    client.release();
  }
...
// Endpoint untuk mendapatkan bounds dari area yang dipilih.
// MySQL does not provide PostgreSQL's ST_Extent aggregate, so calculate the
// bounding box from the GeoJSON geometries returned by MySQL.
app.post("/api/areas/bounds", async (req, res) => {
  ...
  try {
    ...
    const getGeometryRows = async (area) => {
      const level = String(area.level || "").toLowerCase();
      let table;
      let where;
      let params;
      ...
      switch (level) {
        ...
        case "kabupaten":
          table = "kab_kota";
          where = "kab_kota = ? AND provinsi = ?";
          params = [area.kab_kota, area.provinsi];
          break;
        case "kecamatan":
          table = "kecamatan";
          where = "kecamatan = ? AND kab_kota = ? AND provinsi = ?";
          params = [area.kecamatan, area.kab_kota, area.provinsi];
          break;
        case "kelurahan":
          table = "kel_desa";
          where =
            "kel_desa = ? AND kecamatan = ? AND kab_kota = ? AND provinsi = ?";
          params = [area.kel_desa, area.kecamatan, area.kab_kota, area.provinsi];
          break;
        default:
          return [];
      }
      ...
      const result = await client.query(
        `SELECT ST_AsGeoJSON(geom_valid) AS geom_json
         FROM ${table}
         WHERE ${where} AND geom_valid IS NOT NULL
         LIMIT 5000`,
        params,
      );
      return result.rows;
    ...
    for (const area of selectedAreas) {
      const rows = await getGeometryRows(area);
      for (const row of rows) {
        ...
        try {
          ...
          console.warn("⚠️ [BOUNDS] Invalid GeoJSON skipped:", geometryError.message);
        }
      }
    }

    if (!geometryCount || !Number.isFinite(bounds.minLng)) {
    ...
    console.log("📐 [BOUNDS] Calculated MySQL GeoJSON bounds:", responseBounds);
    return res.json({ bounds: responseBounds });
  } catch (error) {
    console.error("❌ Error getting bounds:", error);
    return res.status(500).json({
      error: "Failed to get bounds: " + (error?.message || String(error)),
    });
  } finally {
    client.release();
  }

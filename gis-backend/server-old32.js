...
// ============================================================
// SIMITI ENTERPRISE - GPS BNPB DISASTER IDENTIFICATION + SAFE LOCATION
// ADDITIVE API: tidak mengubah /api/location-assessment maupun
// /api/location-proximity.
//
// Alur:
// 1. GPS user -> identify 6 layer BNPB yang diberikan.
// 2. Ambil kandidat lokasi kegiatan terdekat dari PostgreSQL.
// 3. Kandidat diverifikasi ulang terhadap 6 layer BNPB.
// 4. Pilih kandidat terdekat yang seluruh layer terpetakan aman/rendah.
// ============================================================

const BNPB_GPS_IDENTIFY_LAYERS = [
  { key:"risiko_banjir", label:"Risiko Banjir", category:"Risiko", type:"MapServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_RISIKO_BANJIR_JBTBPJ/MapServer", layerId:0 },
  { key:"risiko_kekeringan", label:"Risiko Kekeringan", category:"Risiko", type:"ImageServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/layer_risiko_kekeringan/ImageServer" },
  { key:"risiko_banjir_bandang", label:"Risiko Banjir Bandang", category:"Risiko", type:"MapServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_RISIKO_BANJIRBANDANG_JBTBPJ/MapServer", layerId:0 },
  { key:"risiko_abrasi", label:"Risiko Abrasi", category:"Risiko", type:"ImageServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/layer_risiko_gelombang_ekstrim_dan_abrasi/ImageServer" },
  { key:"risiko_longsor", label:"Risiko Longsor", category:"Risiko", type:"MapServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_RISIKO_TANAHLONGSOR_JBTBPJ/MapServer", layerId:0 },
  { key:"risiko_karhutla", label:"Risiko Karhutla", category:"Risiko", type:"ImageServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/layer_risiko_kebakaran_hutan_dan_lahan/ImageServer" },
  { key:"bahaya_banjir", label:"Bahaya Banjir", category:"Bahaya", type:"MapServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_BAHAYA_BANJIR_JBTBPJ/MapServer", layerId:0 },
  { key:"bahaya_kekeringan", label:"Bahaya Kekeringan", category:"Bahaya", type:"MapServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/layer_bahaya_kekeringan_30/MapServer", layerId:0 },
  { key:"bahaya_banjir_bandang", label:"Bahaya Banjir Bandang", category:"Bahaya", type:"MapServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_BAHAYA_BANJIRBANDANG_JBTBPJ/MapServer", layerId:0 },
  { key:"bahaya_abrasi", label:"Bahaya Abrasi", category:"Bahaya", type:"MapServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/layer_bahaya_gelombang_ekstrim_dan_abrasi_30/MapServer", layerId:0 },
  { key:"bahaya_longsor", label:"Bahaya Longsor", category:"Bahaya", type:"MapServer", url:"https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_BAHAYA_TANAHLONGSOR_JBTBPJ/MapServer", layerId:0 },
];

const normalizeBnpbClass = (value) => String(value ?? "").trim().replace(/\s+/g, " ").toLowerCase();
...
const classifyBnpbAttributes = (attributes = {}) => {
  ...
  const preferred = entries.find(([key]) => /kelas|class|status|risiko|risk|bahaya|hazard|indeks|index|tingkat|level/i.test(String(key)));
  const source = preferred || entries.find(([, value]) => /sangat tinggi|tinggi|sedang|rendah|aman|bahaya|rawan|waspada|siaga/i.test(String(value)));
  return { field: source ? String(source[0]) : null, value: source ? String(source[1]).trim() : null };
};

const normalizeBnpbRasterValue = (rawValue) => {
  if (rawValue === null || rawValue === undefined || rawValue === "") return { value:null, class:null, score:null };
  const n = Number(rawValue);
  if (Number.isFinite(n)) {
    if (n === 1) return { value:rawValue, class:"Rendah", score:1 };
    if (n === 2) return { value:rawValue, class:"Sedang", score:2 };
    if (n === 3) return { value:rawValue, class:"Tinggi", score:3 };
    return { value:rawValue, class:n > 0 ? "Terpetakan" : "Tidak terpetakan", score:null };
  }
  const v=normalizeBnpbClass(rawValue);
  if (v.includes("tinggi") || v.includes("bahaya") || v.includes("rawan")) return {value:rawValue,class:"Tinggi",score:3};
  if (v.includes("sedang") || v.includes("waspada") || v.includes("siaga")) return {value:rawValue,class:"Sedang",score:2};
  if (v.includes("rendah") || v.includes("aman")) return {value:rawValue,class:"Rendah",score:1};
  return {value:rawValue,class:String(rawValue),score:null};
};

const isBnpbSafeClass = (value) => {
  const v=normalizeBnpbClass(value);
  if (!v) return true;
  if (v.includes("tinggi") || v.includes("bahaya") || v.includes("rawan") || v.includes("sedang") || v.includes("waspada") || v.includes("siaga")) return false;
  return v.includes("rendah") || v.includes("aman") || v === "0" || v === "1";
};

const queryBnpbMapServerPoint = async (layer, latitude, longitude) => {
  const params=new URLSearchParams({
    f:"json",
    geometry:JSON.stringify({x:Number(longitude),y:Number(latitude),spatialReference:{wkid:4326}}),
    geometryType:"esriGeometryPoint", sr:"4326", layers:`all:${Number(layer.layerId ?? 0)}`,
    tolerance:"1", mapExtent:`${Number(longitude)-.001},${Number(latitude)-.001},${Number(longitude)+.001},${Number(latitude)+.001}`,
    imageDisplay:"1000,1000,96", returnGeometry:"false"
  });
  const response=await fetch(`${layer.url}/identify?${params.toString()}`,{headers:{Accept:"application/json"}});
  if(!response.ok) throw new Error(`BNPB MapServer HTTP ${response.status}`);
  const json=await response.json();
  if(json?.error) throw new Error(json.error.message || "BNPB MapServer identify error");
  const result=Array.isArray(json?.results)?json.results[0]:null;
  if(!result) return {...layer,inside:false,available:true,class:null,value:null,field:null,score:null,attributes:null};
  const attributes=result.attributes||{};
  const classified=classifyBnpbAttributes(attributes);
  const raw=result.value ?? result.pixelValue ?? attributes.Value ?? attributes.value ?? attributes.PixelValue ?? attributes.pixelvalue ?? classified.value;
  const raster=normalizeBnpbRasterValue(raw);
  return {...layer,inside:raster.value!==null || Boolean(classified.value),available:true,class:classified.value || raster.class,value:raster.value ?? classified.value,field:classified.field || "BNPB",score:raster.score,attributes};
};

const queryBnpbImageServerPoint = async (layer, latitude, longitude) => {
  const params=new URLSearchParams({
    f:"json",
    geometry:JSON.stringify({x:Number(longitude),y:Number(latitude),spatialReference:{wkid:4326}}),
    geometryType:"esriGeometryPoint", returnFirstValueOnly:"true", interpolation:"RSP_NearestNeighbor", returnGeometry:"false", outFields:"*"
  });
  const response=await fetch(`${layer.url}/getSamples?${params.toString()}`,{headers:{Accept:"application/json"}});
  if(!response.ok) throw new Error(`BNPB ImageServer HTTP ${response.status}`);
  const json=await response.json();
  if(json?.error) throw new Error(json.error.message || "BNPB ImageServer getSamples error");
  const sample=Array.isArray(json?.samples)&&json.samples.length?json.samples[0]:null;
  if(!sample) return {...layer,inside:false,available:true,class:null,value:null,field:null,score:null,attributes:null};
  const raw=sample.value ?? sample.pixelValue ?? sample.attributes?.Value ?? sample.attributes?.value ?? sample.attributes?.PixelValue ?? null;
  const raster=normalizeBnpbRasterValue(raw);
  return {...layer,inside:raster.value!==null,available:true,class:raster.class,value:raster.value,field:"pixelValue",score:raster.score,attributes:sample.attributes||{}};
};

const queryBnpbPoint = async (layer, latitude, longitude) => layer.type === "ImageServer" ? queryBnpbImageServerPoint(layer,latitude,longitude) : queryBnpbMapServerPoint(layer,latitude,longitude);

const identifyBnpbAtPoint = async (latitude, longitude) => Promise.all(BNPB_GPS_IDENTIFY_LAYERS.map(async layer => {
  try { return await queryBnpbPoint(layer,latitude,longitude); }
  catch(error) { return {...layer,inside:false,available:false,class:null,value:null,field:null,score:null,attributes:null,error:error instanceof Error?error.message:String(error)}; }
}));
...
app.get("/api/location-bnpb", async (req, res) => {
  ...
  const candidateLimit = Math.min(
    Math.max(Number(req.query.candidateLimit) || 12, 1),
    30,
  );
  ...
  try {
    const disasterLayers = await identifyBnpbAtPoint(latitude, longitude);
    ...
    try {
      ...
      const candidateResult = await pool.query(
        `
          ...
          WHERE geom IS NOT NULL
            ...
            AND (
              status IS NULL
              OR LOWER(TRIM(status)) NOT IN ('nonaktif', 'inactive', 'arsip')
            )
          ORDER BY geom <-> ST_SetSRID(ST_MakePoint($2, $1), 4326)
          LIMIT $3
        `,
        [latitude, longitude, candidateLimit],
      );
      ...
      console.warn(
        "⚠️ [LOCATION BNPB] Kandidat lokasi aman gagal dimuat:",
    ...
    for (const candidate of candidates) {
      ...
      const candidateLayers = await identifyBnpbAtPoint(
        candidate.latitude,
        candidate.longitude,
      );

      const unavailableCount = candidateLayers.filter(
        (item) => !item.available,
      ).length;

      const unsafeLayers = candidateLayers.filter(
        (item) => item.inside && !isBnpbSafeClass(item.class),
      );

      const safe =
        unavailableCount === 0 &&
        unsafeLayers.length === 0;

      verifiedCandidates.push({
        ...candidate,
        safe,
        verified: true,
        unavailableLayerCount: unavailableCount,
        unsafeLayerCount: unsafeLayers.length,
        unsafeLayers: unsafeLayers.map((item) => ({
          key: item.key,
          label: item.label,
          class: item.class,
          category: item.category,
      ...
      // Kandidat sudah diurutkan berdasarkan jarak.
      // Kandidat pertama yang benar-benar aman adalah rekomendasi utama.
      if (safe) break;
    }

    const nearestSafeLocation =
      verifiedCandidates.find((candidate) => candidate.safe) || null;

    const nearestCandidate =
      verifiedCandidates[0] || candidates[0] || null;

    return res.json({
      success: true,
      source: "BNPB InaRISK + SIMITI PostgreSQL/PostGIS",
      analysisType: "GPS disaster identification + nearest verified safe location",
      generatedAt: new Date().toISOString(),
      processingTimeMs: Date.now() - startedAt,
      ...
      safeLocation: nearestSafeLocation,
      nearestCandidate: nearestCandidate
        ? {
            ...nearestCandidate,
            recommendation:
              nearestCandidate.safe
                ? "Lokasi terdekat dan terverifikasi aman terhadap seluruh layer BNPB yang digunakan."
                : "Lokasi terdekat belum terverifikasi aman terhadap seluruh layer BNPB.",
          }
        : null,
      summary: {
        ...
        safeCandidateCount: verifiedCandidates.filter((item) => item.safe).length,
        candidateCheckedCount: verifiedCandidates.length,
      },
    });
  } catch (error) {
    ...
    return res.status(500).json({
      success: false,
      message: "Gagal melakukan identifikasi bencana BNPB.",
      error: error instanceof Error ? error.message : String(error),
      processingTimeMs: Date.now() - startedAt,
    });
  }

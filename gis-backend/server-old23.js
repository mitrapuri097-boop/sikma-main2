...
function lokasiApiWhere(req) {
  ...
  if (String(das).trim()) {
    add(`%${String(das).trim()}%`, `keterangan ILIKE $PARAM`);
  }
...
app.get("/api/lokasi/filters", async (req, res) => {
  try {
    ...
    res.json({
      ...
      // Table existing belum memiliki kolom DAS.
      // Keterangan dipakai sebagai fallback master DAS.

...
// ============================================================
// ============================================================
// ADDITIVE: NASA FIRMS / VIIRS NOAA-21
// Hanya menambahkan endpoint baru. Fungsi/route existing lainnya dipertahankan.
// ============================================================
const FIRMS_MAP_KEY =
...
app.get("/api/firms/noaa21", async (req, res) => {
  ...
  try {
    ...
    console.log("🔥 FIRMS WFS STATUS:", response.status);
    console.log("🔥 FIRMS WFS BODY:", csv.slice(0, 1000));

    if (!response.ok) {
      return res.status(response.status).json({
        success: false,
        message: `NASA FIRMS WFS HTTP ${response.status}`,
        details: csv.slice(0, 2000),
      });
    }
      ...
      return [
        {
          latitude,
          longitude,
          bright_ti4: row.bright_ti4 ?? null,
          bright_ti5: row.bright_ti5 ?? null,
          scan: row.scan ?? null,
...
// Dipertahankan untuk frontend popup. Tidak mengubah route existing lain.
// ============================================================
app.get("/api/firms/noaa21/info", async (req, res) => {
  ...
  try {
    ...
    console.log("NASA FIRMS GetFeatureInfo STATUS:", response.status);
    console.log("NASA FIRMS GetFeatureInfo BODY:", raw.slice(0, 2000));

    if (!response.ok) {
      return res.status(502).json({
        success: false,
        message: `NASA FIRMS HTTP ${response.status}`,
        nasaBody: raw.slice(0, 2000),
      });
    }

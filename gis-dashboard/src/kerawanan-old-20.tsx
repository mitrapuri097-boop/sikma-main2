...
const Kerawanan = () => {
  ...
  const identifyBnpbAtPoint = async (latitude: number, longitude: number) => {
    const active = BNPB_INARISK_LAYERS.filter((item) => item.url && activeLayers.has(item.key));
    ...
    try {
      ...
      for (const config of active) {
        ...
        try {
          const response = await fetch(`${config.url}/identify?${params.toString()}`, { headers: { Accept: "application/json" }, cache: "no-store" });
          ...
          hits.forEach((hit: any) => {
            ...
            results.push({ service: config.name, url: config.url, value: firstValue == null ? hit?.value : String(firstValue), attributes: attrs });
          });
        } catch (serviceError) {
      ...
      setBnpbIdentifyPopup({ open: true, loading: false, error: results.length ? "" : "Tidak ada nilai InaRISK pada titik ini.", latitude, longitude, results });
    } catch (error: any) {
        ...
        <main className="flex-1 min-h-0 p-2 md:p-3 overflow-hidden">
          <div className="h-full grid grid-cols-[minmax(0,1fr)_350px] grid-rows-[minmax(0,1fr)_184px] gap-2">
            ...
            <section className="relative min-h-0 rounded-xl overflow-hidden bg-sky-100 border border-white shadow-sm">
              ...
              {bnpbIdentifyPopup.open && <div className="absolute right-3 bottom-14 z-[1900] w-[340px] max-w-[calc(100%-24px)]"><div className="rounded-xl bg-white border border-slate-200 shadow-xl overflow-hidden"><div className="bg-[#0b4d3c] text-white px-4 py-3 flex justify-between"><div><div className="text-[8px] text-emerald-200 uppercase">BNPB InaRISK • IDENTIFY</div><div className="text-sm font-bold">Informasi Risiko / Bahaya</div></div><button onClick={()=>setBnpbIdentifyPopup(p=>({...p,open:false}))}>×</button></div><div className="p-4 text-[10px] max-h-[250px] overflow-y-auto">{bnpbIdentifyPopup.loading?"Memuat informasi...":bnpbIdentifyPopup.error?bnpbIdentifyPopup.error:bnpbIdentifyPopup.results.map((r,i)=><div key={i} className="py-2 border-b last:border-0"><b>{r.service}</b><div className="text-slate-500 mt-1">{r.value||"Tidak ada nilai"}</div></div>)}</div></div></div>}
            </section>

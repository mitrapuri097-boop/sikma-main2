/*
 * KERAWANAN (8) — MERGE kerawanan(6) + kerawanan(7)
 *
 * Basis: kerawanan(7) karena sudah mencakup UI enterprise dan fitur
 * tambahan (BNPB InaRISK, Open-Meteo, layer risiko/bahaya/infrastruktur,
 * serta dashboard bawah). Fungsi inti kerawanan(6) tetap dipertahankan;
 * khusus mekanisme zoomend lama ditambahkan kembali secara eksplisit.
 * Tidak mengubah API endpoint existing dari kedua versi.
 */
...
const Kerawanan = () => {
  ...
  const BNPB_INARISK_LAYERS = [
    { key: "bnpb_risiko_banjir", name: "Risiko Banjir", group: "Risiko", url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_RISIKO_BANJIR_JBTBPJ/MapServer", layerId: 0 },
    { key: "bnpb_risiko_kekeringan", name: "Risiko Kekeringan", group: "Risiko", url: null, layerId: 0 },
    { key: "bnpb_risiko_banjir_bandang", name: "Risiko Banjir Bandang", group: "Risiko", url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_RISIKO_BANJIRBANDANG_JBTBPJ/MapServer", layerId: 0 },
    { key: "bnpb_risiko_abrasi", name: "Risiko Abrasi", group: "Risiko", url: null, layerId: 0 },
    { key: "bnpb_risiko_longsor", name: "Risiko Longsor", group: "Risiko", url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_RISIKO_TANAHLONGSOR_JBTBPJ/MapServer", layerId: 0 },
    { key: "bnpb_risiko_karhutla", name: "Risiko Karhutla", group: "Risiko", url: null, layerId: 0 },
    { key: "bnpb_bahaya_banjir", name: "Bahaya Banjir", group: "Bahaya", url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_BAHAYA_BANJIR_JBTBPJ/MapServer", layerId: 0 },
    { key: "bnpb_bahaya_kekeringan", name: "Bahaya Kekeringan", group: "Bahaya", url: null, layerId: 0 },
    { key: "bnpb_bahaya_banjir_bandang", name: "Bahaya Banjir Bandang", group: "Bahaya", url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_BAHAYA_BANJIRBANDANG_JBTBPJ/MapServer", layerId: 0 },
    { key: "bnpb_bahaya_abrasi", name: "Bahaya Abrasi", group: "Bahaya", url: null, layerId: 0 },
    { key: "bnpb_bahaya_longsor", name: "Bahaya Longsor", group: "Bahaya", url: "https://gis.bnpb.go.id/server/rest/services/inarisk/INDEKS_BAHAYA_TANAHLONGSOR_JBTBPJ/MapServer", layerId: 0 },
  ] as const;
  ...
  }>({ open: false, loading: false, error: "", latitude: null, longitude: null, results: [] });
  ...
  const [mapWeatherPopup, setMapWeatherPopup] =
    useState<MapWeatherPopupState>({
      open: false,
      loading: false,
      error: "",
      latitude: null,
  ...
  const loadBnpbLayer = async (key: string) => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        bboxSR: "4326",
        imageSR: "4326",
        size: `${Math.min(Math.max(size.x, 800), 1600)},${Math.min(Math.max(size.y, 600), 1200)}`,
        dpi: "96",
        format: "png32",
        transparent: "true",
      ...
      const imageUrl = `${config.url}/export?${params.toString()}`;
      const imageBounds = [[south, west], [north, east]];

      if (bnpbOverlayRefs.current[key]) {
        try { map.removeLayer(bnpbOverlayRefs.current[key]); } catch {}
      }
      ...
      console.log(`✅ BNPB InaRISK aktif: ${config.name}`);
    } catch (error: any) {
      ...
      setLoadingLayerNames((prev) => { const next = new Set(prev); next.delete(key); return next; });
    }
  };
  ...
  const removeBnpbLayer = (key: string) => {
    ...
    if (overlay && mapInstanceRef.current) {
      try { mapInstanceRef.current.removeLayer(overlay); } catch {}
    }
  ...
  const getBnpbServiceMeta = async (url: string) => {
    ...
    if (json?.error) throw new Error(json.error.message || "Metadata BNPB gagal");
  ...
  const identifyBnpbAtPoint = async (latitude: number, longitude: number) => {
    const active = BNPB_INARISK_LAYERS.filter((item) => item.url && activeLayersRef.current.has(item.key));
    if (!active.length || !mapInstanceRef.current) { setBnpbIdentifyPopup((prev) => ({ ...prev, open: false })); return; }
    setBnpbIdentifyPopup({ open: true, loading: true, error: "", latitude, longitude, results: [] });
    try {
      const map = mapInstanceRef.current; const bounds = map.getBounds(); const size = map.getSize();
      const p = wgs84ToEpsg3395(latitude, longitude); const results: BnpbIdentifyResult[] = [];
      const offsets = [[0,0],[120,0],[-120,0],[0,120],[0,-120],[250,0],[-250,0],[0,250],[0,-250]];
      for (const config of active) {
        try {
          const meta = await getBnpbServiceMeta(config.url!); const ext = meta?.fullExtent;
          if (ext && !pointInsideExtent3395(p.x,p.y,ext)) { results.push({service:config.name,url:config.url!,status:"out_of_coverage",message:"Lokasi berada di luar cakupan layer BNPB ini.",debug:{point3395:p,fullExtent:ext}}); continue; }
          let found:any=null, raw:any=null, lastDebug:any=null, successful=false;
          for (const [dx,dy] of offsets) {
            const q = new URLSearchParams({f:"json",geometry:`${p.x+dx},${p.y+dy}`,geometryType:"esriGeometryPoint",sr:"3395",layers:`all:${config.layerId}`,tolerance:"8",mapExtent:leafletBoundsToEpsg3395(bounds),imageDisplay:`${Math.max(size.x,800)},${Math.max(size.y,600)},96`,returnGeometry:"false"});
            const requestUrl=`${config.url}/identify?${q.toString()}`; const response=await fetch(requestUrl,{headers:{Accept:"application/json"},cache:"no-store"}); const body=await response.text();
            if(!response.ok){lastDebug={offset:[dx,dy],httpStatus:response.status,requestUrl,responseText:body.slice(0,4000)};continue;}
            let json:any; try{json=JSON.parse(body)}catch{lastDebug={offset:[dx,dy],requestUrl,responseText:body.slice(0,4000),parseError:true};continue;}
            successful=true; const hits=Array.isArray(json?.results)?json.results:[]; const hit=hits.find((h:any)=>normalizeBnpbRasterValue(h?.value)!==null); const value=normalizeBnpbRasterValue(hit?.value);
            lastDebug={offset:[dx,dy],requestUrl,response:json,resultCount:hits.length,hits:hits.slice(0,10)};
            if(json?.error) continue; if(value!==null){found=value;raw=hit?.value;break;}
          }
          if(found!==null) results.push({service:config.name,url:config.url!,status:"value",value:String(found),rawValue:raw,debug:{samplingOffsetsMeters:offsets,lastResponse:lastDebug}});
          else if(successful) results.push({service:config.name,url:config.url!,status:"nodata",message:"Semua titik sampling tidak mengembalikan nilai raster BNPB.",debug:{samplingOffsetsMeters:offsets,lastResponse:lastDebug}});
          else results.push({service:config.name,url:config.url!,status:"error",message:"Tidak mendapat response Identify BNPB yang dapat dibaca.",debug:{samplingOffsetsMeters:offsets,lastResponse:lastDebug}});
        } catch(e:any){results.push({service:config.name,url:config.url!,status:"error",message:e?.message||"Gagal menghubungi service BNPB.",debug:{exception:String(e?.stack||e)}});}
      }
      setBnpbIdentifyPopup({open:true,loading:false,error:"",latitude,longitude,results});
      console.groupCollapsed("🛰️ BNPB Identify V3"); console.log("Point 4326",{latitude,longitude}); console.log("Point 3395",p); console.log("Results",results); console.groupEnd();
    } catch(e:any){setBnpbIdentifyPopup(prev=>({...prev,loading:false,error:e?.message||"Gagal melakukan Identify BNPB."}));}
  };
  ...
  const handleBnpbToggle = async (key: string, checked: boolean) => {
    ...
    if (checked) await loadBnpbLayer(key); else removeBnpbLayer(key);
  };

  const refreshActiveBnpbLayers = async () => {
    for (const config of BNPB_INARISK_LAYERS) {
      if (config.url && activeLayers.has(config.key)) await loadBnpbLayer(config.key);
    }
  };
  ...
  const searchLocationOnMap = async (query: string) => {
    ...
    try {
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=jsonv2&limit=1&countrycodes=id&q=${encodeURIComponent(trimmed)}`,
        { headers: { Accept: "application/json" } },
      );
      ...
      if (mapWeatherMarkerRef.current) {
        ...
        mapWeatherMarkerRef.current = window.L.marker([latitude, longitude]).addTo(
  ...
  const handleLocationSearchChange = (value: string) => {
    ...
    // Debounce supaya request tidak dikirim pada setiap karakter yang diketik.
    locationSearchTimerRef.current = setTimeout(() => {
      void searchLocationOnMap(value);
    }, 700);
  };
  ...
  // Render boundary wilayah + otomatis menuju/zoom ke wilayah yang dipilih.
  // Fungsi existing Kerawanan (6) tetap dipertahankan; perubahan ini hanya
  // menambahkan visual boundary dan navigasi peta setelah hasil pencarian dipilih.
  const renderAndZoomToAreaBoundary = (area: any) => {
    const map = mapInstanceRef.current;
    if (!map || !window.L || !area?.geom) {
      console.warn("Boundary wilayah tidak tersedia:", area?.label);
      return;
    }
    ...
    try {
      ...
      if (typeof geometry === "string") {
        try {
          ...
          console.warn("Geometry wilayah bukan JSON yang valid:", area.label);
          return;
        }
      }

      // Hapus boundary dengan nama yang sama agar tidak menumpuk.
      const boundaryKey = `admin_boundary_${String(area.label || "area")
        .replace(/[^a-zA-Z0-9_-]+/g, "_")}`;
      if (layerGroupsRef.current[boundaryKey]) {
        map.removeLayer(layerGroupsRef.current[boundaryKey]);
        delete layerGroupsRef.current[boundaryKey];
      }
      ...
      const geoJsonLayer = window.L.geoJSON(
        ...
        {
          style: {
            // WILAYAH HASIL PENCARIAN: sengaja dibuat berbeda dari
            // boundary DAS (biru) dan layer kerawanan agar langsung terlihat.
            color: "#f59e0b",
            weight: 4,
            opacity: 1,
            dashArray: "8 5",
          ...
          onEachFeature: (_feature: any, layer: any) => {
            if (layer?.bindTooltip && area?.label) {
            ...
            layer?.on?.({
              mouseover: () => layer.setStyle({ weight: 6, opacity: 1, fillOpacity: 0.20 }),
              mouseout: () => layer.setStyle({ weight: 4, opacity: 1, fillOpacity: 0.12 }),
            });
          },
        } as any,
      );
      ...
      // Pastikan boundary hasil pencarian selalu berada di atas overlay/layer lain.
      if (typeof (geoJsonLayer as any).bringToFront === "function") {
        (geoJsonLayer as any).bringToFront();
      }
      layerGroupsRef.current[boundaryKey] = geoJsonLayer;

      // Ini yang membuat pencarian benar-benar "menuju" wilayah,
      // bukan hanya menyimpan nama/filter. Padding menjaga polygon tidak
      // tertutup sidebar/panel.
      ...
      if (bounds && bounds.isValid()) {
        if (typeof (map as any).flyToBounds === "function") {
          (map as any).flyToBounds(bounds, {
            ...
            maxZoom: area.level === "kelurahan" ? 14 : area.level === "kecamatan" ? 12 : 10,
            duration: 0.8,
          });
        } else {
      ...
      console.log("✅ Boundary rendered + map moved to:", area.label);
    } catch (error) {
      console.error("❌ Gagal render/zoom boundary wilayah:", error);
    }
  };
  ...
  const handleAreaSelect = (area: any) => {
    ...
    if (!isAlreadySelected) {
      ...
      // Render polygon wilayah + langsung arahkan peta ke wilayah tersebut.
      // Tetap memakai `area.geom` yang dikembalikan oleh API existing, sehingga
      // filter/database Kerawanan (6) tidak diubah.
      renderAndZoomToAreaBoundary(area);
  ...
  const handleRemoveArea = async (index: number) => {
    ...
    const boundaryKey = `admin_boundary_${String(removedArea.label || "area")
    .replace(/[^a-zA-Z0-9_-]+/g, "_")}`;
    if (layerGroupsRef.current[boundaryKey]) {
      mapInstanceRef.current?.removeLayer(layerGroupsRef.current[boundaryKey]);
      delete layerGroupsRef.current[boundaryKey];
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      Object.keys(bnpbOverlayRefs.current).forEach((key) => {
        try { mapInstanceRef.current?.removeLayer(bnpbOverlayRefs.current[key]); } catch {}
      });
  ...
  // ================================================================
  // KERAWANAN (8) — compatibility layer dari KERAWANAN (6)
  //
  // Fungsi zoomend dari kerawanan(6) dipertahankan agar layer aktif
  // tetap direload ketika level zoom berubah. Implementasi fitur baru
  // dari kerawanan(7) tetap menjadi basis utama.
  // ================================================================
  const handleZoomEnd = async () => {
    ...
    console.log(
      'Zoom changed to:',
      currentZoom,
      '- Reloading layers for new simplification level',
    );
    ...
    for (const layerName of layersToReload) {
      // Layer BNPB adalah raster ImageOverlay dan tidak memakai
      // loadLayerInBounds seperti layer GeoJSON lama.
      if (String(layerName).startsWith('bnpb_')) {
        continue;
      }

      if (String(layerName).startsWith('kejadian_')) {
        const metadata = activeKejadianLayers.get(layerName);
        if (metadata) {
          await loadKejadianLayer(
            layerName,
            metadata.year,
            metadata.category,
          );
  ...
  // Dipertahankan dari kerawanan(6). Listener dipasang setelah map siap.
  useEffect(() => {
    const map = mapInstanceRef.current;
    if (!map) return;
  ...
  // Mapping kompatibilitas tab lama dari kerawanan(6).
  ...
  const fetchOpenMeteoWeather = async () => {
    ...
    const longitude = Number(import.meta.env.VITE_WEATHER_LONGITUDE ?? 106.816666);
    ...
    try {
      ...
      setOpenMeteoWeather((prev) => ({
        ...
        error:
          error?.message ||
          "Gagal mengambil data cuaca Open-Meteo.",
      }));
    }
  };
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-[#f4f7f6] text-slate-800">
      <style>{`
        ...
        .admin-boundary-tooltip {
          background:#111827; color:#fff; border:2px solid #f59e0b;
          border-radius:8px; font-weight:700; padding:5px 8px;
          box-shadow:0 4px 14px rgba(0,0,0,.25);
        }
        .admin-boundary-tooltip:before { border-top-color:#111827 !important; }
      `}</style>

      {/* =========================================================
          SIDEBAR — visual hierarchy mengikuti mockup
         ========================================================= */}
      <aside className="w-[232px] shrink-0 bg-[#064238] text-white flex flex-col z-[3000] shadow-[8px_0_30px_rgba(6,66,56,.12)]">
        <div className="px-5 py-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-[#0b7257] font-black text-lg shadow-sm">S</div>
            <div className="min-w-0">
              <div className="text-[18px] font-extrabold tracking-wide leading-none">SIMITIGASI</div>
              <div className="text-[7px] text-emerald-100/80 leading-tight mt-1">Sistem Informasi Mitigasi dan Adaptasi<br/>Bencana Hidrometeorologi Kehutanan</div>
            </div>
          </div>
        </div>

        <nav className="flex-1 overflow-y-auto dashboard-scroll px-3 py-4">
          <div className="text-[9px] uppercase tracking-[.18em] text-emerald-300/80 px-3 pb-2">Menu Utama</div>
          {[
            ["Beranda","⌂","/"],
            ...
            <button key={label} onClick={()=>navigate(path)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-[11px] transition ${i===1?"bg-[#0d9a70] text-white shadow-sm":"text-emerald-50/80 hover:bg-white/10"}`}>
              <span className="w-5 text-center text-sm opacity-90">{icon}</span><span>{label}</span>
            </button>
          ))}

          <div className="text-[9px] uppercase tracking-[.18em] text-emerald-300/80 px-3 pt-5 pb-2">Alat & Data</div>
          {[
            ["Lokasi Saya","⌖"], ["Laporan & Unduhan","▤"], ["API / Data Terbuka","‹/›"]
          ].map(([label,icon])=>(
            <button key={label} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-[11px] text-emerald-50/80 hover:bg-white/10 transition">
              <span className="w-5 text-center text-sm">{icon}</span><span>{label}</span>
            </button>
          ))}

          <div className="text-[9px] uppercase tracking-[.18em] text-emerald-300/80 px-3 pt-5 pb-2">Pengaturan</div>
          <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-[11px] text-emerald-50/80 hover:bg-white/10 transition"><span className="w-5 text-center">⚙</span><span>Pengaturan</span></button>
        </nav>

        <div className="m-3 rounded-xl border border-emerald-300/20 bg-[#0b5a49]/70 p-3">
          <div className="flex items-center justify-between text-[9px] font-semibold text-emerald-100"><span>Lokasi Saya</span><button className="rounded-md bg-[#15966e] px-2 py-1 text-[8px]">Ubah</button></div>
          <div className="text-[11px] font-extrabold mt-2">{selectedRegionName}</div>
          <div className="text-[9px] text-emerald-100/70 mt-1">{selectedAreas[0]?.kab_kota || selectedDas[0]?.label || "Indonesia"}</div>
          <div className="text-[9px] text-emerald-100/80 mt-2">⌖ {mapWeatherPopup.latitude?.toFixed(4) || "—"}, {mapWeatherPopup.longitude?.toFixed(4) || "—"}</div>
        </div>
      </aside>
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        <div className="shrink-0 bg-white border-b border-slate-200">
          ...
          {isAuthenticated && <div className="px-4 py-1 bg-emerald-50 border-t border-emerald-100 flex items-center justify-between"><span className="text-[10px] text-emerald-800 font-medium">● Logged in as Admin</span><button onClick={handleLogout} className="text-[10px] font-semibold text-red-600">Logout</button></div>}
        </div>

        <main className="flex-1 min-h-0 p-2 md:p-3 overflow-hidden">
          <div className="h-full grid grid-cols-[minmax(0,1fr)_350px] grid-rows-[minmax(0,1fr)_184px] gap-2">

            {/* =====================================================
                MAP — pusat visual seperti mockup
               ===================================================== */}
            <section className="relative min-h-0 rounded-xl overflow-hidden bg-sky-100 border border-white shadow-sm">
              <div ref={mapRef} className="absolute inset-0" />

              {/* Horizontal layer bar */}
              <div className="absolute left-2 right-2 top-2 z-[1700] h-10 bg-white/95 backdrop-blur rounded-lg border border-slate-200 shadow-[0_5px_18px_rgba(15,23,42,.12)] flex items-center overflow-x-auto dashboard-scroll px-1 gap-1">
                <button onClick={()=>setShowLayerPanel(true)} className="mockup-chip h-8 px-3 rounded-md bg-[#087f61] text-white text-[10px] font-bold">▱ Semua Layer</button>
                {["Administrasi","Kawasan Hutan","DAS","Kerawanan","Tutupan Lahan","Hotspot","Curah Hujan","Pos Hujan","Adaptasi","EWS"].map((label)=>(
                  <button key={label} onClick={()=>setShowLayerPanel(true)} className="mockup-chip h-8 px-3 rounded-md text-[9px] font-semibold text-slate-600 hover:bg-slate-100">{label}</button>
                ))}
              </div>

              {/* Search lokasi */}
              <div className="absolute top-[58px] left-3 z-[1700] w-[285px] mockup-map-search">
                <div className="h-9 bg-white/95 rounded-lg shadow-md border border-slate-200 flex items-center px-3 gap-2">
                  <span className="text-slate-400 text-sm">⌕</span>
                  <input value={areaSearchQuery} onChange={(e)=>handleLocationSearchChange(e.target.value)} onKeyDown={handleLocationSearchKeyDown} className="flex-1 outline-none text-[10px] bg-transparent" placeholder="Cari lokasi, nama daerah, koordinat, atau DAS..." aria-label="Cari lokasi di peta" />
                  {locationSearchLoading && <span className="text-[8px] text-slate-400 animate-pulse">Mencari...</span>}
                </div>
              </div>
              ...
              <div className="absolute left-3 top-[105px] z-[1600] flex flex-col rounded-lg overflow-hidden border border-slate-200 shadow-md bg-white">
                <button onClick={()=>mapInstanceRef.current?.zoomIn()} className="w-9 h-9 text-slate-700 text-lg hover:bg-slate-50">+</button>
                <button onClick={()=>mapInstanceRef.current?.zoomOut()} className="w-9 h-9 border-t text-slate-700 text-lg hover:bg-slate-50">−</button>
              </div>

              {/* Layer drawer — fungsi lama tetap dipertahankan */}
              {showLayerPanel && (
                <div className="absolute left-3 top-[152px] bottom-3 z-[1800] w-[245px] bg-white rounded-xl border border-slate-200 shadow-[0_16px_45px_rgba(15,23,42,.18)] overflow-hidden flex flex-col">
                  <div className="px-4 py-3 border-b border-slate-100 flex items-start justify-between">
                    <div><div className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Layer Services</div><div className="text-sm font-extrabold">SIMITIGASI • Layer</div></div>
                    <button onClick={()=>setShowLayerPanel(false)} className="text-slate-400 text-lg">×</button>
                  </div>
                  {layerError && <div className="mx-3 mt-3 rounded-lg bg-red-50 border border-red-100 p-2 text-[9px] text-red-700">{layerError}</div>}
                  <div className="flex-1 overflow-y-auto dashboard-scroll p-3">
                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-2 pb-1.5 border-b border-slate-200"><h3 className="text-[11px] font-bold text-slate-800">Kerawanan</h3><button onClick={()=>handleAddClick("kerawanan")} className="text-[9px] text-emerald-700">+ Data</button></div>
                      <div className="space-y-1">
                        {(currentBounds && (selectedAreas.length>0 || selectedDas.length>0) ? availableLayers.kerawanan : layerData.kerawanan).map(layer=>(
                          <label key={layer.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 cursor-pointer">
                            <input type="checkbox" className="w-3.5 h-3.5 accent-emerald-700" checked={activeLayers.has(layer.name)} onChange={e=>handleLayerToggle(layer.name,e.target.checked)} disabled={loadingLayerNames.has(layer.name)}/>
                            <span className="text-[10px] flex-1 truncate">{loadingLayerNames.has(layer.name)?"⏳ ":""}{formatTableName(layer.name)}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                    {(["mitigasiAdaptasi","lainnya","kejadian"] as const).map(section=>{
                      const source:any = currentBounds && (selectedAreas.length>0 || selectedDas.length>0) ? availableLayers[section] : layerData[section];
                      const title = section === "mitigasiAdaptasi" ? "Mitigasi & Adaptasi" : section === "lainnya" ? "Lainnya" : "Kejadian";
                      return <div key={section} className="mb-4"><div className="flex items-center justify-between mb-2 pb-1.5 border-b border-slate-200"><h3 className="text-[11px] font-bold text-slate-800">{title}</h3>{section !== "kejadian" && <button onClick={()=>handleAddClick(section)} className="text-[9px] text-emerald-700">+ Data</button>}</div><div className="space-y-1">{(source||[]).map((layer:any)=><label key={layer.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 cursor-pointer"><input type="checkbox" className="w-3.5 h-3.5 accent-emerald-700" checked={activeLayers.has(layer.isManual?layer.name:layer.id)} onChange={e=>handleLayerToggle(layer.isManual?layer.name:layer.id,e.target.checked,layer.year,layer.category,layer.isShapefile)} disabled={loadingLayerNames.has(layer.name)}/><span className="text-[10px] flex-1 truncate">{formatTableName(layer.name)}</span></label>)}</div></div>;
                    })}
                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-2 pb-1.5 border-b border-slate-200"><h3 className="text-[11px] font-bold">BNPB InaRISK</h3><span className="text-[8px] font-bold text-emerald-700">LIVE</span></div>
                      <div className="space-y-1">{BNPB_INARISK_LAYERS.map(layer=><label key={layer.key} className={`flex items-center gap-2 px-2 py-1.5 rounded-lg ${layer.url?"hover:bg-blue-50 cursor-pointer":"opacity-45 cursor-not-allowed"}`}><input type="checkbox" className="w-3.5 h-3.5 accent-emerald-700" checked={activeLayers.has(layer.key)} onChange={e=>handleBnpbToggle(layer.key,e.target.checked)} disabled={!layer.url||loadingLayerNames.has(layer.key)}/><span className="text-[10px] flex-1 truncate">{layer.name}</span></label>)}</div>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* Legend */}
              <div className="absolute right-3 bottom-3 z-[1700] w-[190px] bg-white/95 rounded-xl border border-slate-200 shadow-lg overflow-hidden">
                <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between"><b className="text-[11px]">Legenda</b><span className="text-[8px] text-slate-400">{activeLayerCount} aktif</span></div>
                <div className="p-3 space-y-1.5 text-[9px]">
                  {[['#7BC96F','Rendah'],['#F4D35E','Sedang'],['#F59E0B','Tinggi'],['#E4572E','Sangat Tinggi']].map(([c,l])=><div key={l} className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full" style={{background:c}}/><span className="text-slate-600">{l}</span></div>)}
                  <div className="pt-2 mt-2 border-t border-slate-100 font-semibold text-slate-500">Kejadian Bencana</div>
                  {[['#3b82f6','Banjir'],['#f59e0b','Longsor'],['#ef4444','Karhutla'],['#8b5cf6','Hujan Lebat']].map(([c,l])=><div key={l} className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full border-2" style={{borderColor:c}}/><span>{l}</span></div>)}
                </div>
              </div>

              {/* Bottom map controls / timeline */}
              <div className="absolute left-3 right-[205px] bottom-3 z-[1700] flex items-center gap-2">
                <div className="flex-1 min-w-0 bg-white/95 rounded-lg border border-slate-200 shadow-md h-10 flex items-center px-3 gap-4 overflow-hidden">
                  <button className="text-slate-500">‹</button>
                  {['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agt','Sep','Okt','Nov','Des'].map((m)=><button key={m} className={`text-[8px] font-semibold ${m==='Mei'?'text-emerald-700':'text-slate-400'}`}>{m}</button>)}
                  <button className="text-slate-500 ml-auto">⌃</button>
                </div>
                <button className="h-10 px-4 bg-white/95 rounded-lg border border-slate-200 shadow-md text-[9px] font-semibold text-slate-600">↻ Animasi</button>
                <button onClick={()=>refreshActiveBnpbLayers()} className="h-10 px-4 bg-white/95 rounded-lg border border-slate-200 shadow-md text-[9px] font-semibold text-emerald-700">⟳ Real-time</button>
              </div>

              {bnpbIdentifyPopup.open && <div className="absolute right-3 bottom-14 z-[1900] w-[360px] max-w-[calc(100%-24px)]">
                <div className="rounded-xl bg-white border border-slate-200 shadow-xl overflow-hidden">
                  <div className="bg-[#0b4d3c] text-white px-4 py-3 flex justify-between">
                    <div><div className="text-[8px] text-emerald-200 uppercase">BNPB InaRISK • IDENTIFY</div><div className="text-sm font-bold">Informasi Risiko / Bahaya</div></div>
                    <button onClick={()=>setBnpbIdentifyPopup(p=>({...p,open:false}))}>×</button>
                  </div>
                  <div className="p-4 text-[10px] max-h-[320px] overflow-y-auto">
                    <div className="mb-3 rounded-lg bg-slate-50 border border-slate-100 p-2 text-[9px] text-slate-500">
                      Titik: {bnpbIdentifyPopup.latitude?.toFixed(6)}, {bnpbIdentifyPopup.longitude?.toFixed(6)}
                    </div>
                    {bnpbIdentifyPopup.loading ? "Memeriksa data raster BNPB..." : bnpbIdentifyPopup.error ? bnpbIdentifyPopup.error : bnpbIdentifyPopup.results.map((r,i)=><div key={i} className="py-2.5 border-b last:border-0">
                      <div className="flex items-center justify-between gap-2"><b>{r.service}</b><span className={`text-[8px] font-bold px-2 py-0.5 rounded-full ${r.status==='value'?'bg-emerald-50 text-emerald-700':r.status==='out_of_coverage'?'bg-amber-50 text-amber-700':r.status==='nodata'?'bg-slate-100 text-slate-500':'bg-red-50 text-red-700'}`}>{r.status==='value'?'TERSEDIA':r.status==='out_of_coverage'?'DI LUAR CAKUPAN':r.status==='nodata'?'NODATA':'ERROR'}</span></div>
                      <div className="mt-1 text-slate-600 font-semibold">{r.status==='value' ? `Nilai API: ${r.value}` : r.message}</div>
                      {r.debug && <details className="mt-2"><summary className="cursor-pointer text-[8px] font-semibold text-slate-500">Lihat response BNPB (debug)</summary><pre className="mt-1 max-h-48 overflow-auto rounded bg-slate-950 text-emerald-200 p-2 text-[7px] leading-3 whitespace-pre-wrap break-all">{JSON.stringify(r.debug, null, 2)}</pre></details>}
                    </div>)}
                  </div>
                </div>
              </div>}
            ...
            {/* =====================================================
               ...
               ===================================================== */}
            <aside className="row-span-2 min-h-0 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
              <div className="px-4 py-3 border-b border-slate-100">
                <div className="flex items-center justify-between"><div><div className="text-[14px] font-extrabold">Informasi Lokasi Terpilih</div><div className="text-[8px] text-slate-400 mt-1">Data diperbarui: {formatWeatherTime(openMeteoWeather.updatedAt)} WIB</div></div><span className="text-[8px] font-bold text-emerald-700 bg-emerald-50 rounded-full px-2 py-1">● Data Terbaru</span></div>
                <div className="mt-3 flex gap-4 overflow-x-auto text-[8px] font-semibold text-slate-400">
                  {['Ringkasan','Risiko','Historis','Mitigasi','Adaptasi','Dokumen'].map(tab=><button key={tab} onClick={()=>setSelectedInfoTab(tab)} className={`pb-2 whitespace-nowrap border-b-2 ${selectedInfoTab===tab?'border-emerald-600 text-emerald-700':'border-transparent'}`}>{tab}</button>)}
                </div>
              </div>

              <div className="flex-1 overflow-y-auto dashboard-scroll p-4">
                {selectedInfoTab === 'Ringkasan' && <>
                  <div className="text-[10px] font-bold text-slate-700 mb-3">A. Rincian Lokasi</div>
                  <div className="grid grid-cols-2 gap-x-5 gap-y-3 text-[9px]">
                    <div><span className="text-slate-400 block">Nama Lokasi</span><b>{selectedRegionName}</b></div>
                    <div><span className="text-slate-400 block">Tutupan Lahan Dominan</span><b>{tutupanLahanData[0]?.deskripsi_domain || '—'}</b></div>
                    <div><span className="text-slate-400 block">Administrasi</span><b>{selectedAreas[0]?.kab_kota || selectedAreas[0]?.provinsi || '—'}</b></div>
                    <div><span className="text-slate-400 block">Tutupan Hutan Sekunder</span><b>—</b></div>
                    <div><span className="text-slate-400 block">DAS / Sub-DAS</span><b>{selectedDas[0]?.nama_das || '—'}</b></div>
                    <div><span className="text-slate-400 block">Elevasi</span><b>—</b></div>
                    <div><span className="text-slate-400 block">Koordinat</span><b>{mapWeatherPopup.latitude!=null ? `${mapWeatherPopup.latitude.toFixed(4)}° LS, ${mapWeatherPopup.longitude?.toFixed(4)}° BT` : '—'}</b></div>
                    <div><span className="text-slate-400 block">Kemiringan Lereng</span><b>—</b></div>
                  </div>

                  <div className="mt-5">
                    <div className="text-[10px] font-bold text-slate-700 mb-3">Informasi Risiko (BNPB InaRISK)</div>
                    <div className="space-y-2">
                      {bnpbIdentifyPopup.results.filter(r => activeLayers.has(BNPB_INARISK_LAYERS.find(x => x.name === r.service)?.key || "")).map((r, i) => (
                        <div key={i} className="rounded-lg border border-slate-100 bg-slate-50 p-2.5">
                          <div className="flex items-center justify-between gap-2">
                            <b className="text-[9px] text-slate-700">{r.service}</b>
                            <span className={`text-[7px] font-bold px-2 py-1 rounded-full ${r.status==='value'?'bg-emerald-50 text-emerald-700':r.status==='out_of_coverage'?'bg-amber-50 text-amber-700':r.status==='nodata'?'bg-slate-200 text-slate-600':'bg-red-50 text-red-700'}`}>{r.status==='value'?'DATA ADA':r.status==='out_of_coverage'?'DI LUAR CAKUPAN':r.status==='nodata'?'NODATA':'ERROR'}</span>
                          </div>
                          <div className="mt-1.5 text-sm font-extrabold text-slate-800">{r.status==='value' ? r.value : '—'}</div>
                          <div className="mt-1 text-[7px] text-slate-400">{r.status==='value' ? 'Nilai raster dari BNPB InaRISK' : r.message}</div>
                        </div>
                      ))}
                      {!bnpbIdentifyPopup.results.length && <div className="rounded-lg border border-dashed border-slate-200 p-3 text-[8px] text-slate-400">Klik titik di peta setelah layer BNPB diaktifkan untuk mengambil nilai risiko.</div>}
                    </div>
                  </div>

                  <div className="mt-4 rounded-lg border border-orange-100 bg-orange-50/70 p-3 flex items-center justify-between"><div><div className="text-[8px] text-orange-700 font-semibold">Status Risiko Keseluruhan</div><div className="text-sm font-extrabold text-orange-700 mt-1">{activeLayerCount>0?'Terpetakan':'Belum dihitung'}</div></div><span className="text-xl">♨</span></div>

                  <div className="text-[10px] font-bold text-slate-700 mt-5 mb-3">B. Indeks Risiko Multi-bencana</div>
                  <div className="grid grid-cols-5 gap-1.5">
                    {[['Banjir',risikoData.risiko_banjir],['Longsor',risikoData.risiko_longsor],['Karhutla',risikoData.risiko_karhutla],['Kekeringan',risikoData.risiko_kekeringan],['Banjir Bandang',risikoData.risiko_banjir_bandang]].map(([label,data]:any)=><div key={label} className="rounded-lg border border-slate-100 p-2 text-center"><div className="text-[7px] text-slate-400 leading-tight">{label}</div><div className="text-sm font-extrabold mt-1">{data?.length||0}</div><div className="text-[7px] text-slate-400">kelas data</div><div className="h-1 rounded-full bg-emerald-100 mt-2 overflow-hidden"><div className="h-full bg-emerald-500" style={{width:`${Math.min(100,(data?.length||0)*20)}%`}}/></div></div>)}
                  </div>

                  <div className="text-[10px] font-bold text-slate-700 mt-5 mb-3">C. Kondisi Lingkungan & Pemantauan (Real-time)</div>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ['Curah Hujan',openMeteoWeather.current?.precipitation!=null?`${openMeteoWeather.current.precipitation} mm`:'—'],
                      ['Suhu Udara',openMeteoWeather.current?.temperature_2m!=null?`${openMeteoWeather.current.temperature_2m} °C`:'—'],
                      ['Kelembapan',openMeteoWeather.current?.relative_humidity_2m!=null?`${openMeteoWeather.current.relative_humidity_2m}%`:'—'],
                      ['Tekanan',openMeteoWeather.current?.surface_pressure!=null?`${openMeteoWeather.current.surface_pressure} hPa`:'—'],
                      ['Prakiraan Hujan',openMeteoWeather.daily?.rain_sum?.[0]!=null?`${openMeteoWeather.daily.rain_sum[0]} mm`:'—'],
                      ['Hotspot',kejadianListings.length?`${kejadianListings.length} titik`:'—'],
                    ].map(([label,value])=><div key={label} className="rounded-lg bg-slate-50 border border-slate-100 p-2.5"><div className="text-[7px] text-slate-400">{label}</div><div className="text-[11px] font-extrabold mt-1">{value}</div></div>)}
                  </div>
                  <div className="mt-2 rounded-lg border border-emerald-100 bg-emerald-50 p-3 flex items-center justify-between"><div><div className="text-[8px] text-emerald-700 font-semibold">Stasiun EWS</div><div className="text-sm font-extrabold text-emerald-700">{activeLayerCount>0?'Waspada':'Normal'}</div></div><span className="text-xl text-emerald-600">△</span></div>
                </>}

                {selectedInfoTab === 'Risiko' && <div className="space-y-3"><div className="text-[10px] font-bold">Ringkasan Risiko</div>{[['Banjir',risikoData.risiko_banjir],['Longsor',risikoData.risiko_longsor],['Karhutla',risikoData.risiko_karhutla],['Kekeringan',risikoData.risiko_kekeringan],['Banjir Bandang',risikoData.risiko_banjir_bandang]].map(([label,data]:any)=><div key={label} className="rounded-lg border p-3"><div className="flex justify-between text-[9px]"><b>{label}</b><span>{data?.length||0} kelas</span></div><div className="mt-2 h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-emerald-500" style={{width:`${Math.min(100,(data?.length||0)*20)}%`}}/></div></div>)}</div>}
                {selectedInfoTab === 'Historis' && <div><div className="text-[10px] font-bold mb-3">Kejadian Terkini di Sekitar Lokasi</div><div className="space-y-2">{kejadianListings.slice(0,8).map((item:any,i)=><div key={i} className="flex gap-2 rounded-lg bg-slate-50 p-2"><span className="w-6 h-6 rounded-full bg-orange-100 text-orange-700 flex items-center justify-center text-[9px]">⚠</span><div className="min-w-0"><b className="text-[9px] block truncate">{item.title||item.incident_type||'Kejadian'}</b><span className="text-[8px] text-slate-400">{item.incident_date||'Tanggal tidak tersedia'}</span></div></div>)}{!kejadianListings.length&&<div className="text-[9px] text-slate-400">Aktifkan layer kejadian untuk menampilkan histori.</div>}</div></div>}
                {selectedInfoTab === 'Mitigasi' && <div><div className="text-[10px] font-bold mb-3">Rekomendasi Mitigasi & Adaptasi (AI)</div><div className="space-y-2">{['Rehabilitasi Hutan & Lahan Kritis','Pembangunan Embung & Sumur Resapan','Sistem Peringatan Dini (EWS) Komunitas','Penguatan Sekat Kanal & Pembasahan Gambut'].map((x,i)=><div key={x} className="flex gap-2 p-3 rounded-lg bg-emerald-50 border border-emerald-100"><span className="text-emerald-700">✓</span><span className="text-[9px] font-semibold">{x}</span></div>)}</div></div>}
                {selectedInfoTab === 'Adaptasi' && <div><div className="text-[10px] font-bold mb-3">Data Adaptasi</div><div className="grid grid-cols-2 gap-2">{[['Rehabilitasi DAS',rehabilitasiDasData.length],['Rehabilitasi Hutan',rehabilitasiHutanData.length],['Restorasi Gambut',restorasiGambutData.length],['Teknik KTA',penerapanTeknikKtaData.length]].map(([l,v])=><div key={l} className="border rounded-lg p-3"><div className="text-[8px] text-slate-400">{l}</div><b className="text-lg">{v}</b><div className="text-[7px] text-slate-400">record tersedia</div></div>)}</div></div>}
                {selectedInfoTab === 'Dokumen' && <div><div className="text-[10px] font-bold mb-3">Dokumen & Data Terbuka</div><div className="space-y-2">{['Laporan kondisi lokasi','Data layer aktif','Metadata layer','Ringkasan API'].map(x=><button key={x} className="w-full text-left border rounded-lg p-3 hover:bg-slate-50"><div className="text-[9px] font-bold">{x}</div><div className="text-[8px] text-slate-400 mt-1">Tersedia melalui modul Laporan & Unduhan</div></button>)}</div></div>}
              </div>
            </aside>
            ...
            {/* =====================================================
               ...
               ===================================================== */}
            <section className="min-h-0 grid grid-cols-[1.35fr_1fr_1fr] gap-2">
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-3 overflow-hidden">
                <div className="flex items-center justify-between"><div><div className="text-[8px] uppercase tracking-wide text-slate-400 font-bold">Tren Kejadian 5 Tahun Terakhir</div><div className="text-[11px] font-extrabold mt-1">Jumlah Kejadian</div></div><span className="text-[8px] text-slate-400">Semua Jenis</span></div>
                <div className="h-[112px] mt-2 flex items-end gap-3 px-2">
                  {[2021,2022,2023,2024,2025].map((year,i)=>{const v=[rawanLimpasanData.length,rawanLongsorData.length,rawanKarhutlaData.length,kejadianListings.length,totalEvents][i]||0; const h=Math.max(5,Math.min(88,v*8)); return <div key={year} className="flex-1 flex flex-col items-center justify-end gap-1"><div className="w-full max-w-[22px] rounded-t bg-emerald-500/80" style={{height:h}}/><span className="text-[7px] text-slate-400">{year}</span></div>})}
                </div>
              </div>

              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-3 overflow-hidden">
                <div className="text-[8px] uppercase tracking-wide text-slate-400 font-bold">Sebaran Tingkat Kerawanan</div>
                <div className="flex items-center gap-4 mt-3">
                  <div className="w-[92px] h-[92px] rounded-full border-[16px] border-emerald-400 relative shrink-0"><div className="absolute inset-[-16px] rounded-full border-[16px] border-transparent border-t-orange-400 border-r-amber-300 rotate-12"/><div className="absolute inset-0 flex flex-col items-center justify-center"><b className="text-sm">{rawanLongsorData.length+rawanLimpasanData.length+rawanKarhutlaData.length}</b><span className="text-[7px] text-slate-400">kelas</span></div></div>
                  <div className="space-y-2 text-[8px]"><div>● <b>Rendah</b> <span className="text-slate-400">data tersedia</span></div><div>● <b>Sedang</b> <span className="text-slate-400">data tersedia</span></div><div>● <b>Tinggi</b> <span className="text-slate-400">data tersedia</span></div><div>● <b>Sangat Tinggi</b> <span className="text-slate-400">data tersedia</span></div></div>
                </div>
              </div>

              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-3 overflow-hidden">
                <div className="flex justify-between items-center"><div><div className="text-[8px] uppercase tracking-wide text-slate-400 font-bold">Kejadian Terkini di Sekitar Lokasi</div><div className="text-[7px] text-slate-400 mt-1">{kejadianListings.length||0} record</div></div><button className="text-[8px] text-emerald-700 font-semibold">Lihat Semua</button></div>
                <div className="mt-2 space-y-1.5">{kejadianListings.slice(0,4).map((item:any,i)=><div key={i} className="flex items-center gap-2"><span className="w-5 h-5 rounded-full bg-orange-100 flex items-center justify-center text-[8px]">⚠</span><div className="min-w-0 flex-1"><b className="text-[8px] block truncate">{item.title||item.incident_type||'Kejadian Bencana'}</b><span className="text-[7px] text-slate-400">{item.incident_date||'—'}</span></div></div>)}{!kejadianListings.length&&<div className="text-[8px] text-slate-400 py-4">Belum ada kejadian aktif untuk lokasi ini.</div>}</div>
              </div>
            </section>
          </div>
        </main>
          ...
          >
            <div className="flex justify-between items-center p-4 border-b">
              <h3 className="text-base font-semibold">
                Tambah Data {getSectionTitle(currentSection)}
              </h3>
              <button
                onClick={() => !isUploading && setShowAddModal(false)}
            ...
            <div className="p-4">
              ...
              <div className="text-[10px] text-gray-500 mt-2">
                * Wajib upload minimal file .shp, disarankan juga upload .shx,
                .dbf, .prj
              </div>

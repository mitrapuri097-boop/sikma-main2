...
const Kerawanan = () => {
  ...
  // Ringkasan nilai risiko BNPB yang ditampilkan di panel informasi lokasi.
  const [bnpbRiskSummary, setBnpbRiskSummary] = useState<Array<{
    service: string;
    value: string;
    url: string;
    label: string;
  ...
  const normalizeBnpbRiskValue = (raw: any) => {
    const text = String(raw ?? "").trim();
    if (!text) return "Tidak ada nilai";

    const lower = text.toLowerCase();
    if (lower.includes("sangat tinggi")) return "Sangat Tinggi";
    if (lower === "tinggi" || lower.includes(" tinggi")) return "Tinggi";
    if (lower.includes("sedang")) return "Sedang";
    if (lower === "rendah" || lower.includes(" rendah")) return "Rendah";
    if (lower.includes("sangat rendah")) return "Sangat Rendah";

    // InaRISK raster umumnya mengembalikan kelas numerik 1–5.
    // Pemetaan ini mengikuti urutan kelas pada legenda dashboard: 1 rendah -> 5 sangat tinggi.
    ...
    if (Number.isFinite(numeric)) {
      ...
      if (numericMap[numeric]) return numericMap[numeric];
      return `Nilai ${text}`;
    }
  ...
  const identifyBnpbAtPoint = async (latitude: number, longitude: number, showPopup = true) => {
    ...
    if (!active.length || !mapInstanceRef.current) {
      setBnpbRiskSummary([]);
      if (showPopup) setBnpbIdentifyPopup((prev) => ({ ...prev, open: false }));
      return;
    }
    ...
    try {
      ...
      const size = map.getSize();
      const mapExtent = `${b.getWest()},${b.getSouth()},${b.getEast()},${b.getNorth()}`;
      const results: Array<{ service: string; url: string; value?: string; attributes?: Record<string, any> }> = [];
      ...
      for (const config of active) {
        ...
        try {
          ...
          hits.forEach((hit: any) => {
            ...
            const firstValue = Object.values(attrs).find((v) => v !== null && v !== undefined && String(v).trim() !== "");
            const rawValue = firstValue == null ? hit?.value : firstValue;
            results.push({
              service: config.name,
              url: config.url!,
      ...
      const summary = active.map((config) => {
        const hit = results.find((r) => r.service === config.name);
        return {
          service: config.name,
          value: hit?.value || "Tidak ada nilai pada titik ini",
          label: hit?.value ? normalizeBnpbRiskValue(hit.value) : "Tidak ada data",
          url: config.url!,
        };
      });
      setBnpbRiskSummary(summary);

      if (showPopup) {
        setBnpbIdentifyPopup({
          open: true,
          loading: false,
          error: results.length ? "" : "Tidak ada nilai InaRISK pada titik ini.",
          latitude,
          longitude,
          results,
        });
      ...
      setBnpbRiskSummary([]);
      if (showPopup) {
        setBnpbIdentifyPopup((prev) => ({ ...prev, loading: false, error: error?.message || "Gagal melakukan Identify BNPB." }));
      }
    }
  };

  const identifyBnpbForSelectedArea = async (area: any, showPopup = false) => {
    if (!area || !mapInstanceRef.current || !window.L) return;
    const active = BNPB_INARISK_LAYERS.filter((item) => item.url && activeLayersRef.current.has(item.key));
    if (!active.length) {
      setBnpbRiskSummary([]);
      return;
    ...
    try {
      let center: any = null;
      if (area.geom) {
        const temp = window.L.geoJSON({ type: "Feature", geometry: area.geom, properties: {} });
        const bounds = temp.getBounds();
        if (bounds.isValid()) center = bounds.getCenter();
      }
      if (!center && Number.isFinite(Number(area.latitude)) && Number.isFinite(Number(area.longitude))) {
        center = { lat: Number(area.latitude), lng: Number(area.longitude) };
      }
      if (!center) return;
      await identifyBnpbAtPoint(Number(center.lat), Number(center.lng), showPopup);
    } catch (error) {
      console.warn("Gagal mengambil risiko BNPB untuk area terpilih:", error);
    }
  };
  ...
  const handleBnpbToggle = async (key: string, checked: boolean) => {
    ...
    setActiveLayers((prev) => {
      ...
      // Keep the synchronous ref in lock-step because Identify may run
      // immediately after this toggle, before React commits the state update.
      activeLayersRef.current = next;
      return next;
    });
    if (checked) {
      await loadBnpbLayer(key);
      const selectedArea = selectedAreas[0];
      if (selectedArea) await identifyBnpbForSelectedArea(selectedArea, false);
    } else {
      removeBnpbLayer(key);
      const remaining = BNPB_INARISK_LAYERS.filter((item) => item.key !== key && item.url && activeLayersRef.current.has(item.key));
      if (!remaining.length) setBnpbRiskSummary([]);
      else if (selectedAreas[0]) await identifyBnpbForSelectedArea(selectedAreas[0], false);
    }
  };
  ...
  // ======================================================
  // LOCATION NAVIGATOR
  // Ketik nama daerah -> cari ke database administrasi SIMITIGASI
  // -> pilih hasil terbaik -> render boundary -> fitBounds -> reload layer aktif.
  // Nominatim hanya menjadi fallback jika endpoint internal tidak tersedia.
  // ======================================================
  const searchLocationOnMap = async (query: string) => {
    const trimmed = query.trim();
    if (trimmed.length < 2 || !mapInstanceRef.current || !window.L) return;
    ...
    try {
      const levels = ["provinsi", "kabupaten", "kecamatan", "kelurahan"] as const;
      ...
      const score = (area: any) => {
        const label = String(area.label || "").toLowerCase();
        const names = [area.provinsi, area.kab_kota, area.kecamatan, area.kel_desa]
          .filter(Boolean)
          .map((v) => String(v).toLowerCase());
        let value = 0;
        if (label === normalized) value += 1000;
        if (names.some((name) => name === normalized)) value += 800;
        if (label.startsWith(normalized)) value += 300;
        if (label.includes(normalized)) value += 100;
        // Prefer larger administrative units when the query is generic.
        ...
        value += levelWeight[area.level] || 0;
        return value;
      };

      const unique = new Map<string, any>();
      allResults.forEach((area: any) => {
        const key = `${area.level}|${area.label}|${area.provinsi || ""}|${area.kab_kota || ""}|${area.kecamatan || ""}|${area.kel_desa || ""}`;
        if (!unique.has(key)) unique.set(key, area);
      });

      const ranked = Array.from(unique.values()).sort((a, b) => score(b) - score(a));
      setAreaSearchResults(ranked.slice(0, 12));

      if (!ranked.length) {
        // Fallback geocoder untuk nama lokasi yang tidak tersedia di tabel administrasi.
        const response = await fetch(
        ...
        if (!result || !Number.isFinite(latitude) || !Number.isFinite(longitude)) {
          throw new Error(`Lokasi "${trimmed}" tidak ditemukan.`);
        }
        ...
        mapWeatherMarkerRef.current
          ...
          .openPopup();
        await fetchMapWeather(latitude, longitude);
        return;
      }

      // ENTER memilih hasil paling relevan secara otomatis.
      await handleAreaSelect(ranked[0]);
    } catch (error: any) {
      if (requestId === locationSearchRequestRef.current) {
        console.error("Gagal mencari lokasi:", error);
        setLayerError(error?.message || "Gagal mencari lokasi.");
        setTimeout(() => setLayerError(""), 5000);
      }
    } finally {
      if (requestId === locationSearchRequestRef.current) {
  ...
  const handleLocationSearchChange = (value: string) => {
    ...
    // Tampilkan kandidat tanpa menggeser peta. Peta baru bergerak setelah klik/Enter.
    locationSearchTimerRef.current = setTimeout(() => {
      void (async () => {
        ...
        try {
          const levels = ["provinsi", "kabupaten", "kecamatan", "kelurahan"] as const;
          const responses = await Promise.all(levels.map(async (level) => {
            const response = await fetch(
              `${API_URL}/api/areas/search?query=${encodeURIComponent(value.trim())}&level=${level}`,
              { headers: { Accept: "application/json" }, cache: "no-store" },
            );
            if (!response.ok) return [];
            const data = await response.json();
          ...
          if (requestId !== locationSearchRequestRef.current) return;
          const unique = new Map<string, any>();
          responses.flat().forEach((area: any) => {
            const key = `${area.level}|${area.label}|${area.provinsi || ""}|${area.kab_kota || ""}|${area.kecamatan || ""}|${area.kel_desa || ""}`;
            if (!unique.has(key)) unique.set(key, area);
          });
          setAreaSearchResults(Array.from(unique.values()).slice(0, 12));
          setShowAreaSearchDropdown(true);
        } catch (error) {
          console.error("Error mencari area:", error);
          setAreaSearchResults([]);
          setShowAreaSearchDropdown(true);
        } finally {
  ...
  const handleLocationResultSelect = async (area: any) => {
    if (locationSearchTimerRef.current) clearTimeout(locationSearchTimerRef.current);
    setAreaSearchQuery("");
    setShowAreaSearchDropdown(false);
    setAreaSearchResults([]);
    await handleAreaSelect(area);
  };
  ...
  const handleAreaSelect = async (area: any) => {
    ...
    if (!isAlreadySelected) {
      ...
      // Setelah wilayah dipilih, ambil nilai risiko BNPB dari titik tengah wilayah
      // untuk mengisi panel informasi tanpa menunggu user klik manual.
      await identifyBnpbForSelectedArea(area, false);
    }
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#eaf6f8] text-slate-800">
      <style>{`
        ...
        .dashboard-scroll::-webkit-scrollbar-thumb { background:#b7c8c1; border-radius:999px; }
      `}</style>

      {/* Sidebar */}
      <aside className="w-[238px] shrink-0 bg-[#063d32] text-white flex flex-col shadow-xl z-[3000]">
        <div className="px-5 py-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center text-[#0b6b53] font-black">S</div>
            <div>
              <div className="text-lg font-extrabold tracking-wide">SIMITIGASI</div>
              <div className="text-[8px] text-emerald-200 leading-tight">Sistem Informasi Mitigasi dan Adaptasi<br/>Bencana Hidrometeorologi Kehutanan</div>
            </div>
          </div>
        </div>
        <nav className="flex-1 overflow-y-auto dashboard-scroll px-3 py-4 space-y-1">
          <div className="text-[9px] uppercase tracking-[0.18em] text-emerald-300 px-3 py-2">Menu Utama</div>
          {[
            ["Beranda","⌂","/"],["Peta Interaktif","◇","/kerawanan"],["Kejadian Bencana","⚠","/kebencanaan"],
            ["Kawasan Rawan","◇","/kerawanan"],["Mitigasi","◌","/kerawanan"],["Adaptasi","◍","/kerawanan"]
          ].map(([label,icon,path],i)=>(
            <button key={label} onClick={()=>navigate(path)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs transition ${i===1?"bg-[#0c7257] shadow-inner":"text-emerald-50/80 hover:bg-white/10"}`}>
              <span className="w-5 text-center text-sm">{icon}</span><span>{label}</span>
            </button>
          ))}
          <div className="text-[9px] uppercase tracking-[0.18em] text-emerald-300 px-3 pt-5 pb-2">Analisis</div>
          {["Dashboard","Statistik","Tren & Prediksi","Laporan"].map(label=>(
            <button key={label} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs text-emerald-50/80 hover:bg-white/10 transition"><span className="w-5 text-center">▦</span><span>{label}</span></button>
          ))}
          <div className="text-[9px] uppercase tracking-[0.18em] text-emerald-300 px-3 pt-5 pb-2">Data & Layanan</div>
          {["Layer Data","Dokumentasi","API Services"].map(label=>(
            <button key={label} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs text-emerald-50/80 hover:bg-white/10 transition"><span className="w-5 text-center">◫</span><span>{label}</span></button>
          ))}
        </nav>
        <div className="m-3 rounded-2xl border border-emerald-300/20 bg-emerald-900/40 p-3">
          <div className="flex items-center gap-2 text-xs font-semibold"><span className="w-2 h-2 rounded-full bg-emerald-300 animate-pulse"/> Sistem & Layanan</div>
          <div className="text-sm font-extrabold mt-2">Semua Sistem Aktif</div>
          <div className="text-[10px] text-emerald-100/70 mt-1">Terhubung ke sumber data resmi</div>
          <button className="mt-3 w-full rounded-lg bg-[#15966e] hover:bg-[#1baa7e] py-2 text-[10px] font-bold">Detail Status</button>
        </div>
      </aside>
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 min-h-0 p-2 md:p-3 overflow-hidden">
          <div className="h-full flex flex-col gap-2">
            {/* Main map workspace */}
            <section className="relative flex-1 min-h-0 rounded-2xl overflow-hidden bg-sky-100 shadow-sm border border-white">
              <div ref={mapRef} className="absolute inset-0" />

              {/* Search */}
              <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1700] w-[260px] md:w-[300px]">
                <div className="h-10 bg-white rounded-xl shadow-lg border border-slate-200 flex items-center px-3 gap-2">
                  ...
                  {locationSearchLoading && (
                    <span className="text-[9px] text-slate-400 animate-pulse">Mencari...</span>
                  )}
                </div>
              </div>

              {/* Layer services */}
              <div className="absolute left-3 top-3 bottom-3 z-[1800] w-[245px] bg-white rounded-2xl border border-slate-200 shadow-[0_16px_45px_rgba(15,23,42,.16)] overflow-hidden flex flex-col">
                <div className="px-4 py-4 border-b border-slate-100 flex items-start justify-between">
                  <div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Layer Services</div><div className="text-lg font-extrabold">SIMITIGASI • Layer</div></div>
                  <button onClick={()=>setLayerError("")} className="text-slate-400 text-xl">×</button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto dashboard-scroll p-3">
                  <div className="mb-4">
                    <h3 className="text-sm font-bold text-slate-800 mb-2 pb-1.5 border-b-2 border-blue-500">Kerawanan</h3>
                    <div className="space-y-1">
                      {(currentBounds && (selectedAreas.length>0 || selectedDas.length>0) ? availableLayers.kerawanan : layerData.kerawanan).map(layer=>(
                        <label key={layer.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 cursor-pointer">
                          <input type="checkbox" className="w-3.5 h-3.5 accent-blue-600" checked={activeLayers.has(layer.name)} onChange={e=>handleLayerToggle(layer.name,e.target.checked)} disabled={loadingLayerNames.has(layer.name)}/>
                          <span className="text-[11px] flex-1 truncate">{loadingLayerNames.has(layer.name)?"⏳ ":""}{formatTableName(layer.name)}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-2 pb-1.5 border-b-2 border-blue-500"><h3 className="text-sm font-bold">BNPB InaRISK</h3><span className="text-[8px] text-slate-400 font-bold">LIVE API</span></div>
                    <div className="space-y-1">
                      {BNPB_INARISK_LAYERS.map(layer=>(
                        <label key={layer.key} className={`flex items-center gap-2 px-2 py-1.5 rounded-lg ${layer.url?"hover:bg-blue-50 cursor-pointer":"opacity-50 cursor-not-allowed"}`}>
                          <input type="checkbox" className="w-3.5 h-3.5 accent-blue-600" checked={activeLayers.has(layer.key)} disabled={!layer.url || loadingLayerNames.has(layer.key)} onChange={e=>void handleBnpbToggle(layer.key,e.target.checked)}/>
                          <span className="text-[11px] flex-1 truncate">{loadingLayerNames.has(layer.key)?"⏳ ":""}{layer.name}</span>
                        </label>
                      ))}
                    </div>
                    <div className="mt-2 rounded-lg bg-blue-50 border border-blue-100 px-2.5 py-2 text-[9px] leading-relaxed text-slate-600">Layer bertanda <b className="text-blue-700">LIVE API</b> terhubung langsung ke sumber data resmi melalui API.</div>
                  </div>

                  {[['Mitigasi dan Adaptasi','mitigasiAdaptasi'],['Lain lain','lainnya'],['Kejadian','kejadian']].map(([title,section])=>{
                    const source:any = currentBounds && (selectedAreas.length>0 || selectedDas.length>0) ? availableLayers[section as keyof typeof availableLayers] : layerData[section as keyof typeof layerData];
                    return <div key={section} className="mb-4"><h3 className="text-sm font-bold text-slate-800 mb-2 pb-1.5 border-b-2 border-blue-500">{title}</h3><div className="space-y-1">{(source||[]).map((layer:any)=><label key={layer.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 cursor-pointer"><input type="checkbox" className="w-3.5 h-3.5 accent-blue-600" checked={activeLayers.has(layer.isManual?layer.name:layer.id)} onChange={e=>handleLayerToggle(layer.isManual?layer.name:layer.id,e.target.checked,layer.year,layer.category,layer.isShapefile)} disabled={loadingLayerNames.has(layer.name)}/><span className="text-[11px] flex-1 truncate">{formatTableName(layer.name)}</span></label>)}</div></div>;
                  })}
                </div>
              </div>

              {/* Map controls */}
              <div className="absolute left-[260px] top-4 z-[1700] flex flex-col bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
                <button className="w-9 h-9 font-bold text-lg hover:bg-slate-50">+</button><button className="w-9 h-9 font-bold text-lg border-t border-slate-100 hover:bg-slate-50">−</button><button className="w-9 h-9 border-t border-slate-100 hover:bg-slate-50">▱</button><button className="w-9 h-9 border-t border-slate-100 hover:bg-slate-50">⌖</button>
              </div>

              {/* Weather popup */}
              {mapWeatherPopup.open && <div className="absolute right-[255px] top-3 z-[1850] w-[290px] max-w-[calc(100%-270px)]"><div className="bg-white rounded-2xl border border-slate-200 shadow-[0_18px_45px_rgba(15,23,42,.18)] overflow-hidden"><div className="p-4">
                <div className="flex items-start justify-between"><div><div className="text-[9px] font-bold text-blue-600 uppercase tracking-wide">OPEN-METEO • CUACA WILAYAH</div><div className="text-lg font-extrabold mt-1">{mapWeatherPopup.locationName}</div>{(mapWeatherPopup.district||mapWeatherPopup.province)&&<div className="text-[10px] text-slate-500 mt-1">📍 {[mapWeatherPopup.district,mapWeatherPopup.province].filter(Boolean).join(', ')}</div>}</div><button onClick={closeMapWeatherPopup} className="text-slate-400 text-lg">×</button></div>
                {mapWeatherPopup.loading?<div className="py-10 text-center text-xs text-slate-500">Mengambil data cuaca...</div>:mapWeatherPopup.error?<div className="mt-4 rounded-lg bg-red-50 p-2 text-[10px] text-red-700">{mapWeatherPopup.error}</div>:mapWeatherPopup.current?<><div className="mt-4 flex items-center gap-3"><div className="w-12 h-12 rounded-full bg-sky-50 flex items-center justify-center text-2xl">{weatherIcon(mapWeatherPopup.current.weather_code)}</div><div><div className="text-3xl font-black">{mapWeatherPopup.current.temperature_2m??'—'}°C</div><div className="text-[11px] text-blue-600 font-semibold">{weatherDescription(mapWeatherPopup.current.weather_code)}</div></div></div><div className="mt-4 rounded-xl bg-blue-50 border border-blue-100 p-3"><div className="text-[9px] font-bold text-blue-600 uppercase">Curah Hujan Hari Ini</div><div className="text-2xl font-black text-blue-700">{mapWeatherPopup.dailyRain??'—'} <span className="text-sm font-semibold">mm</span></div><div className="text-[9px] text-slate-500">Saat ini: {mapWeatherPopup.current.precipitation??'—'} mm</div></div><div className="mt-2 divide-y divide-slate-100">{[["💧","Kelembapan",`${mapWeatherPopup.current.relative_humidity_2m??'—'} %`],["🌡","Terasa seperti",`${mapWeatherPopup.current.apparent_temperature??'—'} °C`],["☁","Tutupan awan",`${mapWeatherPopup.current.cloud_cover??'—'} %`],["≋","Kecepatan angin",`${mapWeatherPopup.current.wind_speed_10m??'—'} km/j`],["➤","Arah angin",`${mapWeatherPopup.current.wind_direction_10m??'—'}°`],["◉","Tekanan udara",`${mapWeatherPopup.current.surface_pressure??'—'} hPa`]].map(([i,l,v])=><div key={l} className="flex items-center justify-between py-2 text-[10px]"><span className="text-slate-600">{i} {l}</span><b>{v}</b></div>)}</div><div className="mt-2 pt-2 border-t text-[9px] text-slate-400 flex justify-between"><span>Sumber: Open-Meteo</span><span>{formatWeatherTime(mapWeatherPopup.updatedAt)} WIB</span></div></>:null}
              </div></div></div>}

              {/* Bottom legend */}
              <div className="absolute left-[260px] bottom-4 z-[1700] bg-white rounded-xl shadow-lg border border-slate-200 p-3 w-[150px]"><div className="text-[10px] font-bold mb-2">Legenda Risiko Banjir</div>{[["#dc2626","Sangat Tinggi"],["#f97316","Tinggi"],["#facc15","Sedang"],["#84cc16","Rendah"],["#22c55e","Sangat Rendah"]].map(([c,l])=><div key={l} className="flex items-center gap-2 text-[9px] py-0.5"><span className="w-4 h-2 rounded-sm" style={{background:c}}/><span>{l}</span></div>)}</div>

              {/* API verification */}
              <div className="absolute right-3 top-3 bottom-3 z-[1800] w-[230px] bg-white rounded-2xl border border-slate-200 shadow-[0_16px_45px_rgba(15,23,42,.16)] overflow-hidden flex flex-col">
                <div className="px-4 py-4 border-b border-slate-100"><div className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Verifikasi Sumber Data</div><div className="flex mt-3 border-b"><span className="px-2 pb-2 text-[10px] font-bold text-blue-600 border-b-2 border-blue-600">Ringkasan</span><span className="px-2 pb-2 text-[10px] text-slate-400">Detail API</span></div></div>
                <div className="p-4 space-y-4 overflow-y-auto dashboard-scroll"><div><div className="flex justify-between items-center"><b className="text-sm">Open-Meteo API</b><span className="text-[9px] font-bold text-green-700 bg-green-50 px-2 py-1 rounded-full">Connected</span></div><div className="mt-3 space-y-2 text-[9px]"><div className="flex justify-between"><span className="text-slate-400">Penyedia</span><b>Open-Meteo</b></div><div className="flex justify-between"><span className="text-slate-400">Endpoint</span><b>api.open-meteo.com</b></div><div className="flex justify-between"><span className="text-slate-400">Status</span><b>200 OK</b></div></div><button className="mt-3 w-full border rounded-lg py-2 text-[9px] font-semibold">Buka Dokumentasi ↗</button></div><div className="border-t pt-4"><div className="flex justify-between items-center"><b className="text-sm">BNPB InaRISK API</b><span className="text-[9px] font-bold text-green-700 bg-green-50 px-2 py-1 rounded-full">Connected</span></div><div className="mt-3 space-y-2 text-[9px]"><div className="flex justify-between"><span className="text-slate-400">Penyedia</span><b>BNPB</b></div><div className="flex justify-between"><span className="text-slate-400">Service Group</span><b>InaRISK JBTBPJ</b></div><div className="flex justify-between"><span className="text-slate-400">Status</span><b>200 OK</b></div></div><button className="mt-3 w-full border rounded-lg py-2 text-[9px] font-semibold">Buka REST Directory ↗</button></div><div className="rounded-xl bg-emerald-50 border border-emerald-100 p-3 text-[9px] text-emerald-800"><b>✓ Semua data</b><br/>bersumber dari API resmi dan diperbarui secara real-time.</div></div>
              </div>

              {bnpbIdentifyPopup.open && <div className="absolute right-[255px] bottom-3 z-[1900] w-[360px] max-w-[calc(100%-270px)]"><div className="rounded-2xl bg-white border border-slate-200 shadow-xl overflow-hidden"><div className="bg-[#0b4d3c] text-white px-4 py-3 flex justify-between"><div><div className="text-[9px] text-emerald-200 uppercase">BNPB InaRISK • IDENTIFY</div><div className="text-sm font-bold">Informasi Risiko / Bahaya</div></div><button onClick={()=>setBnpbIdentifyPopup(p=>({...p,open:false}))}>×</button></div><div className="p-4 text-[10px]">{bnpbIdentifyPopup.loading?"Memuat informasi...":bnpbIdentifyPopup.error?bnpbIdentifyPopup.error:bnpbIdentifyPopup.results.map((r,i)=><div key={i} className="py-2 border-b last:border-0"><b>{r.service}</b><div className="text-slate-500 mt-1">{normalizeBnpbRiskValue(r.value)||"Tidak ada nilai"}</div><div className="text-[9px] text-slate-400 mt-0.5">Nilai API: {r.value||"-"}</div></div>)}</div></div></div>}
            </section>

            {/* Bottom information strip */}
            <section className="h-[165px] shrink-0 grid grid-cols-1 xl:grid-cols-[1.45fr_1fr_1fr] gap-2">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 overflow-hidden"><div className="text-[10px] uppercase tracking-wide text-slate-400 font-bold">Informasi Lokasi Terpilih</div><div className="grid grid-cols-2 gap-6 mt-3"><div className="text-[10px] space-y-2"><div className="flex justify-between"><span className="text-slate-400">Provinsi</span><b>{selectedAreas[0]?.provinsi||"—"}</b></div><div className="flex justify-between"><span className="text-slate-400">Kabupaten/Kota</span><b>{selectedAreas[0]?.kab_kota||"—"}</b></div><div className="flex justify-between"><span className="text-slate-400">Kecamatan</span><b>{selectedAreas[0]?.kecamatan||"—"}</b></div><div className="flex justify-between"><span className="text-slate-400">Kelurahan</span><b>{selectedAreas[0]?.kel_desa||"—"}</b></div></div><div><div className="text-[10px] font-bold mb-2">Informasi Risiko (BNPB InaRISK)</div>{bnpbRiskSummary.length>0 ? <div className="space-y-2 max-h-[105px] overflow-y-auto pr-1">{bnpbRiskSummary.slice(0,6).map((r)=><div key={r.service} className="rounded-lg border border-slate-100 bg-slate-50 px-2 py-2"><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-semibold text-slate-700">{r.service}</span><span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${r.label.includes("Sangat Tinggi")||r.label==="Tinggi"?"bg-red-50 text-red-700":r.label==="Sedang"?"bg-amber-50 text-amber-700":r.label.includes("Rendah")?"bg-emerald-50 text-emerald-700":"bg-slate-100 text-slate-600"}`}>{r.label}</span></div><div className="mt-1 text-[9px] text-slate-400">Nilai API: {r.value}</div></div>)}</div> : BNPB_INARISK_LAYERS.some(l=>activeLayers.has(l.key)) ? <div className="text-[10px] text-slate-400">Mengambil nilai risiko BNPB dari titik tengah wilayah...</div> : <div className="text-[10px] text-slate-400">Aktifkan layer BNPB untuk melihat nilai risiko.</div>}</div></div></div>
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4"><div className="flex justify-between"><div><div className="text-[10px] uppercase tracking-wide text-slate-400 font-bold">Aksi Cepat</div><div className="text-sm font-extrabold mt-1">Kelola lokasi</div></div></div><div className="grid grid-cols-2 gap-2 mt-4"><button className="border rounded-lg py-2 text-[10px]">⌖ Zoom ke Lokasi</button><button className="border rounded-lg py-2 text-[10px]">⌁ Lihat Grafik Risiko</button><button className="border rounded-lg py-2 text-[10px]">⇩ Unduh Laporan</button><button className="border rounded-lg py-2 text-[10px]">↗ Bagikan Lokasi</button></div></div>
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 overflow-hidden"><div className="flex justify-between"><div><div className="text-[10px] uppercase tracking-wide text-slate-400 font-bold">Log Permintaan API</div><div className="text-[10px] text-slate-400">Terakhir</div></div><span className="text-[9px] text-blue-600">Lihat Semua</span></div><div className="mt-3 space-y-2 text-[9px]"><div className="flex items-center gap-2 p-2 bg-emerald-50 rounded-lg"><span className="font-bold text-emerald-700">200</span><div className="flex-1"><b>Open-Meteo Forecast</b><div className="text-slate-400">GET /v1/forecast</div></div><span className="text-slate-400">{formatWeatherTime(openMeteoWeather.updatedAt)}</span></div><div className="flex items-center gap-2 p-2 bg-emerald-50 rounded-lg"><span className="font-bold text-emerald-700">200</span><div className="flex-1"><b>Risiko Banjir (BNPB)</b><div className="text-slate-400">MapServer/export</div></div><span>✓</span></div><div className="flex items-center gap-2 p-2 bg-emerald-50 rounded-lg"><span className="font-bold text-emerald-700">200</span><div className="flex-1"><b>Bahaya Banjir (BNPB)</b><div className="text-slate-400">MapServer/export</div></div><span>✓</span></div></div></div>
            </section>
          </div>
        </main>
      </div>

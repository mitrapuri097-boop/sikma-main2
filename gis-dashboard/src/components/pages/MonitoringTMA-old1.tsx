...
const REFRESH_MS = Number(import.meta.env.VITE_KHDTK_REFRESH_MS || 60000);
...
function normalizeStation(row: RawStation, index: number): Station {
  ...
  return {
    ...
    potassium: numberValue(source.potassium, source.kalium, source.k),
...
export default function MonitoringKHDTK() {
  ...
  const attentionCount = stations.filter((station) =>
    ["Awas", "Siaga", "Waspada"].includes(station.status),
  ).length;
      ...
      <div className="khdtk-shell">
        ...
        {loading && !stations.length ? (
          ...
          <>
            ...
            <div className="khdtk-footer-status">
              <span>
                <span className="status-dot" />{" "}
                {attentionCount
                  ? `${attentionCount} lokasi perlu perhatian`
                  : "Semua lokasi normal"}
              </span>
              {selectedStation && (
                <span>
                  Terpilih: <strong>{selectedStation.name}</strong>
                </span>
              )}
            </div>
          </>

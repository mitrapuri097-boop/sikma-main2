...
function normalizeRows(payload: unknown): TmaRow[] {
  ...
  return Object.entries(data as Record<string, unknown>).map(([key, value]) => {
    if (!value || typeof value !== "object") return { id: key, value } as TmaRow;
...
function StatCard({
  ...
  return (
    <div className="group rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:p-4">
      ...
      <div className="mt-3 text-xl font-extrabold tracking-tight text-slate-950 sm:mt-4 sm:text-2xl">{value}</div>
...
function LayerRow({ name, count, active = true, note }: {
  name: string;
  count: number;
  active?: boolean;
  note?: string;
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-3">
      <div className="flex min-w-0 items-center gap-2.5">
        <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${active ? "bg-emerald-500" : "bg-slate-300"}`} />
...
function SensorChart({ rows, metric }: { rows: TmaRow[]; metric: string }) {
  ...
  return (
    <div className="relative h-[250px] w-full overflow-hidden rounded-xl border border-slate-200 bg-white p-3">
      ...
      <svg viewBox="0 0 120 92" className="h-[190px] w-full" preserveAspectRatio="none" role="img" aria-label={`Grafik ${labels[metric]}`}>
        {[20, 40, 60, 80].map((y) => (
          <line key={y} x1="8" y1={y} x2="112" y2={y} stroke="#e2e8f0" strokeWidth="0.35" />
        ))}
        <line x1="8" y1="86" x2="112" y2="86" stroke="#cbd5e1" strokeWidth="0.5" />
        {points && (
          <polyline
            points={points}
            fill="none"
    ...
    <div className="min-h-screen bg-slate-100 p-2 sm:p-3 md:p-5">
      <div className="mx-auto w-full max-w-[1800px] space-y-3 sm:space-y-4 lg:space-y-5">
        ...
        <div className="grid grid-cols-1 gap-2.5 min-[420px]:grid-cols-2 sm:gap-3 lg:grid-cols-4">
          <StatCard label="Total Sensor" value={rows.length} icon={<Waves className="h-5 w-5" />} />
          <StatCard label="Sensor Aman" value={statusSummary.aman} icon={<ShieldCheck className="h-5 w-5" />} tone="green" />
          <StatCard label="Waspada / Siaga" value={statusSummary.waspada + statusSummary.siaga} icon={<Gauge className="h-5 w-5" />} tone="orange" />
          <StatCard label="Bahaya / Awas" value={statusSummary.bahaya} icon={<Activity className="h-5 w-5" />} tone="red" />
        </div>
          ...
          <div className="grid gap-3 p-3 sm:gap-4 sm:p-4 xl:grid-cols-[minmax(0,1fr)_320px]">
            <div>
              ...
              <div className="mt-2 text-[10px] text-slate-400">
                Catatan: endpoint saat ini mengirim data sensor terkini. Grafik akan menampilkan titik data yang tersedia; histori waktu membutuhkan endpoint histori.
              </div>
            </div>
            ...
            <aside className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-xs font-bold text-slate-900">Layer & Analisis</div>
                  <div className="text-[10px] text-slate-400">Monitoring hidrologi</div>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-2">
                ...
                <LayerRow
                  ...
                  active={false}
                  note={invalidCoordinateCount ? "Belum tampil di peta" : "Semua sensor memiliki koordinat"}
                />
              </div>

              <div className="mt-5 text-[10px] font-bold uppercase tracking-wide text-slate-400">
                Status Sensor
              </div>
              <div className="mt-2 space-y-2">
                <StatusRow label="Aman" count={statusSummary.aman} color="bg-emerald-500" />
                <StatusRow label="Waspada" count={statusSummary.waspada} color="bg-amber-500" />
                <StatusRow label="Siaga" count={statusSummary.siaga} color="bg-orange-500" />
                <StatusRow label="Bahaya / Awas" count={statusSummary.bahaya} color="bg-red-500" />
              </div>

              <div className="mt-5 rounded-lg border border-slate-200 bg-white p-3">
                <div className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Last Update</div>
                <div className="mt-1 text-xs font-semibold text-slate-800">{lastUpdate}</div>
                <div className="mt-3 text-[10px] font-bold uppercase tracking-wide text-slate-400">Status API</div>
                <div className={`mt-1 text-xs font-bold ${error ? "text-red-600" : loading ? "text-amber-600" : "text-emerald-600"}`}>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
            <div>
              ...
              <div className="mt-0.5 text-[10px] text-slate-400">Lokasi stasiun sensor yang memiliki koordinat valid</div>
            </div>
          ...
          <div className="relative h-[360px] min-[480px]:h-[420px] sm:h-[500px] lg:h-[560px] xl:h-[620px]">
                    ...
                    <Popup>
                      <div className="w-[280px] sm:w-[300px]">
                        <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
                          ...
                          <div className="min-w-0">
                            <div className="truncate text-sm font-bold text-slate-900">{stationName}</div>
                        ...
                        <div className="mt-3 flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2.5">
                          <span className="text-xs font-medium text-slate-500">Status TMA</span>
                          <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ring-1 ${statusBadge(status)}`}>
                            {status}
                          </span>
                        </div>
                        ...
                        <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3">
                          ...
                          <div className="mt-1 flex items-end gap-1">
                            <span className="text-2xl font-bold tracking-tight text-slate-900">{tma.toFixed(1)}</span>
                            <span className="mb-1 text-xs font-medium text-slate-500">m</span>
                          </div>
                        </div>

                        <div className="mt-4 text-[10px] font-bold uppercase tracking-wide text-slate-400">Data Sensor</div>
                        <div className="mt-2 grid grid-cols-2 gap-2">
                          <InfoItem icon={<Thermometer className="h-3 w-3" />} label="Suhu" value={`${suhu.toFixed(1)} °C`} />
                          <InfoItem icon={<Droplets className="h-3 w-3" />} label="Kelembaban" value={`${kelembaban.toFixed(1)} %`} />
                          <InfoItem label="EC" value={`${ec}`} />
                          <InfoItem label="pH" value={`${ph.toFixed(1)}`} />
                          <InfoItem icon={<CloudRain className="h-3 w-3" />} label="Curah Hujan" value={`${rain}`} />
                          <InfoItem icon={<Mountain className="h-3 w-3" />} label="Elevasi" value={`${elevation}`} />
                        </div>

                        <div className="mt-4 text-[10px] font-bold uppercase tracking-wide text-slate-400">Power & Telemetri</div>
                        <div className="mt-2 grid grid-cols-2 gap-2">
                          <InfoItem label="Tegangan" value={`${tegangan.toFixed(2)} V`} />
                          <InfoItem label="Arus" value={`${arus.toFixed(1)}`} />
                          <InfoItem label="Power" value={`${power.toFixed(2)} W`} />
                          <InfoItem icon={<BatteryCharging className="h-3 w-3" />} label="Baterai" value={`${persen}%`} />
                        </div>

                        {(station.RainDate || station.RainTime) && (
                          <div className="mt-3 rounded-lg bg-slate-50 px-3 py-2">
                            <div className="text-[10px] text-slate-400">Update Curah Hujan</div>
                            <div className="mt-0.5 text-xs font-semibold text-slate-700">
                              {String(station.RainDate || "-")} {String(station.RainTime || "")}
                            </div>
                          </div>
                        )}
                        ...
                        <div className="mt-3 border-t border-slate-200 pt-3 text-[10px] text-slate-400">
                          Update data:{" "}
                          <span className="font-medium text-slate-600">{String(station.tanggal || "-")}</span>{" "}
                          <span className="font-medium text-slate-600">{String(station.jam || "-")}</span>
                        </div>
                      </div>
                    </Popup>
                  </Marker>
            ...
            <div className="pointer-events-none absolute left-3 top-3 z-[1000] sm:left-4 sm:top-4">
              <div className="rounded-xl border border-white/80 bg-white/95 px-3 py-2 shadow-lg backdrop-blur">
                <div className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Monitoring TMA</div>
                <div className="mt-0.5 flex items-center gap-2">
                  <span className="text-sm font-bold text-slate-800">{validRows.length}</span>
                  <span className="text-xs text-slate-500">sensor terpetakan</span>
                </div>
              </div>
            </div>
          </div>

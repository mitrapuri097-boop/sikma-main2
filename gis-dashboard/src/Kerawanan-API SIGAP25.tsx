...
const Kerawanan = () => {
  ...
  // ======================================================
  // ENTERPRISE USER LAYER AUTHORIZATION
  // can_view menjadi sumber kebenaran untuk layer lokal.
  // null = belum ada token/authorization belum diterapkan.
  // Set kosong = user terautentikasi tetapi tidak punya akses layer.
  // ======================================================
  const [authorizedLayerNames, setAuthorizedLayerNames] = useState<Set<string> | null>(null);
  ...
  const normalizeAuthorizationLayerName = (name: string) => {
  ...
  const isLayerAuthorized = (name: string) => {
    ...
    return authorizedLayerNames.has(normalized);
  };

  const filterAuthorizedLayerList = (items: any[] = []) =>
    items.filter((item) => isLayerAuthorized(item?.name ?? item?.id));

  const filterAuthorizedLayerData = (data: any) => {
    if (!data || authorizedLayerNames === null) return data;
    return {
      ...data,
      kerawanan: filterAuthorizedLayerList(data.kerawanan || []),
      mitigasiAdaptasi: filterAuthorizedLayerList(data.mitigasiAdaptasi || []),
      lainnya: filterAuthorizedLayerList(data.lainnya || []),
      // Auto kejadian tetap tersedia; layer manual tetap melewati authorization.
      kejadian: (data.kejadian || []).filter((item: any) =>
        item?.isAutoGenerated || isLayerAuthorized(item?.name ?? item?.id),
      ),
    };
  };

  const loadMyLayerAuthorizations = async () => {
    const token = getAuthToken();

    if (!token) {
      console.warn("🔐 Tidak ada token SIMITI; authorization layer tidak dapat diterapkan.");
      setAuthorizedLayerNames(null);
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/my-layer-authorizations`, {
        headers: {
      ...
      const names = new Set<string>(
        rows
          .filter((item: any) => item?.can_view === true)
          .map((item: any) => normalizeAuthorizationLayerName(item.table_name)),
      );

      setAuthorizedLayerNames(names);
      setAuthorizationLoaded(true);

      console.log("🔐 SIMITI authorized layers:", Array.from(names));
    } catch (error: any) {
      console.error("❌ Gagal mengambil authorization layer:", error);
      // Fail closed: token ada tetapi authorization gagal -> jangan tampilkan layer lokal.
      setAuthorizedLayerNames(new Set());
      setAuthorizationError(error?.message || "Authorization layer gagal dimuat.");
      setAuthorizationLoaded(true);
    }
  };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          throw new Error(
            errorBody?.message || `Akses View layer "${originalLayerName}" ditolak oleh server.`,
          );
        }
  ...
  const handleLayerToggle = async (
    ...
    // ENTERPRISE VIEW GUARD untuk layer lokal.
    if (isChecked && !bnpbKey && !isLayerAuthorized(tableName)) {
      console.warn("🚫 Layer ditolak authorization can_view:", tableName);
      setLayerError(
        authorizationLoaded
          ? `Anda tidak memiliki izin View untuk layer "${formatTableName(tableName)}".`
          : "Authorization layer belum selesai dimuat. Silakan coba lagi.",
      );
      setTimeout(() => setLayerError(""), 5000);
      return;
    }
  ...
  useEffect(() => {
    const bootstrapLayerAccess = async () => {
      // Authorization wajib selesai sebelum katalog layer dimuat.
      await loadMyLayerAuthorizations();
      await fetchLayers();

      // Defense-in-depth: jika ada layer aktif dari state/cache lama,
      // hanya layer yang can_view=true yang boleh tetap hidup.
      setActiveLayers((prev) => {
        const next = new Set<string>();
        for (const name of prev) {
          if (String(name).startsWith("kejadian_") || isLayerAuthorized(name)) {
            next.add(name);
          } else {
            const apiName = normalizeLayerApiName(name);
            const existing = layerGroupsRef.current?.[apiName] ?? layerGroupsRef.current?.[name];
            if (existing && mapInstanceRef.current) {
              safeRemoveMapLayer(existing);
              delete layerGroupsRef.current[apiName];
  ...
  const fetchLayers = async () => {
    try {
      ...
      const authorizedData = filterAuthorizedLayerData(data);
      console.log("Fetched layers data (authorized):", authorizedData);

      setLayerData(authorizedData);

      // Jika ada bounds yang dipilih, filter layers
      if (currentBounds) {
        void checkLayerAvailability(currentBounds);
      } else {
        // Tidak ada bounds: tetap gunakan catalog yang sudah difilter authorization.
        setAvailableLayers(authorizedData);
      }
    } catch (error) {
  ...
  // agar filter lama tidak ikut memengaruhi query layer.
  const handleMainTabChange = (tab: "administrasi" | "das") => {
    setActiveTab(tab);
  ...
  const buildEnterpriseSearchResults = async (query: string) => {
    ...
    try {
      ...
      const allLayers = [
        ...(layerData?.kerawanan || []).filter((x: any) => isLayerAuthorized(x?.name ?? x?.id)).map((x: any) => ({ ...x, group: "Kerawanan" })),
                ...
                <div className="w-full h-full bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                  ...
                  <div className="px-3 pt-3">
                    <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-1.5 shadow-sm">
                      ...
                      <div className="grid grid-cols-2 gap-1">
                        <label className={`flex items-center justify-center gap-2 min-h-9 rounded-lg px-2.5 cursor-pointer transition-all ${activeTab === "administrasi" ? "bg-white border border-slate-200 shadow-sm text-slate-800" : "text-slate-500 hover:bg-white/70"}`}>
                          <input
                            type="radio"
                            name="mainTab"
                            checked={activeTab === "administrasi"}
                            onChange={() => handleMainTabChange("administrasi")}
                            className="w-3.5 h-3.5 accent-emerald-600"
                          />
                          <span className="text-[10px] font-bold">Administrasi</span>
                        </label>
                        <label className={`flex items-center justify-center gap-2 min-h-9 rounded-lg px-2.5 cursor-pointer transition-all ${activeTab === "das" ? "bg-white border border-slate-200 shadow-sm text-slate-800" : "text-slate-500 hover:bg-white/70"}`}>
                          <input
                            type="radio"
                            name="mainTab"
                            checked={activeTab === "das"}
                            onChange={() => handleMainTabChange("das")}
                            className="w-3.5 h-3.5 accent-emerald-600"
                          />
                          <span className="text-[10px] font-bold">DAS</span>
                        </label>
                      </div>
                    </div>
                  </div>

                  {/* Enterprise geographic search — directly below mode selector */}
                  <div className="px-3 pt-3">
                    <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-visible">
                      <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between">
                        <div>
                          <div className="text-[8px] font-extrabold uppercase tracking-[0.12em] text-slate-400">Wilayah Analisis</div>
                          <div className="text-[9px] font-semibold text-slate-700 mt-0.5">{activeTab === "administrasi" ? "Administrasi" : "Daerah Aliran Sungai"}</div>
                        </div>
                        <span className={`text-[7px] font-bold px-2 py-1 rounded-full ${activeTab === "administrasi" ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-700"}`}>
                          {activeTab === "administrasi" ? "ADMIN" : "DAS"}
                        </span>
                      </div>
                      <div className="p-2.5">
                        {activeTab === "administrasi" ? (
                          <div className="grid grid-cols-[105px_1fr] gap-2">
                            <select value={adminLevel} onChange={(e) => { setAdminLevel(e.target.value as typeof adminLevel); setAreaSearchQuery(""); setAreaSearchResults([]); setShowAreaSearchDropdown(false); }} className="h-8 rounded-lg border border-slate-200 bg-slate-50 px-2 text-[9px] outline-none focus:border-emerald-400" aria-label="Tingkat administrasi">
                              <option value="provinsi">Provinsi</option><option value="kabupaten">Kabupaten/Kota</option><option value="kecamatan">Kecamatan</option><option value="kelurahan">Kelurahan/Desa</option>
                            </select>
                            <div className="relative">
                              <input value={areaSearchQuery} onChange={(e) => handleAreaSearchChange(e.target.value)} onFocus={() => { if (areaSearchResults.length > 0) setShowAreaSearchDropdown(true); }} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); void searchAreas(areaSearchQuery); } }} className="w-full h-8 rounded-lg border border-slate-200 bg-slate-50 px-2.5 text-[9px] outline-none focus:border-emerald-400" placeholder={`Cari ${adminLevel}...`} aria-label="Cari wilayah administrasi" />
                              {showAreaSearchDropdown && areaSearchResults.length > 0 && <div className="absolute left-0 right-0 top-9 z-[2400] max-h-52 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-xl">{areaSearchResults.map((area: any, index: number) => <button key={`${area.id ?? area.kode ?? area.label ?? "area"}-${index}`} type="button" onClick={() => handleAreaSelect(area)} className="w-full px-2.5 py-2 text-left text-[9px] hover:bg-emerald-50 border-b border-slate-100 last:border-0"><span className="block font-semibold text-slate-700 truncate">{area.label || area.nama || area.name || "Wilayah"}</span><span className="block text-[7px] text-slate-400 mt-0.5">{area.level || adminLevel}</span></button>)}</div>}
                            </div>
                          </div>
                        ) : (
                          <div className="relative">
                            <div className="relative">
                              ...
                              <input value={dasSearchQuery} onChange={(e) => handleDasSearchChange(e.target.value)} onFocus={() => { if (dasSearchResults.length > 0) setShowDasSearchDropdown(true); }} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); void searchDas(dasSearchQuery); } }} className="w-full h-9 rounded-lg border border-slate-200 bg-slate-50 pl-7 pr-2.5 text-[9px] outline-none focus:border-blue-400 focus:bg-white" placeholder="Cari nama DAS..." aria-label="Cari DAS" />
                            </div>
                            {showDasSearchDropdown && dasSearchResults.length > 0 && <div className="absolute left-0 right-0 top-10 z-[2400] max-h-52 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-xl">{dasSearchResults.map((das: any, index: number) => <button key={`${das.id ?? das.kode ?? das.nama_das ?? "das"}-${index}`} type="button" onClick={() => handleDasSelect(das)} className="w-full px-2.5 py-2 text-left text-[9px] hover:bg-blue-50 border-b border-slate-100 last:border-0"><span className="block font-semibold text-slate-700 truncate">{das.nama_das || das.label || das.nama || das.name || "DAS"}</span><span className="block text-[7px] text-slate-400 mt-0.5">Daerah Aliran Sungai</span></button>)}</div>}
                          </div>
                        )}
                        {(activeTab === "administrasi" ? selectedAreas.length > 0 : selectedDas.length > 0) && <div className="mt-2 flex flex-wrap gap-1">{activeTab === "administrasi" ? selectedAreas.map((area: any, index: number) => <button key={`${area.label}-${index}`} type="button" onClick={() => void handleRemoveArea(index)} className="max-w-full rounded-full bg-emerald-50 border border-emerald-100 px-2 py-1 text-[7px] font-semibold text-emerald-700 truncate" title="Hapus wilayah">{area.label || area.nama || area.name} ×</button>) : selectedDas.map((das: any, index: number) => <button key={`${das.nama_das}-${index}`} type="button" onClick={() => void handleRemoveDas(index)} className="max-w-full rounded-full bg-blue-50 border border-blue-100 px-2 py-1 text-[7px] font-semibold text-blue-700 truncate" title="Hapus DAS">{das.nama_das || das.label || das.nama || das.name} ×</button>)}</div>}
                      </div>
                    </div>
                  </div>
                  ...
                  {authorizationError && (
                    <div className="mx-3 mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[9px] text-amber-800">
                      ...
                      <div className="mt-0.5">{authorizationError}</div>
                    </div>
                  )}

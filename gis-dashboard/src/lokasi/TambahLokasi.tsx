...
const API_ENDPOINTS = {
  ...
  spatialDetect: `${API_URL}/api/spatial/detect-location`,
};
...
const ACTIVITY_GROUPS = ["Mitigasi", "Adaptasi"];
const getActivityGroup = (activity: string) =>
  activity.toLowerCase().startsWith("adaptasi") ? "Adaptasi" : "Mitigasi";
...
const STEPS: Array<{
  ...
  },
  {
    id: "spatial",
    number: "02",
    title: "Lokasi, Administrasi & DAS",
    description: "Koordinat, wilayah administrasi dan DAS otomatis",
    icon: MapPinned,
  },
  {
    id: "documentation",
...
export default function TambahLokasi() {
  ...
  const [activityGroupFilter, setActivityGroupFilter] = useState("Semua");
  ...
  const detectLocationFromCoordinate = async (
    ...
    setLocationDetectionMessage("Mendeteksi administrasi dan DAS dari peta...");

    try {
      const response = await fetch(API_ENDPOINTS.spatialDetect, {
        method: "POST",
      ...
      setForm((prev) => ({
        ...
        kabupaten_kota: String(data.kabupaten_kota ?? data.kab_kota ?? ""),
        kecamatan: String(data.kecamatan ?? ""),
        desa_kelurahan: String(data.desa_kelurahan ?? data.kel_desa ?? ""),
        das: String(data.nama_das ?? data.das ?? ""),
      }));

      setSelectedDasCode(String(data.kode_das ?? ""));
      setDasSearch(String(data.nama_das ?? data.das ?? ""));
      setCoordinateValidated(true);
      setLocationDetectionMessage(
        data.provinsi || data.nama_das || data.das
          ? "Lokasi administrasi dan DAS berhasil dideteksi dari peta."
          : "Koordinat valid, tetapi titik belum beririsan dengan layer administrasi/DAS.",
      );
    } catch (error) {
      console.error("Deteksi lokasi gagal:", error);
      setCoordinateValidated(false);
      setLocationDetectionMessage(
  ...
  const buildPayload = () => ({
    ...
    // Kompatibilitas endpoint lama: tahun tetap diturunkan dari tanggal.
    tahun_pelaksanaan: form.tanggal_pelaksanaan
      ? Number(form.tanggal_pelaksanaan.slice(0, 4))
      : null,
        ...
        <div className="grid gap-5 xl:grid-cols-[250px_minmax(0,1fr)]">
          ...
          <div className="min-w-0">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <Field label="Tanggal Pelaksanaan" required>
                    <Input
                      ...
                      onChange={(v) => update("tanggal_pelaksanaan", v)}
                      type="date"
                    />
                  </Field>
            ...
            {activeStep === "spatial" && (
              <WorkspaceSection
                number="02"
                title="Lokasi Spasial, Administrasi & DAS"
                description="Pilih titik pada peta atau masukkan koordinat. Sistem akan mendeteksi wilayah administrasi dan DAS secara otomatis."
                icon={MapPinned}
              >
                ...
                <SectionInfo
                  ...
                  text="Setiap perubahan koordinat akan memanggil deteksi point-in-polygon pada backend untuk mengisi Provinsi, Kabupaten/Kota, Kecamatan, Desa/Kelurahan, dan DAS dari layer peta."
                />
              </WorkspaceSection>
            )}

            {/* STEP 02B - ADMINISTRASI & DAS, DIGABUNG DALAM STEP LOKASI */}
            {activeStep === "spatial" && (
              <WorkspaceSection
                number="02B"
                title="Hasil Deteksi Administrasi & DAS"
                description="Wilayah administrasi dan DAS diisi otomatis dari titik koordinat. Pilihan manual tetap tersedia sebagai koreksi."
                icon={Layers3}
              >
                <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-[9px] text-blue-700">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <Field
                    ...
                    required
                    description="Diisi otomatis dari layer DAS pada peta; pencarian manual tersedia untuk koreksi."
                  >
                    <div className="relative">
                      <input
                        value={dasSearch}
              ...
              >
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  >
                    ...
                    <ReviewRow
                      label="Tanggal"
                      value={form.tanggal_pelaksanaan || "Belum dipilih"}
                    />

...
const Kerawanan = () => {
    ...
    try {
      ...
      const normalized = legend
        .map((item: any) => {
          ...
          return {
            label: String(
              item?.label ?? item?.values?.[0] ?? item?.value ?? "Tidak berlabel",
            ).trim(),
  ...
  const refreshActiveSigapLayers = async () => {
    ...
    if (activeSigapLayer) {
      await loadSigapKawasanHutan(undefined, activeSigapLayer);
    }
  };
          ...
          if (isSigapActive) {
            return (
              <div className="p-3 h-full flex flex-col">
                <div className="mb-2 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-[9px] text-slate-400">
                      Kelas dan warna mengikuti renderer resmi SIGAP.
                    </div>
                  </div>
                ...
                <div
                  className="overflow-auto flex-1 rounded-lg border border-slate-200"
                  style={{ maxHeight: "calc(35vh - 125px)" }}
                >
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-slate-50 z-10">
                      <tr className="border-b border-slate-200">
                        <th
                          className="text-left py-2 px-2 font-semibold text-slate-600 bg-slate-50"
                          style={{ width: "50px" }}
                        >
                          No
                        </th>
                        ...
                        <th className="text-left py-2 px-2 font-semibold text-slate-600 bg-slate-50">
                    ...
                    <tbody>
                      {sigapKawasanHutanLegend.length > 0 ? (
                        sigapKawasanHutanLegend.map((item, idx) => (
                          <tr
                            key={`${item.label}-${idx}`}
                            ...
                            <td className="py-2 px-2">
                              {item.imageData ? (
                                <img
                                  ...
                                  alt=""
                                  className="w-10 h-5 rounded border border-slate-300 object-contain bg-white"
                                />
                              ) : (
                                <span
                                  className="block w-10 h-5 rounded border border-slate-300"
                                  style={{
                        ...
                        <tr>
                          <td
                            colSpan={3}
                            className="py-8 text-center text-[10px] text-slate-400"
                          >
                            {sigapKawasanHutanLegendLoading
                              ? "Memuat kelas resmi SIGAP..."

...
const Kerawanan = () => {
  ...
  const addSatelliteHotspotMarkers = (hotspots: FirmsHotspot[]) => {
    ...
    hotspots.forEach((hotspot) => {
      ...
      marker.on("mouseover", () => {
        marker.setStyle({ radius: 7, fillOpacity: 1 });
      });

      marker.on("mouseout", () => {
        marker.setStyle({ radius: 4, fillOpacity: 0.95 });
      });
    ...
    console.log("🔥 FIRMS clickable markers:", markerLayer.getLayers().length);
    satelliteMarkerLayerRef.current = markerLayer;

    // Pastikan marker berada di atas tile WMS.
  ...
  const addSatelliteGetFeatureInfoFallback = () => {
    ...
    const handler = async (event: any) => {
      if (!satelliteHotspots.enabled) return;
      ...
      try {
        ...
        // Browser tidak lagi memanggil NASA langsung.
        // Request GetFeatureInfo diproxy melalui backend SIMITI
        // untuk menghindari CORS.
        const params = new URLSearchParams({
          bbox: `${sw.x},${sw.y},${ne.x},${ne.y}`,
          width: String(Math.max(1, Math.round(size.x))),
          height: String(Math.max(1, Math.round(size.y))),
          x: String(Math.max(0, Math.round(point.x))),
          y: String(Math.max(0, Math.round(point.y))),
        });

        const response = await fetch(
          `${API_URL}/api/firms/noaa21/info?${params.toString()}`,
          {
            headers: {
        ...
        if (!response.ok || payload?.success !== true) {
          throw new Error(
            payload?.message || `FIRMS proxy HTTP ${response.status}`,
          );
        }
        ...
        if (features.length === 0) {
          console.log("ℹ️ Tidak ada detail FIRMS pada titik yang diklik.");
          return;
        }
        ...
        const hotspot: FirmsHotspot = {
          latitude: Number(coordinates[1] ?? event.latlng.lat),
          longitude: Number(coordinates[0] ?? event.latlng.lng),
          bright_ti4: props.bright_ti4 ?? props.brightness,
          bright_ti5: props.bright_ti5 ?? props.bright_t31,
          scan: props.scan,
          track: props.track,
        ...
        console.warn("NASA FIRMS GetFeatureInfo gagal:", error);
      }
    };

    map.on("click", handler);
    satelliteGetFeatureInfoHandlerRef.current = handler;
  };
  ...
  useEffect(() => {
    ...
    addSatelliteGetFeatureInfoFallback();

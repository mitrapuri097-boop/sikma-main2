...
const Kerawanan = () => {
  ...
  // ======================================================
  // BNPB Identify — Raster-safe V2 (raw response debug)
    ...
    try {
      ...
      for (const config of active) {
        try {
          ...
          // DEBUG V2: simpan response mentah agar kita tidak lagi menebak
          // struktur Identify BNPB. Prioritas nilai tetap `hit.value`.
          console.groupCollapsed(`🛰️ BNPB Identify: ${config.name}`);
          console.log("URL:", config.url);
          console.log("Point 4326:", { latitude, longitude });
          console.log("Point 3395:", point3395);
          ...
          if (value === null) {
            results.push({
              ...
              status: "nodata",
              message: hits.length
                ? "Identify berhasil, tetapi `hit.value` kosong/NoData. Lihat detail response BNPB di bawah."
                : "Identify berhasil tetapi `results` kosong. Lihat detail response BNPB di bawah.",
              debug: { hit: hits[0] ?? null, response: json },
            });
          } else {
            results.push({
              service: config.name,
              ...
              value,
              rawValue,
              debug: { hit: hitWithValue, response: json },
            });
          }
                ...
                <div className="rounded-xl bg-white border border-slate-200 shadow-xl overflow-hidden">
                  ...
                  <div className="p-4 text-[10px] max-h-[320px] overflow-y-auto">
                    ...
                    {bnpbIdentifyPopup.loading ? "Memeriksa data raster BNPB..." : bnpbIdentifyPopup.error ? bnpbIdentifyPopup.error : bnpbIdentifyPopup.results.map((r,i)=><div key={i} className="py-2.5 border-b last:border-0">
                      ...
                      {r.debug && <details className="mt-2 text-[8px]">
                        <summary className="cursor-pointer text-slate-400 font-semibold">Lihat response BNPB (debug)</summary>
                        <pre className="mt-1 max-h-32 overflow-auto rounded bg-slate-950 text-slate-200 p-2 whitespace-pre-wrap break-all">{JSON.stringify(r.debug.response ?? r.debug.hit, null, 2)}</pre>
                      </details>}
                    </div>)}
                  </div>
                </div>

        ...
        <main className="relative flex-1 min-h-0 p-2 md:p-3 overflow-y-auto xl:overflow-hidden">
          <div className="min-h-full xl:h-full grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_350px] grid-rows-[minmax(520px,1fr)_auto_auto] xl:grid-rows-[minmax(0,1fr)_184px] gap-2">
            ...
            <section className="relative min-h-[520px] xl:min-h-0 rounded-xl overflow-hidden bg-sky-100 border border-white shadow-sm">
              ...
              {/* =====================================================
                  FILTER MODE — ADMINISTRASI / DAS
                  Radio benar-benar menentukan mode filter lokasi.
                 ===================================================== */}
              <div className="absolute top-[105px] left-14 z-[1700] w-[min(390px,calc(100%-76px))]">
                <div className="rounded-xl bg-white/97 border border-slate-200 shadow-[0_8px_25px_rgba(15,23,42,.16)] px-3 py-2.5">
                  {activeTab === "administrasi" ? (
                    <div className="mt-2 grid grid-cols-[105px_1fr] gap-2">
                      <select
                        value={adminLevel}
                        onChange={(e) => {
                          setAdminLevel(e.target.value as typeof adminLevel);
                          setAreaSearchQuery("");
                          setAreaSearchResults([]);
                          setShowAreaSearchDropdown(false);
                        }}
                        className="h-8 rounded-lg border border-slate-200 bg-white px-2 text-[9px] outline-none focus:border-emerald-400"
                        aria-label="Tingkat administrasi"
                      >
                        <option value="provinsi">YYYProvinsi</option>
                      ...
                      <div className="relative">
                        <input
                          ...
                          onFocus={() => {
                            if (areaSearchResults.length > 0) setShowAreaSearchDropdown(true);
                          }}
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              e.preventDefault();
                              void searchAreas(areaSearchQuery);
                            }
                          }}
                          className="w-full h-8 rounded-lg border border-slate-200 bg-white px-2.5 text-[9px] outline-none focus:border-emerald-400"
                          placeholder={`Cari ${adminLevel}...`}
                          aria-label="Cari wilayah administrasi"
                        />
                        {showAreaSearchDropdown && areaSearchResults.length > 0 && (
                          <div className="absolute left-0 right-0 top-9 z-[2200] max-h-52 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-xl">
                            {areaSearchResults.map((area: any, index: number) => (
                              <button
                                key={`${area.id ?? area.kode ?? area.label ?? "area"}-${index}`}
                                type="button"
                                onClick={() => handleAreaSelect(area)}
                                className="w-full px-2.5 py-2 text-left text-[9px] hover:bg-emerald-50 border-b border-slate-100 last:border-0"
                              >
                                <span className="block font-semibold text-slate-700 truncate">
                                  {area.label || area.nama || area.name || "Wilayah"}
                                </span>
                                <span className="block text-[7px] text-slate-400 mt-0.5">
                                  {area.level || adminLevel}
                                </span>
                              </button>
                            ))}
                          </div>
                    ...
                    <div className="mt-2 relative">
                      <input
                        ...
                        onFocus={() => {
                          if (dasSearchResults.length > 0) setShowDasSearchDropdown(true);
                        }}
                        className="w-full h-8 rounded-lg border border-slate-200 bg-white px-2.5 text-[9px] outline-none focus:border-emerald-400"
                        placeholder="Cari DAS..."
                        aria-label="Cari DAS"
                      />
                      {showDasSearchDropdown && dasSearchResults.length > 0 && (
                        <div className="absolute left-0 right-0 top-9 z-[2200] max-h-52 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-xl">
                          {dasSearchResults.map((das: any, index: number) => (
                            <button
                              key={`${das.id ?? das.kode ?? das.nama_das ?? "das"}-${index}`}
                              type="button"
                              onClick={() => handleDasSelect(das)}
                              className="w-full px-2.5 py-2 text-left text-[9px] hover:bg-emerald-50 border-b border-slate-100 last:border-0"
                            >
                              <span className="block font-semibold text-slate-700 truncate">
                                {das.nama_das || das.label || das.nama || das.name || "DAS"}
                              </span>
                              <span className="block text-[7px] text-slate-400 mt-0.5">
                                DAS
                              </span>
                  ...
                  {(activeTab === "administrasi" ? selectedAreas.length > 0 : selectedDas.length > 0) && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {activeTab === "administrasi"
                        ? selectedAreas.map((area: any, index: number) => (
                            <button
                              key={`${area.label}-${index}`}
                              type="button"
                              onClick={() => void handleRemoveArea(index)}
                              className="max-w-full rounded-full bg-emerald-50 border border-emerald-100 px-2 py-1 text-[7px] font-semibold text-emerald-700 truncate"
                              title="Hapus wilayah"
                            >
                              {area.label || area.nama || area.name} ×
                            </button>
                          ))
                        : selectedDas.map((das: any, index: number) => (
                            <button
                              key={`${das.nama_das}-${index}`}
                              type="button"
                              onClick={() => void handleRemoveDas(index)}
                              className="max-w-full rounded-full bg-blue-50 border border-blue-100 px-2 py-1 text-[7px] font-semibold text-blue-700 truncate"
                              title="Hapus DAS"
                            >
                              {das.nama_das || das.label || das.nama || das.name} ×
                            </button>
                          ))}
                    </div>
                  )}

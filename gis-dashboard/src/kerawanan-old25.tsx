...
const Kerawanan = () => {
  ...
  const [bnpbIdentifyPopup, setBnpbIdentifyPopup] = useState<{
    ...
    results: Array<{ service: string; url: string; value?: string; attributes?: Record<string, any> }>;
  ...
  const loadBnpbLayer = async (key: string) => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        bbox: leafletBoundsToEpsg3395(bounds),
        bboxSR: "3395",
        imageSR: "4326",
      ...
      console.log(`✅ BNPB InaRISK aktif EPSG:3395: ${config.name}`);
    } catch (error: any) {
  ...
  const identifyBnpbAtPoint = async (latitude: number, longitude: number) => {
    ...
    try {
      const map = mapInstanceRef.current;
      const size = map.getSize();
      const point3395 = wgs84ToEpsg3395(latitude, longitude);
      const mapExtent = leafletBoundsToEpsg3395(map.getBounds());
      const results: Array<{ service: string; url: string; value?: string; attributes?: Record<string, any>; status?: string }> = [];

      for (const config of active) {
        try {
          // Service metadata dipakai hanya untuk membedakan "di luar coverage"
          // dari "NoData". Nilai raster tetap diambil dari hit.value.
          let outsideCoverage = false;
          try {
            const metaResponse = await fetch(`${config.url}?f=json`, { cache: "no-store" });
            if (metaResponse.ok) {
              const meta = await metaResponse.json();
              const extent = meta?.fullExtent || meta?.extent;
              const spatialRef = extent?.spatialReference?.wkid;
              if (extent && (!spatialRef || Number(spatialRef) === 3395)) {
                outsideCoverage =
                  point3395.x < Number(extent.xmin) ||
                  point3395.x > Number(extent.xmax) ||
                  point3395.y < Number(extent.ymin) ||
                  point3395.y > Number(extent.ymax);
              }
            }
          } catch (metaError) {
            console.warn(`⚠️ Metadata BNPB gagal ${config.name}:`, metaError);
          }

          const params = new URLSearchParams({
            f: "json",
            geometry: `${point3395.x},${point3395.y}`,
          ...
          const response = await fetch(`${config.url}/identify?${params.toString()}`, {
          ...
          if (!hit) {
            results.push({
              service: config.name,
              url: config.url,
              status: outsideCoverage ? "outside" : "nodata",
              value: outsideCoverage ? "Di luar coverage" : "NoData",
            });
            continue;
          }

          // PENTING: raster BNPB -> nilai utama ada di hit.value.
          const value = hit?.value;
          if (value === undefined || value === null || String(value).trim() === "" || String(value).toLowerCase() === "nodata") {
            results.push({
              service: config.name,
              url: config.url,
              status: outsideCoverage ? "outside" : "nodata",
          ...
          console.warn(`❌ Identify BNPB gagal untuk ${config.name}:`, serviceError);
          results.push({
            service: config.name,
            url: config.url,
            status: "error",
            value: serviceError?.message || "Request error",
          });
        }
      }
      ...
      setBnpbIdentifyPopup({
        open: true,
        loading: false,
        error: usable.length ? "" : "Tidak ada nilai raster BNPB pada titik ini.",
        latitude,
        longitude,
        results,
      });
      ...
      setBnpbIdentifyPopup((prev) => ({
        ...prev,
        loading: false,
        error: error?.message || "Gagal melakukan Identify BNPB.",
      }));
    }
  };
  ...
  // ============================================================
  // PENCARIAN LOKASI — Opsi A
  // Backend diprioritaskan (master_kabupaten/master_kecamatan/etc).
  // Jika backend belum memiliki geometry/hasil, fallback ke Nominatim.
  // ============================================================
  const searchLocationOnMap = async (query: string) => {
    ...
    try {
      const levels: Array<"provinsi" | "kabupaten" | "kecamatan" | "kelurahan"> = [
        adminLevel,
        ...(["kabupaten", "provinsi", "kecamatan", "kelurahan"] as const).filter((x) => x !== adminLevel),
      ];
      ...
      try {
        const responses = await Promise.all(
          levels.map(async (lv) => {
            try {
              const r = await fetch(`${API_URL}/api/areas/search?query=${encodeURIComponent(trimmed)}&level=${lv}`, {
      ...
      // Hilangkan duplikat dan normalisasi nama field Opsi A.
      const normalized = backendResults.map((r: any) => ({
        ...r,
        label: r.label || r.nama_kabupaten || r.nama_kecamatan || r.nama_kelurahan || r.kab_kota || r.kecamatan || r.kel_desa || r.provinsi,
        level: r.level || adminLevel,
        geom: r.geom || r.geometry || r.geojson || null,
      })).filter((r: any) => r.label);

      if (normalized.length > 0) {
        setAreaSearchResults(normalized.slice(0, 10));
        setShowAreaSearchDropdown(true);
        return;
      }

      // Fallback: geocoder publik supaya fitur "ketik daerah -> langsung menuju lokasi"
      // tetap bekerja walaupun tabel boundary lokal masih kosong.
      const geoResponse = await fetch(
        `https://nominatim.openstreetmap.org/search?format=jsonv2&limit=8&countrycodes=id&q=${encodeURIComponent(trimmed + " Indonesia")}`,
        { headers: { Accept: "application/json" }, cache: "no-store" },
      );
      if (!geoResponse.ok) throw new Error(`Geocoding HTTP ${geoResponse.status}`);
      const geoRows = await geoResponse.json();
      const fallback = (geoRows || []).map((r: any) => ({
        label: String(r.display_name || trimmed).split(",").slice(0, 4).join(","),
        level: adminLevel,
        center: { lat: Number(r.lat), lng: Number(r.lon) },
        geom: null,
      }));
      if (requestId !== locationSearchRequestRef.current) return;
      setAreaSearchResults(fallback);
      setShowAreaSearchDropdown(true);
    } catch (error) {
      ...
      if (requestId === locationSearchRequestRef.current) setLocationSearchLoading(false);
    }
  };
  ...
  const handleLocationSearchChange = (value: string) => {
    ...
    locationSearchTimerRef.current = setTimeout(() => void searchLocationOnMap(value), 450);
  };
  ...
  const handleLocationSearchSelect = async (area: any) => {
    const map = mapInstanceRef.current;
    if (!map || !window.L) return;

    setShowAreaSearchDropdown(false);
    setAreaSearchQuery(area.label || "");

    // Jangan panggil handleAreaSelect() di sini karena fungsi lama mengubah
    // currentBounds berdasarkan tabel GIS lama yang kosong. Peta cukup diarahkan
    // ke geometry hasil Opsi A.
    const geometry = area.geom || area.geometry || null;
    let bounds: any = null;
    let center: any = area.center || null;

    if (geometry) {
      try {
        const geoLayer = window.L.geoJSON({ type: "Feature", geometry }, {
          style: { color: "#087f61", weight: 3, fillOpacity: 0.08 },
        });
        bounds = geoLayer.getBounds();
        if (bounds.isValid()) center = bounds.getCenter();
        if (boundaryLayerRef.current) {
          try { map.removeLayer(boundaryLayerRef.current); } catch {}
        }
        boundaryLayerRef.current = geoLayer.addTo(map);
      } catch (e) {
        console.warn("Geometry lokasi tidak dapat dirender:", e);
      }
    }

    if (!center && bounds?.isValid()) center = bounds.getCenter();
    if (!center) {
      const query = String(area.label || areaSearchQuery);
      try {
        const r = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&limit=1&countrycodes=id&q=${encodeURIComponent(query)}`, { headers: { Accept: "application/json" } });
        const rows = await r.json();
        if (rows?.[0]) center = { lat: Number(rows[0].lat), lng: Number(rows[0].lon) };
      } catch {}
    }
    ...
    setSelectedAreas([{
      label: area.label,
      level: area.level || adminLevel,
      provinsi: area.provinsi,
      kab_kota: area.kab_kota || area.nama_kabupaten,
      kecamatan: area.kecamatan || area.nama_kecamatan,
      kel_desa: area.kel_desa || area.nama_kelurahan,
    }]);

    const targetZoom = area.level === "provinsi" ? 8 : area.level === "kabupaten" ? 10 : 12;
    if (bounds?.isValid()) map.flyToBounds(bounds, { padding: [50, 50], duration: 1.1, maxZoom: targetZoom });
    else map.flyTo([lat, lng], targetZoom, { duration: 1.1 });

    if (mapWeatherMarkerRef.current) {
      mapWeatherMarkerRef.current.setLatLng([lat, lng]);
    } else {
      mapWeatherMarkerRef.current = window.L.marker([lat, lng]).addTo(map);
    }
    mapWeatherMarkerRef.current.bindTooltip(String(area.label || "Lokasi"), { direction: "top", offset: [0, -8] }).openTooltip();

    fetchMapWeather(lat, lng);
    identifyBnpbAtPoint(lat, lng);
  };

  const handleAreaSearchChange = handleLocationSearchChange;
          ...
          <div className="h-full grid grid-cols-[minmax(0,1fr)_350px] grid-rows-[minmax(0,1fr)_184px] gap-2">
            ...
            <section className="relative min-h-0 rounded-xl overflow-hidden bg-sky-100 border border-white shadow-sm">
              ...
              <div className="absolute top-[58px] left-3 z-[1700] w-[285px] mockup-map-search">
                ...
                {showAreaSearchDropdown && areaSearchQuery.trim().length >= 2 && (
                  <div className="mt-1 bg-white rounded-lg border border-slate-200 shadow-xl overflow-hidden max-h-[250px] overflow-y-auto dashboard-scroll">
                    {areaSearchResults.length > 0 ? areaSearchResults.map((area:any, idx:number) => (
                      <button key={`${area.label}-${idx}`} onClick={() => handleLocationSearchSelect(area)} className="w-full text-left px-3 py-2.5 hover:bg-emerald-50 border-b last:border-b-0">
                        <div className="text-[10px] font-bold text-slate-700">{area.label}</div>
                        <div className="text-[8px] text-slate-400 mt-0.5 capitalize">{area.level || adminLevel}</div>
                      </button>
                    )) : !locationSearchLoading ? (
                      <div className="px-3 py-3 text-[9px] text-slate-500">Lokasi tidak ditemukan.</div>
                    ) : null}
                  </div>
                )}
              </div>
              ...
              {bnpbIdentifyPopup.open && <div className="absolute right-3 bottom-14 z-[1900] w-[340px] max-w-[calc(100%-24px)]"><div className="rounded-xl bg-white border border-slate-200 shadow-xl overflow-hidden"><div className="bg-[#0b4d3c] text-white px-4 py-3 flex justify-between"><div><div className="text-[8px] text-emerald-200 uppercase">BNPB InaRISK • IDENTIFY</div><div className="text-sm font-bold">Informasi Risiko / Bahaya</div></div><button onClick={()=>setBnpbIdentifyPopup(p=>({...p,open:false}))}>×</button></div><div className="p-4 text-[10px] max-h-[300px] overflow-y-auto">{bnpbIdentifyPopup.loading?"Memuat informasi...":bnpbIdentifyPopup.results.length===0?bnpbIdentifyPopup.error||"Belum ada hasil.":bnpbIdentifyPopup.results.map((r:any,i:number)=><div key={i} className="py-2.5 border-b last:border-0"><div className="flex items-center justify-between gap-2"><b>{r.service}</b><span className={`text-[8px] px-2 py-1 rounded-full ${r.status==="ok"?"bg-emerald-100 text-emerald-700":r.status==="nodata"?"bg-slate-100 text-slate-500":r.status==="outside"?"bg-amber-100 text-amber-700":"bg-red-100 text-red-700"}`}>{r.status==="ok"?"Ada nilai":r.status==="nodata"?"NoData":r.status==="outside"?"Di luar coverage":"Request error"}</span></div><div className="text-slate-700 font-bold mt-1">{r.value||"Tidak ada nilai"}</div></div>)}</div></div></div>}
            </section>

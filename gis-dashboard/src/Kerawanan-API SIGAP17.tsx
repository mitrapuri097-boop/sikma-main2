        ...
        <main className="relative flex-1 min-h-0 p-2 md:p-3 overflow-y-auto xl:overflow-hidden">
          <div className="min-h-full xl:h-full grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_350px] grid-rows-[minmax(520px,1fr)_auto_auto] xl:grid-rows-[minmax(0,1fr)_184px] gap-2">
            ...
            <section className="relative min-h-[520px] xl:min-h-0 rounded-xl overflow-hidden bg-sky-100 border border-white shadow-sm">
              ...
              {/* =========================================================
                  ENTERPRISE LAYER ACCESS — kanan
                  Menu "Semua Layer" dan chip kategori dihapus dari map.
                  Seluruh fungsi layer lama tetap dipakai melalui drawer ini.
                 ========================================================= */}
              <button
                type="button"
                onClick={() => setShowLayerPanel((v) => !v)}
                className="absolute right-3 top-3 z-[1850] h-10 px-3 rounded-xl bg-[#064238] text-white shadow-[0_8px_24px_rgba(6,66,56,.25)] border border-white/10 flex items-center gap-2 hover:bg-[#075344] transition"
                title="Buka pengelola layer SIMITIGASI"
                aria-label="Buka pengelola layer SIMITIGASI"
              >
                <span className="w-7 h-7 rounded-lg bg-white/10 flex items-center justify-center text-sm">▱</span>
                <span className="text-[9px] font-extrabold tracking-wide">SIMITIGASI</span>
                <span className="text-[8px] opacity-70">{activeLayerCount}</span>
              </button>

              {showLayerPanel && (
                <div className="absolute right-3 top-[58px] bottom-3 z-[1800] w-[calc(100%-24px)] max-w-[340px] sm:w-[300px] bg-white rounded-2xl border border-slate-200 shadow-[0_18px_55px_rgba(15,23,42,.24)] overflow-hidden flex flex-col">
              ...
              {/* Legend — fungsi tetap, diposisikan kiri agar tidak bertabrakan dengan SIMITIGASI */}
              <div className="absolute left-3 bottom-3 z-[1700] w-[160px] sm:w-[190px] max-h-[42vh] overflow-y-auto dashboard-scroll bg-white/95 rounded-xl border border-slate-200 shadow-lg">
                <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between">
                  <b className="text-[11px]">Legenda</b>
                  <span className="text-[8px] text-slate-400">
                    {activeLayerCount} aktif
                  </span>
                </div>
                <div className="p-3 space-y-1.5 text-[9px]">
                  {Array.from(activeLayers).some((layerName) =>
                    isSigapKawasanHutanLayer(layerName),
                  ) ? (
                    <>
                      <div className="pb-1 font-semibold text-slate-600">
                        Fungsi Kawasan Hutan
                      </div>
                      {sigapKawasanHutanLegend.length > 0 ? (
                        sigapKawasanHutanLegend.map((item, index) => (
                          <div
                          ...
                          >
                            {item.imageData ? (
                              ...
                              <span
                                className="w-3 h-3 shrink-0 rounded-sm border border-slate-300"
                                style={{ background: item.color || "#808080" }}
                              />
                            )}
                        ...
                        <div className="text-[8px] text-slate-400 leading-tight">
                          {sigapKawasanHutanLegendLoading
                            ? "Memuat legenda SIGAP..."
                            : "Legenda SIGAP belum tersedia."}
                        </div>
                      )}
                    </>
                  ) : (
                  ...
                  <div className="pt-2 mt-2 border-t border-slate-100 font-semibold text-slate-500">
                    Kejadian Bencana
                  </div>
                  {[
                    ["#3b82f6", "Banjir"],
              ...
              <div className="hidden sm:flex absolute left-[205px] right-[12px] bottom-3 z-[1600] flex items-center gap-2">
                <div className="flex-1 min-w-0 bg-white/95 rounded-lg border border-slate-200 shadow-md h-10 flex items-center px-3 gap-4 overflow-hidden">
                  ...
                  {[
                    ...
                    <button
                      key={m}
                      className={`text-[8px] font-semibold ${m === "Mei" ? "text-emerald-700" : "text-slate-400"}`}
                    >
                      {m}
                    </button>
                  ))}
                ...
                <button className="h-10 px-4 bg-white/95 rounded-lg border border-slate-200 shadow-md text-[9px] font-semibold text-slate-600">
                  ↻ Animasi
                </button>
                <button
                  onClick={() => refreshActiveBnpbLayers()}
                  className="h-10 px-4 bg-white/95 rounded-lg border border-slate-200 shadow-md text-[9px] font-semibold text-emerald-700"
                >
                  ⟳ Real-time
                </button>
              </div>

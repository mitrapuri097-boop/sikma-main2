...
const Kerawanan = () => {
  ...
  const removeSatelliteHotspotMarkers = () => {
  ...
  const parseFirmsCsv = (csv: string) => {
    const lines = csv
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean);
  ...
  const formatWibDateTime = (acqDate?: string, acqTime?: string) => {
    if (!acqDate || !acqTime) return "-";

    const compactTime = String(acqTime).padStart(4, "0");
    const hh = compactTime.slice(0, 2);
    const mm = compactTime.slice(2, 4);

    const utcDate = new Date(`${acqDate}T${hh}:${mm}:00Z`);
    if (Number.isNaN(utcDate.getTime())) return `${acqDate} ${hh}:${mm} UTC`;
  ...
  const formatHotspotNumber = (value?: string, digits = 2) => {
  ...
  const buildHotspotPopup = (hotspot: Record<string, string>) => {
    const satellite = hotspot.satellite || "NOAA-21";
    const instrument = hotspot.instrument || "VIIRS";
    const frp = hotspot.frp
      ? `${formatHotspotNumber(hotspot.frp, 2)} MW`
      : "-";
    const confidence = hotspot.confidence || "-";
    const acquisition = formatWibDateTime(
      hotspot.acq_date,
      hotspot.acq_time,
    );
    const latitude = hotspot.latitude || "-";
    const longitude = hotspot.longitude || "-";
    const dayNight = hotspot.daynight || "-";

    return `
      <div style="min-width:260px;max-width:300px;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px;">
          <div>
            <div style="font-size:13px;font-weight:800;color:#b45309;">🔥 HOTSPOT SATELIT</div>
            <div style="font-size:11px;color:#64748b;">NASA FIRMS • ${satellite}</div>
          </div>
          <div style="font-size:11px;font-weight:700;color:#fff;background:#f97316;padding:4px 7px;border-radius:999px;">VIIRS</div>
        </div>

        <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;font-size:11px;">
          <div style="background:#fff7ed;padding:7px;border-radius:8px;">
            <div style="color:#94a3b8;">Produk</div>
            <div style="font-weight:700;color:#334155;">${FIRMS_SOURCE}</div>
          </div>
          <div style="background:#fff7ed;padding:7px;border-radius:8px;">
            <div style="color:#94a3b8;">Instrumen</div>
            <div style="font-weight:700;color:#334155;">${instrument}</div>
          </div>
          <div style="background:#fff7ed;padding:7px;border-radius:8px;">
          ...
          <div style="background:#fff7ed;padding:7px;border-radius:8px;">
            ...
            <div style="font-weight:700;color:#334155;">${confidence}</div>
          </div>
          <div style="background:#fff7ed;padding:7px;border-radius:8px;grid-column:1 / -1;">
            <div style="color:#94a3b8;">Acquisition time (WIB)</div>
            <div style="font-weight:700;color:#334155;">${acquisition}</div>
          </div>
          <div style="background:#f8fafc;padding:7px;border-radius:8px;">
            <div style="color:#94a3b8;">Latitude</div>
            <div style="font-weight:700;color:#334155;">${formatHotspotNumber(latitude, 5)}°</div>
          </div>
          <div style="background:#f8fafc;padding:7px;border-radius:8px;">
            <div style="color:#94a3b8;">Longitude</div>
            <div style="font-weight:700;color:#334155;">${formatHotspotNumber(longitude, 5)}°</div>
          </div>
          <div style="background:#f8fafc;padding:7px;border-radius:8px;grid-column:1 / -1;">
            <div style="color:#94a3b8;">Day / Night</div>
            <div style="font-weight:700;color:#334155;">${dayNight}</div>
          </div>
        </div>
  ...
  const renderSatelliteHotspotMarkers = (
    ...
    satelliteMarkerLayerRef.current = layerGroup;
  };
  ...
  const fetchSatelliteHotspots = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`NASA FIRMS HTTP ${response.status}`);
      }

      const csv = await response.text();
      const hotspots = parseFirmsCsv(csv).filter(
        (row) => row.latitude && row.longitude,
      );

      if (satelliteHotspots.enabled) {
        renderSatelliteHotspotMarkers(hotspots);
      }
  ...
  useEffect(() => {
    ...
    // FIRMS diperbarui NASA secara berkala; refresh tiap 15 menit.

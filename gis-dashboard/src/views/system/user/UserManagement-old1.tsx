      ...
      setTotalPages(
        Number(
          pagination?.totalPages ??
            Math.max(
              1,
              Math.ceil(Number(pagination?.total ?? rawUsers.length) / limit),
            ),
        ),
      );
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="grid grid-cols-1 gap-4 p-6 sm:grid-cols-2 xl:grid-cols-5">
        <MetricCard
          ...
          label="Total User"
          value={totalUsers}
          helper="Total dari pagination server"
        />
      ...
      <td className="px-4 py-4 text-right align-top">
        <div className="flex justify-end gap-1">
          <button
            ...
            onClick={onView}
            title="Detail"
            className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-blue-700"
          >
          ...
          {user.status === "active" && (
            <button
              ...
              onClick={onSuspend}
              title="Suspend"
              className="flex h-9 w-9 items-center justify-center rounded-lg text-orange-600 hover:bg-orange-50"
            >
  ...
  return (
    <div className="space-y-5">
      <SectionCard
        ...
        description="Permission aktual harus berasal dari konfigurasi RBAC backend."
      >
        <NotConfigured
          title="Permission Matrix — Not Configured"
          description={
            roleInfo.level === null
              ? `Role "${user.role}" belum memiliki mapping RBAC Level 0–5.`
              : `Role "${user.role}" sudah memiliki referensi Level ${roleInfo.level}, tetapi endpoint permission matrix belum tersedia pada backend.`
          }
        />
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-xs font-bold text-slate-800">
                Role database tetap menjadi sumber kebenaran
              </div>
              <p className="mt-1 text-[11px] leading-5 text-slate-500">
                UI tidak mengarang Allowed/Restricted permission. Tidak ada
                permission yang dianggap aktif sampai endpoint atau tabel RBAC
                resmi tersedia.
              </p>
            </div>
          </div>
      ...
      <SectionCard
        ...
        description="Fitur administrasi permission menunggu konfigurasi backend."
      >
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {[
            ["View Permission", "Endpoint permission belum tersedia."],
            ["Assign Permission", "Endpoint assignment belum tersedia."],
            ["Revoke Permission", "Endpoint revocation belum tersedia."],
            ["Permission Audit", "Audit permission belum tersedia."],
      ...
      >
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <SecurityItem
            ...
            title="Password"
            value={
              user.raw.password_hash
                ? "Password hash tersedia"
                : "Not configured"
            }
            status={user.raw.password_hash ? "Configured" : "Not configured"}
            positive={Boolean(user.raw.password_hash)}
          />

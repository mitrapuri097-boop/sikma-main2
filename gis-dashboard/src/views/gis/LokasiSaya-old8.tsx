...
export default function LokasiSaya() {
  ...
  const analyzeLocation = useCallback(async (lat: number, lng: number) => {
    ...
    const bnpbUrl =
      ...
      `&longitude=${encodeURIComponent(lng)}&candidateLimit=12`;
  ...
  const bnpbThreats = useMemo(() => {
    return (bnpbLocation?.layers || [])
      .filter((layer) => layer.available && layer.inside)
      .filter((layer) => {
        const value = String(layer.class || layer.value || "").toLowerCase();
        return value !== "" && value !== "tidak terpetakan";
      });
  }, [bnpbLocation]);

  const bnpbRiskThreats = useMemo(() =>
    bnpbThreats.filter((layer) => layer.category === "Risiko"),
    [bnpbThreats],
  );

  const bnpbHazardThreats = useMemo(() =>
    bnpbThreats.filter((layer) => layer.category === "Bahaya"),
    [bnpbThreats],
  );
          ...
          <div className="rounded-2xl border border-slate-800 bg-[#08172a] p-4 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <div className="mt-1 text-[10px] text-slate-500">
                  Identifikasi langsung dari service BNPB InaRISK pada titik GPS terpilih.
                </div>
              </div>
              <span className="text-[10px] text-slate-500">
            ...
            {bnpbState === "ready" && (
              <div className="mb-3 rounded-xl border border-red-500/20 bg-red-500/5 p-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="text-[9px] font-bold uppercase tracking-[.16em] text-red-300">
                      Hasil deteksi BNPB
                    </div>
                    <div className="mt-1 text-sm font-black text-slate-100">
                      {bnpbThreats.length > 0
                        ? `${bnpbThreats.length} layer bencana terdeteksi pada lokasi`
                        : "Tidak ada layer bencana BNPB yang terdeteksi pada titik ini"}
                    </div>
                  </div>
                  <div className="flex gap-2 text-[9px] font-bold">
                    <span className="rounded-full border border-orange-500/20 bg-orange-500/10 px-2 py-1 text-orange-300">
                      Risiko {bnpbRiskThreats.length}
                    </span>
                    <span className="rounded-full border border-red-500/20 bg-red-500/10 px-2 py-1 text-red-300">
                      Bahaya {bnpbHazardThreats.length}
                    </span>
                  </div>
                </div>
                ...
                {bnpbThreats.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {bnpbThreats.map((layer) => (
                      <span
                        key={`threat-${layer.key}`}
                        className="rounded-lg border border-red-500/20 bg-slate-950/40 px-2.5 py-1.5 text-[10px] font-bold text-slate-200"
                      >
                        {layer.label}
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      >
                        {layer.inside ? layer.class || "TERPETAKAN" : "TIDAK TERDETEKSI"}
                      </span>
                    </div>
                    {layer.error && (
            ...
            {nearestSafeLocation ? (
              <div className="mt-4 rounded-xl border border-emerald-500/20 bg-slate-950/20 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-[10px] text-slate-400">
                      {nearestSafeLocation.jenis_kegiatan || "Lokasi kegiatan"}
                    </div>
                    <div className="mt-2 text-lg font-black text-slate-100">
                      {formatDistance(nearestSafeLocation.distanceMeters)}
                    </div>
                    <div className="mt-1 text-[10px] leading-4 text-slate-400">
                      {mitigationAddress(nearestSafeLocation) ||
                        "Alamat administratif belum tersedia"}
                    </div>
                  </div>
                </div>
                <div className="mt-3 rounded-lg border border-emerald-500/10 bg-emerald-500/5 px-3 py-2 text-[9px] leading-4 text-emerald-200">
                  Diverifikasi terhadap layer BNPB yang digunakan pada analisis GPS.
                </div>
              </div>
            ) : (

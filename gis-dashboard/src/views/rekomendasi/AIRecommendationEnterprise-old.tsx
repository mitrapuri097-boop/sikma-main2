...
const adminLabel = (level: AdminLevel, props: any) => {
  ...
  if (level === "kabupaten") return props?.kab_kota || props?.kabupaten || props?.name || "Kabupaten/Kota";
...
export default function AIRecommendationEnterprise() {
  ...
  const adminBoundaryCacheRef = useRef<Partial<Record<AdminLevel, AdminBoundary[]>>>({});
  const adminRequestRef = useRef<Partial<Record<AdminLevel, Promise<AdminBoundary[]>>>>({});
        ...
        const rows: AdminBoundary[] = features
          ...
          .map((feature: any, index: number) => {
            ...
            return {
              ...
              feature: { type: "Feature", geometry: feature.geometry, properties: props },
            };
          });
  ...
  const groupCandidatesByBoundary = useMemo(() => {
    ...
    filteredCandidates.forEach((candidate) => {
      ...
      const key = boundary?.id || `${adminLevel}:fallback:${fallbackLabel.toLowerCase()}`;
  ...
  }, [filteredCandidates, adminBoundaries, adminLevel, locateCandidateAdmin]);
  ...
  useEffect(() => {
    ...
    void fetchAdminBoundaries("provinsi").then((rows) => setAdminBoundaries(rows));
  ...
  useEffect(() => {
    ...
    groupCandidatesByBoundary.forEach((group) => {
      ...
      marker.bindTooltip(
        ...
        { direction: "top", offset: [0, -(size / 2)], className: "simiti-tooltip" },
      );

      marker.on("click", () => {
        const selectedCandidate =
          group.candidates.find((item) => String(item.id) === String(selectedId)) ||
          first;
        ...
        if (group.boundary?.feature) {
          try {
            ...
            if (bounds.isValid()) {
              map.fitBounds(bounds, { padding: [90, 90], maxZoom: Math.max(map.getZoom(), 11) });
              return;
            }
          } catch {
  ...
  }, [groupCandidatesByBoundary, selectedId]);
    ...
    <div className="simiti-map-page">
      ...
      <main className="simiti-map-shell">
        ...
        >
          ...
          <section className="simiti-analysis-section">
            <div className="simiti-analysis-section-title">
              ...
              <small>{groupCandidatesByBoundary.length} {adminLevel} · {filteredCandidates.length} kejadian</small>
            </div>
          ...
          <section className="simiti-analysis-section">
            ...
            <div className="simiti-layer-info">
              ...
              <span>{adminBoundaryLoading ? "Loading boundary..." : `Boundary ${adminLevel}`}</span>
            </div>
          </section>
        ...
        <div className="simiti-map-status">
          <span>
            <Activity size={13} />{" "}
            {loading ? "Memuat data..." : `${groupCandidatesByBoundary.length} ${adminLevel} · ${filteredCandidates.length} kejadian`}
          </span>
          <span>

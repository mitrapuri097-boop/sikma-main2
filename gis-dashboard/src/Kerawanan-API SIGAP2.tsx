...
const Kerawanan = () => {
  ...
  const [sigapKawasanHutanLegend, setSigapKawasanHutanLegend] = useState<
    Array<{ label: string; imageData?: string; contentType?: string; color?: string }>
  >([]);
  ...
  const loadSigapKawasanHutanLegend = async () => {
    try {
      ...
      const layer = Array.isArray(data?.layers)
        ? data.layers.find((item: any) => String(item.layerId ?? item.id) === "0") || data.layers[0]
        : null;
      const legend = Array.isArray(layer?.legend) ? layer.legend : [];

      const normalized = legend
        .map((item: any) => ({
          label: String(item?.label ?? item?.values?.[0] ?? "").trim(),
      ...
      console.log("SIGAP Kawasan Hutan legend loaded", { count: normalized.length });
    } catch (error) {
      console.warn("SIGAP legend tidak dapat dimuat; legend peta tetap aktif:", error);
  ...
  const renderBottomContent = () => {
    ...
    switch (activeBottomTab) {
      ...
      case "kawasan_hutan":
        if ((activeLayers.has("kawasan_hutan") || activeLayers.has("97")) && kawasanHutanData.length > 0) {
          return (
            <div className="p-3 h-full flex flex-col">
              <div
                className="overflow-auto flex-1"
        ...
        } else if (activeLayers.has("kawasan_hutan") || activeLayers.has("97")) {
          return (
            <div className="p-3 flex items-center justify-center h-full">
              <div className="text-center text-gray-500">
                <p className="text-sm">

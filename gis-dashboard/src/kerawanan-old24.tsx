...
const Kerawanan = () => {
  ...
  const identifyBnpbAtPoint = async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      const mapExtent3395 = leafletBoundsToEpsg3395(bounds);
      const results: BnpbIdentifyResult[] = [];

      for (const config of active) {
        try {
          const meta = await getBnpbServiceMeta(config.url!);
          const fullExtent = meta?.fullExtent;

          // Jangan menampilkan "NoData" kalau sebenarnya titik berada
          // di luar cakupan service regional BNPB.
          if (!pointInsideExtent3395(point3395.x, point3395.y, fullExtent)) {
            results.push({
              service: config.name,
              url: config.url!,
          ...
          if (!response.ok) {
            results.push({
              ...
              url: config.url!,
              status: "error",
              message: `BNPB Identify HTTP ${response.status}`,
            });
            continue;
          }
          ...
          if (json?.error) {
            results.push({
              ...
              url: config.url!,
              status: "error",
              message: json.error.message || "BNPB Identify mengembalikan error.",
            });
            continue;
          }

          const hits = Array.isArray(json?.results) ? json.results : [];
          // MapServer raster mengembalikan nilai piksel pada `value`.
          // Jangan mengambil nilai pertama dari `attributes` karena raster
          // BNPB tidak mempunyai field atribut seperti feature layer.
          const hitWithValue = hits.find((hit: any) => normalizeBnpbRasterValue(hit?.value) !== null);
          const rawValue = hitWithValue?.value;
          const value = normalizeBnpbRasterValue(rawValue);
          ...
          if (value === null) {
            results.push({
              ...
              url: config.url!,
              status: "nodata",
              message: "Titik berada dalam cakupan service, tetapi pixel raster BNPB bernilai NoData.",
            });
          } else {
            results.push({
              service: config.name,
          ...
          results.push({
            ...
            url: config.url!,
            status: "error",
            message: serviceError?.message || "Gagal menghubungi service BNPB.",
          });
        }
      }

      const hasValue = results.some((r) => r.status === "value");
  ...
  // ======================================================
  // SEARCH LOKASI
  // Prioritas: database SIMITIGASI (/api/areas/search),
  // fallback: Nominatim hanya jika database tidak menemukan hasil.
  // Hasil database langsung memilih area sehingga boundary + fitBounds
  // + filter administrasi ikut berjalan.
  // ======================================================
  const searchLocationOnMap = async (query: string) => {
    ...
    try {
      const levels = ["kabupaten", "provinsi", "kecamatan", "kelurahan"];
      const responses = await Promise.all(
        levels.map(async (level) => {
          const response = await fetch(
            `${API_URL}/api/areas/search?query=${encodeURIComponent(trimmed)}&level=${level}`,
            { headers: { Accept: "application/json" }, cache: "no-store" },
          );
          if (!response.ok) throw new Error(`Area search HTTP ${response.status}`);
          const data = await response.json();
      ...
      const unique = merged.filter((item, index, arr) => {
        const key = `${item.level}|${item.label}|${item.provinsi || ""}|${item.kab_kota || ""}|${item.kecamatan || ""}|${item.kel_desa || ""}`;
        return arr.findIndex((x) => `${x.level}|${x.label}|${x.provinsi || ""}|${x.kab_kota || ""}|${x.kecamatan || ""}|${x.kel_desa || ""}` === key) === index;
      });
      ...
      const ranked = unique.sort((a, b) => {
        ...
        const score = (x: string, item: any) =>
          x === q ? 0 :
          x.startsWith(q) ? 1 :
          String(item.level) === "kabupaten" ? 2 :
          String(item.level) === "provinsi" ? 3 :
          String(item.level) === "kecamatan" ? 4 : 5;
        return score(al, a) - score(bl, b);
      });

      setAreaSearchResults(ranked.slice(0, 12));

      // Exact match = langsung pilih lokasi, sesuai workflow mockup.
      const exact = ranked.find((item) => String(item.label || "").trim().toLowerCase() === q);
      const best = exact || (ranked.length === 1 ? ranked[0] : null);

      if (best) {
        handleAreaSelect(best);
        setShowAreaSearchDropdown(false);
        return;
      }

      setShowAreaSearchDropdown(ranked.length > 0);

      // Fallback hanya bila database SIMITIGASI tidak punya hasil.
      if (ranked.length === 0) {
        const response = await fetch(
        ...
        if (!result || !mapInstanceRef.current || !window.L) return;
        ...
        if (mapWeatherMarkerRef.current) {
          ...
          mapWeatherMarkerRef.current = window.L.marker([latitude, longitude]).addTo(mapInstanceRef.current);
        }
        mapWeatherMarkerRef.current.bindPopup(
      ...
      if (requestId === locationSearchRequestRef.current) {
        console.error("Gagal mencari area SIMITIGASI:", error);
        setAreaSearchResults([]);
        setShowAreaSearchDropdown(false);
      }
    } finally {
  ...
  const handleLocationSearchKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === "Escape") setShowAreaSearchDropdown(false);
  };
  ...
  // Search lama untuk panel/filter area tetap dipertahankan.
  const debouncedSearch = useRef<NodeJS.Timeout>();
  const handleAreaSearchChange = (value: string) => {
    setAreaSearchQuery(value);
    if (debouncedSearch.current) clearTimeout(debouncedSearch.current);
    debouncedSearch.current = setTimeout(() => searchAreas(value), 300);
  };
            ...
            <section className="relative min-h-0 rounded-xl overflow-hidden bg-sky-100 border border-white shadow-sm">
              ...
              <div className="absolute top-[58px] left-3 z-[1700] w-[285px] mockup-map-search">
                <div className="h-9 bg-white/95 rounded-lg shadow-md border border-slate-200 flex items-center px-3 gap-2">
                  ...
                  <input value={areaSearchQuery} onChange={(e)=>handleLocationSearchChange(e.target.value)} onKeyDown={handleLocationSearchKeyDown} onFocus={()=>areaSearchResults.length>0 && setShowAreaSearchDropdown(true)} className="flex-1 outline-none text-[10px] bg-transparent" placeholder="Cari lokasi, nama daerah, koordinat, atau DAS..." aria-label="Cari lokasi di peta" />
                  {locationSearchLoading && <span className="text-[8px] text-slate-400 animate-pulse">Mencari...</span>}
                </div>
                {showAreaSearchDropdown && areaSearchResults.length > 0 && (
                  <div className="mt-1 max-h-64 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-xl">
                    {areaSearchResults.map((area:any, idx:number)=>(
                      <button key={`${area.level}-${area.label}-${idx}`} onMouseDown={(e)=>e.preventDefault()} onClick={()=>handleAreaSelect(area)} className="w-full px-3 py-2 text-left hover:bg-emerald-50 border-b border-slate-100 last:border-b-0">
                        <div className="text-[10px] font-semibold text-slate-800 truncate">📍 {area.label}</div>
                        <div className="text-[8px] text-slate-400 mt-0.5 capitalize">{area.level}{area.provinsi ? ` • ${area.provinsi}` : ""}</div>
                      </button>
                    ))}
                  </div>
                )}

...
const Kerawanan = () => {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#eef3f0] text-slate-800">
      <style>{`        .custom-kejadian-marker { background: none; border: none; }        .marker-container:hover .marker-bg { fill: #ef4444 !important; }        .custom-kejadian-popup .leaflet-popup-content-wrapper { border-radius: 12px; padding: 0; box-shadow: 0 4px 20px rgba(0,0,0,0.15); }        .custom-kejadian-popup .leaflet-popup-content { margin: 0; width: 280px !important; }        .custom-kejadian-popup .leaflet-popup-tip { display: none; }        .dashboard-scroll::-webkit-scrollbar { width: 6px; height: 6px; }        .dashboard-scroll::-webkit-scrollbar-thumb { background: #b7c8c1; border-radius: 999px; }      `}</style>

      <aside className="w-[238px] shrink-0 bg-[#063d32] text-white flex flex-col shadow-xl z-[3000]">
        <div className="px-5 py-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center text-[#0b6b53] font-black">
              S
            </div>
            <div>
              <div className="text-lg font-extrabold tracking-wide">SIMITI</div>
        ...
        <nav className="flex-1 overflow-y-auto dashboard-scroll px-3 py-4 space-y-1">
          <div className="text-[9px] uppercase tracking-[0.18em] text-emerald-300 px-3 py-2">
            Menu Utama
          </div>
          {[
            ["Beranda", "⌂", "/"],
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs transition ${i === 1 ? "bg-[#0c7257] shadow-inner" : "text-emerald-50/80 hover:bg-white/10"}`}
            >
              <span className="w-5 text-center text-sm">{icon}</span>
              <span>{label}</span>
            </button>
          ))}
          <div className="text-[9px] uppercase tracking-[0.18em] text-emerald-300 px-3 pt-5 pb-2">
            Analisis
          </div>
          {["Dashboard", "Statistik", "Tren & Prediksi", "Laporan"].map(
            (label) => (
              <button
                key={label}
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs text-emerald-50/80 hover:bg-white/10 transition"
              >
        ...
        <div className="m-3 rounded-2xl border border-emerald-300/20 bg-emerald-900/40 p-3">
          ...
          <div className="text-[10px] text-emerald-100/70 mt-1">
            Aktifkan lokasi untuk melihat posisi di sekitar Anda
          </div>
          <button className="mt-2 w-full rounded-lg bg-[#15966e] hover:bg-[#1baa7e] py-2 text-[10px] font-bold">
            ⌖ Aktifkan
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        <div className="shrink-0 bg-white border-b border-slate-200">
          ...
          {isAuthenticated && (
            <div className="px-4 py-1.5 bg-emerald-50 border-t border-emerald-100 flex items-center justify-between">
        ...
        <main className="flex-1 min-h-0 p-2 md:p-3 overflow-hidden">
          <div className="h-full flex flex-col gap-2">
            <section className="relative flex-1 min-h-0 rounded-2xl overflow-hidden bg-slate-200 shadow-sm border border-white">
              <div ref={mapRef} className="absolute inset-0" />

              {mapWeatherPopup.open && (
                <div className="absolute right-5 top-5 z-[1900] w-[410px] max-w-[calc(100%-2rem)]">
                  <div className="relative overflow-hidden rounded-[18px] border border-slate-200 bg-white shadow-[0_20px_50px_rgba(15,23,42,0.22)]">
                    {/* Pointer visual agar terasa menempel ke titik pada peta */}
                    <div className="absolute -left-3 top-[108px] h-6 w-6 rotate-45 border-b border-l border-slate-200 bg-white" />

                    <div className="relative px-6 pt-5 pb-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          <div className="text-[13px] font-extrabold uppercase tracking-wide text-blue-600">
                            OPEN-METEO • CUACA WILAYAH
                          </div>
                          <div className="mt-2 text-[25px] leading-none font-black tracking-tight text-slate-900">
                            {mapWeatherPopup.locationName}
                          </div>
                          {(mapWeatherPopup.district || mapWeatherPopup.province) && (
                        ...
                        <button
                          type="button"
                          onClick={closeMapWeatherPopup}
                          className="shrink-0 w-8 h-8 rounded-full text-slate-500 hover:bg-slate-100 hover:text-slate-800 text-2xl leading-none transition"
                          aria-label="Tutup informasi cuaca"
                        >
                          ×
                        </button>
                      </div>
                      ...
                      {mapWeatherPopup.loading ? (
                        <div className="py-12 text-center text-sm text-slate-500">
                          <div className="mx-auto mb-3 h-9 w-9 animate-spin rounded-full border-4 border-blue-100 border-t-blue-600" />
                          Mengambil data curah hujan dan cuaca...
                        </div>
                      ) : mapWeatherPopup.error ? (
                        <div className="py-5">
                          <div className="rounded-xl border border-red-100 bg-red-50 p-3 text-xs text-red-700">
                            {mapWeatherPopup.error}
                          </div>
                          <div className="mt-2 text-[10px] text-slate-400">
                            Koordinat: {mapWeatherPopup.latitude?.toFixed(5)}, {mapWeatherPopup.longitude?.toFixed(5)}
                          </div>
                        </div>
                      ) : mapWeatherPopup.current ? (
                        <div className="pt-4">
                          <div className="flex items-center gap-4">
                            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-sky-50 text-4xl">
                              {weatherIcon(mapWeatherPopup.current.weather_code)}
                            </div>
                            <div>
                              <div className="text-[38px] leading-none font-black tracking-tight text-slate-900">
                                {mapWeatherPopup.current.temperature_2m ?? "—"}°C
                              </div>
                              <div className="mt-1 text-[17px] font-semibold text-blue-600">
                                {weatherDescription(mapWeatherPopup.current.weather_code)}
                              </div>
                            </div>
                          </div>

                          <div className="mt-5 rounded-xl border border-blue-100 bg-blue-50/90 p-4">
                            <div className="text-[13px] font-bold uppercase tracking-wide text-blue-700">
                              Curah Hujan Hari Ini
                            </div>
                            <div className="mt-1 flex items-end gap-1">
                              <span className="text-[32px] leading-none font-black text-blue-700">
                                {mapWeatherPopup.dailyRain ?? "—"}
                              </span>
                              <span className="mb-0.5 text-[14px] font-semibold text-blue-600">mm</span>
                            </div>
                            <div className="mt-2 text-[12px] text-slate-600">
                              Saat ini: {mapWeatherPopup.current.precipitation ?? "—"} mm
                            </div>
                          </div>

                          <div className="mt-4 divide-y divide-slate-100">
                            {[
                              ["💧", "Kelembapan", `${mapWeatherPopup.current.relative_humidity_2m ?? "—"} %`],
                              ["🌡", "Terasa seperti", `${mapWeatherPopup.current.apparent_temperature ?? "—"} °C`],
                              ["☁", "Tutupan awan", `${mapWeatherPopup.current.cloud_cover ?? "—"} %`],
                              ["≋", "Kecepatan angin", `${mapWeatherPopup.current.wind_speed_10m ?? "—"} km/j`],
                              ["➤", "Arah angin", `${mapWeatherPopup.current.wind_direction_10m ?? "—"}°`],
                              ["≋", "Hembusan angin", `${mapWeatherPopup.current.wind_gusts_10m ?? "—"} km/j`],
                              ["◉", "Tekanan udara", `${mapWeatherPopup.current.surface_pressure ?? "—"} hPa`],
                          ...
                          <div className="mt-3 border-t border-slate-200 pt-3">
                            <div className="flex items-center gap-2 text-[12px] text-slate-500">
                              <span className="text-blue-600">↻</span>
                              <span>Diperbarui: {formatWeatherTime(mapWeatherPopup.updatedAt)}</span>
                            </div>
                            <div className="mt-2 text-[12px] text-slate-600">
                              Sumber: <span className="font-semibold text-blue-600">Open-Meteo</span>
                            </div>
                          </div>
                        </div>
                      ) : null}
              ...
              {bnpbIdentifyPopup.open && (
                <div className="absolute right-5 top-5 z-[1950] mt-[520px] w-[410px] max-w-[calc(100%-2rem)]">
                  <div className="rounded-2xl border border-blue-100 bg-white shadow-[0_20px_50px_rgba(15,23,42,0.20)] overflow-hidden">
                    <div className="flex items-center justify-between bg-[#0b4d3c] px-4 py-3 text-white">
                      <div>
                        <div className="text-[9px] uppercase tracking-[0.16em] text-emerald-200">BNPB InaRISK • IDENTIFY</div>
                        <div className="text-sm font-extrabold">Informasi Risiko / Bahaya</div>
                      </div>
                      <button type="button" onClick={() => setBnpbIdentifyPopup((p) => ({ ...p, open: false }))} className="text-white/80 hover:text-white text-lg">×</button>
                    </div>
                    <div className="max-h-[260px] overflow-y-auto p-3">
                      <div className="mb-2 text-[9px] text-slate-400">Koordinat: {bnpbIdentifyPopup.latitude?.toFixed(5)}, {bnpbIdentifyPopup.longitude?.toFixed(5)}</div>
                      {bnpbIdentifyPopup.loading ? (
                        <div className="py-5 text-center text-xs text-slate-500">Mengambil nilai dari service BNPB...</div>
                      ) : bnpbIdentifyPopup.error ? (
                        <div className="rounded-xl border border-amber-100 bg-amber-50 p-3 text-xs text-amber-700">{bnpbIdentifyPopup.error}</div>
                      ) : (
                        <div className="space-y-2">
                          {bnpbIdentifyPopup.results.map((item, index) => (
                            <div key={`${item.service}-${index}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                              <div className="flex items-center justify-between gap-2">
                                <span className="text-[11px] font-extrabold text-slate-800">{item.service}</span>
                                <span className="rounded-full bg-blue-100 px-2 py-0.5 text-[9px] font-bold text-blue-700">BNPB</span>
                              </div>
                              <div className="mt-1 text-sm font-black text-blue-700">Nilai: {item.value ?? "—"}</div>
                              {item.attributes && Object.keys(item.attributes).length > 0 && (
                                <div className="mt-2 space-y-1">
                                  {Object.entries(item.attributes).slice(0, 6).map(([key, value]) => (
                                    <div key={key} className="flex justify-between gap-3 text-[9px] text-slate-500"><span>{key}</span><span className="font-semibold text-slate-700 text-right">{String(value)}</span></div>
                                  ))}
                                </div>
                              )}
                            </div>
              ...
              <div className="absolute top-3 left-3 right-3 z-[1200] flex items-center gap-2 pointer-events-none">
                <div className="ml-auto pointer-events-auto flex items-center gap-2">
                  <div className="bg-white/95 backdrop-blur rounded-xl shadow-lg px-3 py-2 text-[10px] text-slate-600 flex items-center gap-2">
                    <span className="font-semibold">Analisis Risiko</span>
              ...
              {!isLayerPanelOpen && (
                <button
                  ...
                  className="absolute left-3 bottom-3 z-[1700] rounded-xl bg-[#0b6b53] text-white shadow-xl px-3 py-2 text-[10px] font-bold"
                >
                  ☰ Layer Data
                </button>
              )}

              {/* Functional layer drawer */}
              {isLayerPanelOpen && (
                <div className="absolute left-3 top-14 bottom-3 w-[360px] max-w-[calc(100%-24px)] bg-white/95 backdrop-blur-md shadow-2xl rounded-2xl border border-white/70 flex flex-col z-[1800] overflow-hidden">
                  {/* Dashboard-style drawer header */}
                  <div className="bg-[#0b4d3c] text-white px-4 py-3 flex items-center justify-between flex-shrink-0">
                    <div>
                      <div className="text-[9px] uppercase tracking-[0.16em] text-emerald-200">
                        Data Layer
                      </div>
                      <h2 className="text-sm font-bold">
                        SIMITIGASI • Layer Services
                      </h2>
                    </div>
                    <button
                  ...
                  {isLoadingLayer && (
                    <div className="bg-blue-50 border-b-2 border-blue-500 p-2 flex items-center gap-2">
                      ...
                      <span className="text-xs text-blue-700 font-medium">
                        Memuat {loadingLayerNames.size} layer:{" "}
                        {Array.from(loadingLayerNames)
                          .map((name) => formatTableName(name))
                          .join(", ")}
                        ...
                      </span>
                    </div>
                  ...
                  {layerError && (
                    <div
                      className={`border-b-2 p-2 ${
                        layerError.includes("❌") || layerError.includes("🔌")
                          ? "bg-red-50 border-red-500"
                          : layerError.includes("⚠️")
                  ...
                  <div className="flex-1 overflow-y-auto pb-8">
                    <div className="p-3 pb-20">
                      ...
                      <div className="mb-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          (selectedAreas.length > 0 || selectedDas.length > 0)
                            ...
                            <div
                              key={layer.id}
                              className={`flex items-center gap-1.5 bg-gray-50 px-2 py-1 rounded border border-gray-200 ${loadingLayerNames.has(layer.name) ? "opacity-60 cursor-not-allowed" : ""}`}
                            >
                              <input
                                type="checkbox"
                      ...
                      {/* BNPB InaRISK Section */}
                      <div className="mb-4">
                        <div className="flex items-center justify-between mb-2 pb-1.5 border-b-2 border-blue-600">
                          <h3 className="text-sm font-semibold text-gray-800">BNPB InaRISK</h3>
                          <span className="text-[8px] font-bold uppercase tracking-wide text-slate-400">ArcGIS REST</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          {BNPB_INARISK_LAYERS.map((layer) => (
                            <label key={layer.key} className={`flex items-center gap-1.5 rounded border px-2 py-1.5 ${layer.url ? "border-slate-200 bg-slate-50 hover:bg-blue-50 cursor-pointer" : "border-slate-100 bg-slate-50/60 opacity-60 cursor-not-allowed"}`}>
                              <input
                                type="checkbox"
                                className="w-3 h-3 flex-shrink-0"
                                checked={activeLayers.has(layer.key)}
                                disabled={!layer.url || loadingLayerNames.has(layer.key)}
                                onChange={(e) => void handleBnpbToggle(layer.key, e.target.checked)}
                              />
                              <span className="text-[10px] leading-tight truncate" title={layer.url || "Endpoint BNPB belum tersedia"}>
                                {loadingLayerNames.has(layer.key) ? "⏳ " : ""}{layer.name}
                              </span>
                            </label>
                          ))}
                        </div>
                        <div className="mt-2 rounded-lg border border-blue-100 bg-blue-50/70 px-2.5 py-2 text-[9px] leading-relaxed text-slate-600">
                          Layer aktif diambil langsung dari <span className="font-bold text-blue-700">BNPB InaRISK</span>. Tidak disalin ke database SIMITI. Klik peta untuk melakukan Identify pada layer BNPB yang aktif.
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {(currentBounds &&
                          (selectedAreas.length > 0 || selectedDas.length > 0)
                            ? availableLayers.mitigasiAdaptasi
          ...
          <span className="text-[10px] text-gray-400 flex-shrink-0">
            ...
            <section className="h-[178px] shrink-0 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-2">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 overflow-hidden">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm font-extrabold">Data terkini</div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-4">
                  <div>
                    ...
                    <div className="text-[9px] text-slate-500">Kejadian</div>
                  </div>
                  <div>
                    <div className="text-xl font-black">{activeLayerCount}</div>
                    <div className="text-[9px] text-slate-500">Layer Aktif</div>
                  </div>
                  <div>
                    <div className="text-xl font-black">
                      {layerData.kerawanan.length}
                    </div>
                    <div className="text-[9px] text-slate-500">Kerawanan</div>
                  </div>
                  <div>
                    <div className="text-xl font-black">
                      {layerData.mitigasiAdaptasi.length}
                    </div>
                    <div className="text-[9px] text-slate-500">Mitigasi</div>
                  </div>
                </div>
              </div>
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 overflow-hidden">
                ...
                <div className="flex items-center gap-4 mt-3">
                  <div
                    ...
                    style={{
                      background:
                        "conic-gradient(#dc2626 0 18%, #f97316 18% 43%, #facc15 43% 68%, #22c55e 68% 88%, #16a34a 88% 100%)",
                    }}
                  >
                    <div className="w-12 h-12 bg-white rounded-full relative top-6 left-6 flex items-center justify-center text-[9px] font-bold">
                      Risiko
                    </div>
                  </div>
                  <div className="space-y-1 text-[9px]">
                    {[
                      ["#dc2626", "Sangat Tinggi"],
                      ["#f97316", "Tinggi"],
                      ["#facc15", "Sedang"],
                      ["#22c55e", "Rendah"],
                      ["#16a34a", "Sangat Rendah"],
                    ].map(([c, l]) => (
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 overflow-hidden">
                ...
                <svg viewBox="0 0 300 105" className="w-full h-[105px] mt-1">
                  <path
                    d="M8 78 C38 72, 54 86, 78 76 S112 68, 136 54 S170 28, 194 44 S226 18, 250 34 S278 16, 292 10"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="3"
                    className="text-orange-500"
                  />
                  <path d="M8 88 L292 88" stroke="#e2e8f0" strokeWidth="1" />
                  <g className="fill-slate-400 text-[8px]">
                    <text x="10" y="101">
                      2020
                    </text>
                    <text x="75" y="101">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 overflow-hidden">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm font-extrabold truncate max-w-[180px]">
                      {selectedRegionName || "Indonesia"}
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={fetchOpenMeteoWeather}
                    disabled={openMeteoWeather.loading}
                    className="text-[9px] text-emerald-700 font-bold hover:underline disabled:opacity-50"
                    title="Refresh data Open-Meteo"
                  >
                    {openMeteoWeather.loading ? "Memuat..." : "↻ Refresh"}
                  </button>
                </div>

                {openMeteoWeather.error ? (
                  <div className="mt-3 rounded-lg bg-red-50 border border-red-100 p-2 text-[9px] text-red-700">
                    {openMeteoWeather.error}
                  </div>
                ) : openMeteoWeather.current ? (
                  <>
                    <div className="mt-3 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-sky-50 flex items-center justify-center text-xl">
                        {Number(openMeteoWeather.current.weather_code) >= 51 ? "🌧️" : "☁️"}
                      </div>
                      <div>
                        <div className="text-xl font-black">
                        ...
                        <div className="text-[9px] text-slate-500">
                          {weatherDescription(openMeteoWeather.current.weather_code)}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3 text-[9px]">
                      <div>
                        ...
                        <b>{openMeteoWeather.current.relative_humidity_2m ?? "—"}%</b>
                      </div>
                      <div>
                        <div className="text-slate-400">Hujan hari ini</div>
                        <b>{openMeteoWeather.daily?.precipitation_sum?.[0] ?? "—"} mm</b>
                      </div>
                      <div>
                        <div className="text-slate-400">Angin</div>
                        <b>{openMeteoWeather.current.wind_speed_10m ?? "—"} km/j</b>
                      </div>
                    </div>

                    <div className="mt-2 grid grid-cols-2 gap-2 text-[8px] text-slate-400">
                      <span>Terasa {openMeteoWeather.current.apparent_temperature ?? "—"}°C</span>
                      <span className="text-right">Awan {openMeteoWeather.current.cloud_cover ?? "—"}%</span>
                    </div>
                  </>
                ) : (
                ...
                <div className="mt-2 flex items-center justify-between text-[8px] text-slate-400">
                  ...
                  <span>
                    {openMeteoWeather.updatedAt
                      ? openMeteoWeather.updatedAt.replace("T", " ")
                      : "—"}
                  </span>
                </div>
              </div>

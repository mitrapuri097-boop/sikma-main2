        ...
        <main className="flex-1 min-h-0 p-2 md:p-3 overflow-hidden">
          <div className="h-full flex flex-col gap-2">
            ...
            <section className="relative flex-1 min-h-0 rounded-2xl overflow-hidden bg-sky-100 shadow-sm border border-white">
              ...
              {bnpbIdentifyPopup.open && <div className="absolute right-[255px] bottom-3 z-[1900] w-[360px] max-w-[calc(100%-270px)]"><div className="rounded-2xl bg-white border border-slate-200 shadow-xl overflow-hidden"><div className="bg-[#0b4d3c] text-white px-4 py-3 flex justify-between"><div><div className="text-[9px] text-emerald-200 uppercase">BNPB InaRISK • IDENTIFY</div><div className="text-sm font-bold">Informasi Risiko / Bahaya</div></div><button onClick={()=>setBnpbIdentifyPopup(p=>({...p,open:false}))}>×</button></div><div className="p-4 text-[10px]">{bnpbIdentifyPopup.loading?"Memuat informasi...":bnpbIdentifyPopup.error?bnpbIdentifyPopup.error:bnpbIdentifyPopup.results.map((r,i)=><div key={i} className="py-2 border-b last:border-0"><b>{r.service}</b><div className="text-slate-500 mt-1">{r.value||"Tidak ada nilai"}</div></div>)}</div></div></div>}
            </section>

            {/* Bottom information strip */}
            <section className="h-[165px] shrink-0 grid grid-cols-1 xl:grid-cols-[1.45fr_1fr_1fr] gap-2">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 overflow-hidden"><div className="text-[10px] uppercase tracking-wide text-slate-400 font-bold">Informasi Lokasi Terpilih</div><div className="grid grid-cols-2 gap-6 mt-3"><div className="text-[10px] space-y-2"><div className="flex justify-between"><span className="text-slate-400">Provinsi</span><b>{selectedAreas[0]?.provinsi||"—"}</b></div><div className="flex justify-between"><span className="text-slate-400">Kabupaten/Kota</span><b>{selectedAreas[0]?.kab_kota||"—"}</b></div><div className="flex justify-between"><span className="text-slate-400">Kecamatan</span><b>{selectedAreas[0]?.kecamatan||"—"}</b></div><div className="flex justify-between"><span className="text-slate-400">Kelurahan</span><b>{selectedAreas[0]?.kel_desa||"—"}</b></div></div><div><div className="text-[10px] font-bold mb-2">Informasi Risiko (BNPB InaRISK)</div>{BNPB_INARISK_LAYERS.filter(l=>activeLayers.has(l.key)).slice(0,5).map(l=><div key={l.key} className="flex justify-between text-[10px] py-1"><span>{l.name}</span><span className="px-2 rounded-full bg-orange-50 text-orange-700">Aktif</span></div>)}{!BNPB_INARISK_LAYERS.some(l=>activeLayers.has(l.key))&&<div className="text-[10px] text-slate-400">Aktifkan layer BNPB untuk melihat status risiko.</div>}</div></div></div>

...
const Kerawanan = () => {
  ...
  type BmkgWeatherState = {
    ...
    current: BmkgWeatherItem | null;
    updatedAt: string | null;
  };

  const [bmkgWeather, setBmkgWeather] = useState<BmkgWeatherState>({
    loading: false,
    error: "",
    location: null,
    current: null,
  ...
  const [satelliteHotspots, setSatelliteHotspots] =
    useState<SatelliteHotspotState>({
      loading: false,
      error: "",
      count: 0,
      updatedAt: null,
  ...
  const satelliteMarkerLayerRef = useRef<any>(null);
  const satelliteRefreshTimerRef = useRef<number | null>(null);
  const satelliteGetFeatureInfoHandlerRef = useRef<((e: any) => void) | null>(
    null,
  );

  const FIRMS_MAP_KEY =
    (import.meta.env.VITE_FIRMS_MAP_KEY as string | undefined)?.trim() || "";

  const FIRMS_WMS_BASE =
    "https://firms.modaps.eosdis.nasa.gov/mapserver/wms/fires";

  // NASA FIRMS WFS menyediakan feature/atribut hotspot; WMS tetap dipakai
  // sebagai layer visual agar tampilan merah yang sudah ada tidak berubah.
  const FIRMS_WFS_REGION = "South_East_Asia";
  const FIRMS_WFS_LAYER = "ms:fires_noaa21_24hrs";

  const FIRMS_AREA_BBOX = "95,-11,141,6";
  ...
  const formatWibTime = (acqDate?: string, acqTime?: string | number) => {
    if (!acqDate) return "-";
    ...
    const utc = new Date(
      `${acqDate}T${String(hh).padStart(2, "0")}:${String(mm).padStart(2, "0")}:00Z`,
    );

    if (Number.isNaN(utc.getTime())) {
      return `${acqDate} ${raw} UTC`;
    }
  ...
  const buildHotspotPopup = (hotspot: FirmsHotspot) => {
    const confidence =
      hotspot.confidence === undefined || hotspot.confidence === ""
        ? "-"
        : hotspot.confidence;

    const frp =
      hotspot.frp === undefined || hotspot.frp === ""
        ? "-"
        : `${Number(hotspot.frp).toFixed(2)} MW`;

    return `
      <div style="min-width:250px;font-family:Inter,Arial,sans-serif;">
        <div style="font-weight:800;font-size:14px;color:#0f172a;margin-bottom:8px;">
          🔥 Hotspot VIIRS NOAA-21
        </div>
        <div style="font-size:11px;color:#64748b;margin-bottom:8px;">
          NASA FIRMS • ${escapeHtml(hotspot.instrument || "VIIRS")}
        </div>
        <div style="display:grid;grid-template-columns:110px 1fr;gap:5px 8px;font-size:11px;">
          <div style="color:#64748b;">Satelit</div>
          <div style="font-weight:700;">${escapeHtml(
            hotspot.satellite || "NOAA-21",
          )}</div>

          <div style="color:#64748b;">Produk</div>
          <div style="font-weight:700;">VIIRS_NOAA21_NRT</div>

          <div style="color:#64748b;">FRP</div>
          <div style="font-weight:700;color:#ea580c;">${frp}</div>

          <div style="color:#64748b;">Confidence</div>
          <div style="font-weight:700;">${escapeHtml(confidence)}</div>

          <div style="color:#64748b;">Acquisition</div>
          <div style="font-weight:700;">${escapeHtml(
            formatWibTime(hotspot.acq_date, hotspot.acq_time),
          )} WIB</div>

          <div style="color:#64748b;">Latitude</div>
          <div style="font-weight:700;">${Number(hotspot.latitude).toFixed(
            6,
          )}</div>

          <div style="color:#64748b;">Longitude</div>
          <div style="font-weight:700;">${Number(hotspot.longitude).toFixed(
            6,
          )}</div>

          <div style="color:#64748b;">Day/Night</div>
          <div style="font-weight:700;">${escapeHtml(
            hotspot.daynight || "-",
          )}</div>

          <div style="color:#64748b;">Versi</div>
          <div style="font-weight:700;">${escapeHtml(
            hotspot.version || "-",
          )}</div>
        </div>

        <div style="margin-top:9px;padding-top:7px;border-top:1px solid #e2e8f0;font-size:9px;color:#94a3b8;">
          Sumber: NASA FIRMS — Near Real-Time
        </div>
      </div>
    `;
  };

  const removeSatelliteHotspotLayer = () => {
    if (satelliteRefreshTimerRef.current !== null) {
      window.clearInterval(satelliteRefreshTimerRef.current);
      satelliteRefreshTimerRef.current = null;
    }

    if (mapInstanceRef.current && satelliteGetFeatureInfoHandlerRef.current) {
      mapInstanceRef.current.off(
        "click",
        satelliteGetFeatureInfoHandlerRef.current,
      );
      satelliteGetFeatureInfoHandlerRef.current = null;
    }

    if (satelliteMarkerLayerRef.current && mapInstanceRef.current) {
      try {
        mapInstanceRef.current.removeLayer(satelliteMarkerLayerRef.current);
      } catch {
        // Layer mungkin sudah terhapus.
      }
    }
    ...
    satelliteMarkerLayerRef.current = null;
    satelliteLayerRef.current = null;
  };

  const addSatelliteHotspotLayer = () => {
    if (!mapInstanceRef.current || !window.L || !FIRMS_MAP_KEY) return;

    if (satelliteLayerRef.current && mapInstanceRef.current) {
      mapInstanceRef.current.removeLayer(satelliteLayerRef.current);
    }

    const layerUrl = `${FIRMS_WMS_BASE}/${encodeURIComponent(FIRMS_MAP_KEY)}/`;

    const layer = window.L.tileLayer.wms(layerUrl, {
      layers: "fires_viirs_noaa21_24",
      format: "image/png",
      transparent: true,
      version: "1.1.1",
      opacity: 0.9,
      attribution: "NASA FIRMS • VIIRS NOAA-21",
      maxZoom: 18,
      zIndex: 400,
    });
  ...
  const addSatelliteHotspotMarkers = (hotspots: FirmsHotspot[]) => {
    const map = mapInstanceRef.current;

    if (!map || !window.L) return;

    if (satelliteMarkerLayerRef.current) {
      try {
        map.removeLayer(satelliteMarkerLayerRef.current);
      } catch {
        // Layer mungkin sudah terhapus.
      }
      satelliteMarkerLayerRef.current = null;
    ...
    // Pakai pane khusus di atas WMS agar marker benar-benar menerima klik.
    const paneName = "firmsHotspotPane";
    ...
    hotspots.forEach((hotspot) => {
      ...
      const icon = window.L.divIcon({
        className: "firms-hotspot-clickable",
        html: `
          <span
            style="
              display:block;
      ...
      marker.bindTooltip("🔥 NOAA-21", {
        direction: "top",
        offset: [0, -6],
        opacity: 0.95,
        className: "firms-hotspot-tooltip",
      });

      marker.on("click", (event: any) => {
        console.log("🔥 FIRMS hotspot clicked:", {
          latitude: lat,
          longitude: lng,
          frp: hotspot.frp,
        ...
        // Gunakan popup langsung pada map agar tidak bergantung pada
        // lifecycle bindPopup marker saat ada ribuan marker.
        try {
          const popup = window.L.popup({
            ...
            closeOnClick: true,
            autoPan: true,
            className: "firms-hotspot-popup",
          })
            .setLatLng(event?.latlng || [lat, lng])
            .setContent(buildHotspotPopup(hotspot));

          popup.openOn(map);
        } catch (popupError) {
          console.error("❌ Gagal membuka popup FIRMS:", popupError);
        }
      });
    ...
    satelliteMarkerLayerRef.current = markerLayer;

    console.log("🔥 FIRMS clickable markers:", markerCount);
  };
  ...
  const parseFirmsCsv = (csv: string): FirmsHotspot[] => {
    const rows = csv
      .split(/\r?\n/)
      .map((row) => row.trim())
      .filter(Boolean);
  ...
  const fetchSatelliteHotspotsFromWfs = async (): Promise<FirmsHotspot[]> => {
    if (!FIRMS_MAP_KEY) return [];

    const url =
      `https://firms.modaps.eosdis.nasa.gov/mapserver/wfs/${FIRMS_WFS_REGION}/` +
      `${encodeURIComponent(FIRMS_MAP_KEY)}/?` +
      new URLSearchParams({
        SERVICE: "WFS",
        REQUEST: "GetFeature",
        VERSION: "2.0.0",
        TYPENAME: FIRMS_WFS_LAYER,
        OUTPUTFORMAT: "application/json",
        SRSNAME: "EPSG:4326",
        BBOX: FIRMS_AREA_BBOX,
        COUNT: "10000",
      }).toString();
    ...
    if (!response.ok) {
      throw new Error(`NASA FIRMS WFS HTTP ${response.status}`);
    }
      ...
      return [
        {
          latitude,
          longitude,
          bright_ti4: props.bright_ti4 ?? props.brightness,
          bright_ti5: props.bright_ti5 ?? props.bright_t31,
          scan: props.scan,
          track: props.track,
          acq_date: props.acq_date,
          acq_time: props.acq_time,
          satellite: props.satellite || "N21",
  ...
  const fetchSatelliteHotspotsFromCsv = async (): Promise<FirmsHotspot[]> => {
    if (!FIRMS_MAP_KEY) return [];

    const url =
      `https://firms.modaps.eosdis.nasa.gov/api/area/csv/` +
      `${encodeURIComponent(FIRMS_MAP_KEY)}/${FIRMS_SOURCE}/` +
      `${FIRMS_AREA_BBOX}/1`;
    ...
    if (!response.ok) {
      throw new Error(`NASA FIRMS CSV HTTP ${response.status}`);
    }

    return parseFirmsCsv(await response.text());
  };

  const fetchSatelliteHotspots = async () => {
    setSatelliteHotspots((prev) => ({
      ...prev,
      loading: true,
      error: "",
    }));
    ...
    try {
      // Data atribut hotspot diambil dari backend lokal SIMITI.
      // WMS NASA tetap dipakai sebagai layer visual.
      const response = await fetch(`${API_URL}/api/firms/noaa21`, {
        cache: "no-store",
        headers: { Accept: "application/json" },
      });
      ...
      if (!response.ok || !payload?.success) {
        throw new Error(
          payload?.message || `Backend FIRMS HTTP ${response.status}`,
        );
      }

      const hotspots = Array.isArray(payload.hotspots) ? payload.hotspots : [];

      addSatelliteHotspotMarkers(hotspots);

      setSatelliteHotspots((prev) => ({
        ...prev,
        loading: false,
        error: "",
        count: Number(payload.count ?? hotspots.length),
        updatedAt: payload.updatedAt || new Date().toISOString(),
      }));
    } catch (error: any) {
      console.error("SIMITI FIRMS hotspot fetch error:", error);
      addSatelliteHotspotMarkers([]);
      ...
      setSatelliteHotspots((prev) => ({
        ...
        error:
          error?.message ||
          "Gagal mengambil data hotspot NASA FIRMS / VIIRS NOAA-21.",
      }));
    }
  };

  const addSatelliteGetFeatureInfoFallback = () => {
    // Tidak dipakai.
    // Detail hotspot berasal dari marker WFS /api/firms/noaa21,
    // sehingga tidak ada request GetFeatureInfo langsung ke NASA.
  };

  const refreshSatelliteData = () => {
    if (!FIRMS_MAP_KEY) return;

    if (satelliteHotspots.enabled) {
      addSatelliteHotspotLayer();
      fetchSatelliteHotspots();
    }
  };

  const toggleSatelliteHotspotLayer = () => {
    setSatelliteHotspots((prev) => {
  ...
  useEffect(() => {
    if (!mapReady || !mapInstanceRef.current || !FIRMS_MAP_KEY) return;

    refreshSatelliteData();

    satelliteRefreshTimerRef.current = window.setInterval(
      refreshSatelliteData,
      15 * 60 * 1000,
    );
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Cleanup satellite layer and map
      if (satelliteRefreshTimerRef.current !== null) {
        window.clearInterval(satelliteRefreshTimerRef.current);
        satelliteRefreshTimerRef.current = null;
      }
  ...
  const fetchBmkgWeather = async () => {
    const adm4 =
      (import.meta.env.VITE_BMKG_ADM4 as string | undefined)?.trim() ||
      "31.71.01.1001";
    ...
    try {
      ...
      const location = json?.lokasi || json?.data?.[0]?.lokasi || null;
      const forecastGroups = json?.data?.[0]?.cuaca || [];
      const forecastItems = Array.isArray(forecastGroups)
        ? forecastGroups.flat()
        : [];
      const current = forecastItems[0] || null;

      if (!current) {
        throw new Error("Data prakiraan BMKG kosong.");
      }
      ...
      setBmkgWeather({
        ...
        location,
        current,
        updatedAt: current.analysis_date || current.local_datetime || null,
      });
    } catch (error: any) {
      ...
      setBmkgWeather({
        ...
        error:
          error?.message ||
          "Gagal mengambil data BMKG. Periksa koneksi atau kode ADM4.",
        location: null,
        current: null,
        updatedAt: null,
      });
  ...
  useEffect(() => {
    ...
    // Data prakiraan BMKG diperbarui berkala; refresh ringan setiap 30 menit.
    const interval = window.setInterval(fetchBmkgWeather, 30 * 60 * 1000);
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#eef3f0] text-slate-800">
      <style>{`.firms-hotspot-popup .leaflet-popup-content-wrapper{border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,.22);}.firms-hotspot-popup .leaflet-popup-content{margin:10px 12px;}.firms-hotspot-tooltip{font-size:10px;font-weight:700;}        .custom-kejadian-marker { background: none; border: none; }        .marker-container:hover .marker-bg { fill: #ef4444 !important; }        .custom-kejadian-popup .leaflet-popup-content-wrapper { border-radius: 12px; padding: 0; box-shadow: 0 4px 20px rgba(0,0,0,0.15); }        .custom-kejadian-popup .leaflet-popup-content { margin: 0; width: 280px !important; }        .custom-kejadian-popup .leaflet-popup-tip { display: none; }        .dashboard-scroll::-webkit-scrollbar { width: 6px; height: 6px; }        .dashboard-scroll::-webkit-scrollbar-thumb { background: #b7c8c1; border-radius: 999px; }      `}</style>
            ...
            <section className="relative flex-1 min-h-0 rounded-2xl overflow-hidden bg-slate-200 shadow-sm border border-white">
              ...
              <div className="absolute top-3 left-3 right-3 z-[1200] flex items-center gap-2 pointer-events-none">
                <div className="ml-auto pointer-events-auto flex items-center gap-2">
                  ...
                  <div className="bg-white/95 backdrop-blur rounded-xl shadow-lg px-3 py-2 min-w-[190px]">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button
                        type="button"
                        onClick={toggleSatelliteHotspotLayer}
                        disabled={!FIRMS_MAP_KEY}
                        className={`rounded-lg px-2 py-1 text-[9px] font-bold ${
                          satelliteHotspots.enabled
                            ? "bg-orange-500 text-white"
                            : "bg-slate-100 text-slate-500"
                        } disabled:opacity-50`}
                        title={
                          FIRMS_MAP_KEY
                            ? "Tampilkan/sembunyikan hotspot satelit"
                            : "Set VITE_FIRMS_MAP_KEY terlebih dahulu"
                        }
                      >
                        {satelliteHotspots.enabled ? "ON" : "OFF"}
                      </button>
                    </div>

                    <div className="mt-1 flex items-end justify-between gap-2">
                      <div>
                        <div className="text-lg leading-none font-black text-orange-600">
                          {satelliteHotspots.loading
                            ? "..."
                            : satelliteHotspots.count.toLocaleString("id-ID")}
                        </div>
                        <div className="text-[8px] text-slate-400">
                          hotspot / 24 jam • Indonesia
                        </div>
                      </div>
                      <button
                        type="button"
                        onClick={refreshSatelliteData}
                        disabled={satelliteHotspots.loading || !FIRMS_MAP_KEY}
                    ...
                    {satelliteHotspots.error && (
                      <div className="mt-1 text-[8px] text-red-600">
                        {satelliteHotspots.error}
                      </div>
                    )}

                    {!satelliteHotspots.error &&
                      satelliteHotspots.updatedAt && (
                        <div className="mt-1 text-[8px] text-slate-400">
                          Update:{" "}
                          {new Date(satelliteHotspots.updatedAt).toLocaleString(
                            "id-ID",
                          )}
                        </div>
                      )}
            ...
            <section className="h-[178px] shrink-0 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-2">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 overflow-hidden">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm font-extrabold">
                      {bmkgWeather.location?.desa ||
                        bmkgWeather.location?.kecamatan ||
                        "Lokasi BMKG"}
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={fetchBmkgWeather}
                    disabled={bmkgWeather.loading}
                    className="text-[9px] text-emerald-700 font-bold hover:underline disabled:opacity-50"
                    title="Refresh data BMKG"
                  >
                    {bmkgWeather.loading ? "Memuat..." : "↻ Refresh"}
                  </button>
                </div>
                ...
                {bmkgWeather.error ? (
                  ...
                  <>
                    <div className="mt-3 flex items-center gap-3">
                      {bmkgWeather.current.image ? (
                        <img
                          src={bmkgWeather.current.image}
                          alt={bmkgWeather.current.weather_desc || "Cuaca BMKG"}
                      ...
                      <div>
                        <div className="text-xl font-black">
                          {bmkgWeather.current.t ?? "—"}°C
                        </div>
                        <div className="text-[9px] text-slate-500">
                          {bmkgWeather.current.weather_desc || "—"}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3 text-[9px]">
                      <div>
                        ...
                        <b>{bmkgWeather.current.hu ?? "—"}%</b>
                      </div>
                      <div>
                        <div className="text-slate-400">Hujan</div>
                        <b>{bmkgWeather.current.tp ?? "—"} mm</b>
                      </div>
                      <div>
                        <div className="text-slate-400">Angin</div>
                        <b>{bmkgWeather.current.ws ?? "—"} km/j</b>
                      </div>
                    </div>

                    <div className="mt-2 text-[8px] text-slate-400 truncate">
                      {bmkgWeather.location?.kotkab || ""}
                      {bmkgWeather.location?.provinsi
                        ? ` • ${bmkgWeather.location.provinsi}`
                        : ""}
                    </div>
                  </>
                ...
                <div className="mt-2 flex items-center justify-between text-[8px] text-slate-400">
                  ...
                  <span>
                    {bmkgWeather.updatedAt
                      ? bmkgWeather.updatedAt.replace("T", " ")
                      : "—"}
                  </span>
                </div>
              </div>

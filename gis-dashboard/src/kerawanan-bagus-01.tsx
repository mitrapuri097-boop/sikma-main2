...
const Kerawanan = () => {
  ...
  const loadBnpbLayer = async (key: string) => {
    ...
    if (!config?.url) {
      setLayerError(
        `ℹ️ URL service BNPB untuk "${config?.name || key}" belum tersedia pada daftar sumber.`,
      );
    ...
    try {
      ...
      if (bnpbOverlayRefs.current[key]) {
        try {
          map.removeLayer(bnpbOverlayRefs.current[key]);
        } catch {}
      }
      ...
      setLayerError(
        `❌ Gagal memuat ${config.name}: ${error?.message || "unknown error"}`,
      );
  ...
  const identifyBnpbAtPoint = async (latitude: number, longitude: number) => {
    const active = BNPB_INARISK_LAYERS.filter(
      (item) => item.url && activeLayersRef.current.has(item.key),
    );
  ...
  const refreshActiveBnpbLayers = async () => {
    for (const config of BNPB_INARISK_LAYERS) {
      if (config.url && activeLayers.has(config.key))
  ...
  const loadLayerInBounds = async (
    ...
    try {
      ...
      // Untuk request layer GIS, prioritaskan viewport peta saat ini.
      // Ini mencegah layer mengambil seluruh Indonesia ketika map sudah siap.
      const mapBounds = mapInstanceRef.current.getBounds();
      const boundsToUse = customBounds || [
      ...
      // Kirim zoom supaya backend dapat menerapkan limit/detail sesuai level zoom.
      let url = `${API_URL}/api/layers/${tableName}/geojson?bounds=${encodeURIComponent(boundsString)}&zoom=${zoom}`;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      Object.keys(bnpbOverlayRefs.current).forEach((key) => {
        try {
          mapInstanceRef.current?.removeLayer(bnpbOverlayRefs.current[key]);
        } catch {}
      });
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#f4f7f6] text-slate-800">
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 min-h-0 p-2 md:p-3 overflow-y-auto xl:overflow-hidden">
            ...
            <section className="relative min-h-[520px] xl:min-h-0 rounded-xl overflow-hidden bg-sky-100 border border-white shadow-sm">
              ...
              <div className="absolute left-2 right-2 top-2 z-[1700] h-10 bg-white/95 backdrop-blur rounded-lg border border-slate-200 shadow-[0_5px_18px_rgba(15,23,42,.12)] flex items-center overflow-x-auto dashboard-scroll px-1 gap-1">
                <button
                  ...
                  className="mockup-chip h-8 px-3 rounded-md bg-[#087f61] text-white text-[10px] font-bold"
                >
                  ▱ Semua Layer
                </button>
                {[
                  ...
                  <button
                    key={label}
                    onClick={() => setShowLayerPanel(true)}
                    className="mockup-chip h-8 px-3 rounded-md text-[9px] font-semibold text-slate-600 hover:bg-slate-100"
                  >
                    {label}
                  </button>
                ))}
              ...
              {/* Search lokasi */}
              <div className="absolute top-[58px] left-3 right-3 sm:right-auto z-[1700] w-auto sm:w-[285px] mockup-map-search">
                <div className="h-9 bg-white/95 rounded-lg shadow-md border border-slate-200 flex items-center px-3 gap-2">
                  ...
                  <input
                    ...
                    placeholder="Cari lokasi, nama daerah, koordinat, atau DAS..."
              ...
              {showLayerPanel && (
                <div className="absolute left-3 top-[152px] bottom-3 z-[1800] w-[calc(100%-24px)] max-w-[320px] sm:w-[245px] bg-white rounded-xl border border-slate-200 shadow-[0_16px_45px_rgba(15,23,42,.18)] overflow-hidden flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto dashboard-scroll p-3">
                    ...
                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-2 pb-1.5 border-b border-slate-200">
                        <h3 className="text-[11px] font-bold">BNPB InaRISK</h3>
              ...
              {/* Legend */}
              <div className="absolute right-3 bottom-3 z-[1700] w-[160px] sm:w-[190px] max-h-[42vh] overflow-y-auto dashboard-scroll bg-white/95 rounded-xl border border-slate-200 shadow-lg">
              ...
              <div className="hidden sm:flex absolute left-3 right-[205px] bottom-3 z-[1700] flex items-center gap-2">
            ...
            <aside className="xl:row-span-2 min-h-[420px] xl:min-h-0 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto dashboard-scroll p-4">
                ...
                {selectedInfoTab === "Mitigasi" && (
                  <div>
                    <div className="text-[10px] font-bold mb-3">
                      Rekomendasi Mitigasi & Adaptasi (AI)
                    </div>
                    <div className="space-y-2">
                      {[
                        "Rehabilitasi Hutan & Lahan Kritis",
                        "Pembangunan Embung & Sumur Resapan",
                        "Sistem Peringatan Dini (EWS) Komunitas",
                        "Penguatan Sekat Kanal & Pembasahan Gambut",
                      ].map((x, i) => (
                        <div
                          key={x}
                          className="flex gap-2 p-3 rounded-lg bg-emerald-50 border border-emerald-100"
                        >

          ...
          <div className="h-full flex flex-col gap-2">
            <section className="relative flex-1 min-h-0 rounded-2xl overflow-hidden bg-slate-200 shadow-sm border border-white">
              ...
              <div className="absolute top-3 left-3 right-3 z-[1200] flex items-center gap-2 pointer-events-none">
                <div className="pointer-events-auto bg-white/95 backdrop-blur rounded-xl shadow-lg border border-white px-1.5 py-1 flex gap-1 overflow-x-auto max-w-[calc(100%-340px)]">
                  {[
                    ["Semua Layer", "◉"],
                    ["Administrasi", "⌖"],
                    ["Kawasan Hutan", "▧"],
                    ...
                    <button
                      key={label}
                      className={`whitespace-nowrap rounded-lg px-2.5 py-1.5 text-[10px] font-semibold ${idx === 0 ? "bg-[#0b6b53] text-white" : "text-slate-600 hover:bg-slate-100"}`}
                    >
                      {icon} {label}
                    </button>
                  ))}
                </div>
                <div className="ml-auto pointer-events-auto bg-white/95 backdrop-blur rounded-xl shadow-lg px-3 py-2 text-[10px] text-slate-600 flex items-center gap-2">
              ...
              <div className="absolute top-3 right-3 bottom-3 w-[294px] z-[1300] bg-white/96 backdrop-blur rounded-2xl shadow-2xl border border-white overflow-hidden flex flex-col">
                <div className="px-4 py-3 border-b border-slate-100 flex items-start justify-between">
                  <div>
                    ...
                    <div className="text-[10px] text-slate-500">
                      {selectedAreas[0]?.provinsi || "Indonesia"}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-5 border-b border-slate-100 text-[9px]">
                  {[
                    "Ringkasan",
                    "Kerawanan",
                    "Mitigasi",
                    ...
                    <button
                      key={t}
                      className={`py-2 ${i === 0 ? "text-emerald-700 border-b-2 border-emerald-500 font-bold" : "text-slate-500"}`}
                    >
                      {t}
                    </button>
                  ))}
                </div>
                <div className="p-3 overflow-y-auto dashboard-scroll space-y-3">
                  ...
                  <div className="rounded-xl border border-slate-100 p-3">
                    ...
                    {["Longsor", "Banjir", "Karhutla"].map((x, i) => (
                      <div
                        key={x}
                        className="flex items-center justify-between py-1.5 text-[10px] border-b last:border-0 border-slate-50"
                      >
                        <span>{x}</span>
                  ...
                  <div className="rounded-xl border border-slate-100 p-3">
                    ...
                    <div className="grid grid-cols-2 gap-y-2 text-[9px]">
                      <span className="text-slate-500">Kejadian</span>
                      <b>{totalEvents || "—"}</b>
                      <span className="text-slate-500">Area Rawan</span>
                      <b>
                        {totalRiskArea
                          ? totalRiskArea.toLocaleString("id-ID", {
                              maximumFractionDigits: 0,
                            }) + " Ha"
                          : "—"}
                      </b>
                      <span className="text-slate-500">Mitigasi</span>
                      <b>{layerData.mitigasiAdaptasi.length}</b>
                      <span className="text-slate-500">Layer Data</span>
                      <b>
                        {layerData.kerawanan.length +
                          layerData.mitigasiAdaptasi.length +
                          layerData.lainnya.length}
                      </b>
                    </div>
                  </div>
                  <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-3">
                    ...
                    {activeLayerLabels.length ? (
                      activeLayerLabels.map((x) => (
                        <div
                          key={x}
                          className="text-[9px] text-emerald-700 mt-1"
                        >
                          ● {x}
                        </div>
                      ))
                      ...
                      {activeTab === "administrasi" && (
                        <div className="mb-4 p-3 bg-blue-50 rounded border border-blue-200">
                          ...
                          {selectedDas.length > 0 && (
                            <div className="mb-2 p-2 bg-yellow-50 border border-yellow-200 rounded">
                              <p className="text-xs text-yellow-700">
                                ⚠️ Filter DAS aktif. Hapus filter DAS untuk
                                menggunakan filter administrasi.
                              </p>
                            </div>
                          )}
                          ...
                          {selectedAreas.length > 0 && (
                            <div className="mb-2 flex flex-wrap gap-1">
                              {selectedAreas.map((area, index) => (
                                <div
                                  key={index}
                                  className="bg-white px-2 py-1 rounded text-xs border border-blue-300 flex items-center gap-1"
                                >
                                  <span className="text-[10px] bg-blue-100 px-1 rounded">
                                    {area.level}
                                  </span>
                                  <span
                                ...
                                <>
                                  ...
                                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded shadow-lg max-h-48 overflow-y-auto z-50">
                                    {areaSearchResults.map((result, index) => (
                                      ...
                                      >
                                        <span className="text-[10px] bg-gray-100 px-1 rounded mr-1">
                                          {result.level}
                                        </span>
                                        {result.label}
                                      </div>
                          ...
                          <div className="text-[10px] text-gray-600 mt-1">
                            * Pilih level lalu cari area untuk membatasi
                            tampilan layer
                          </div>
                        </div>
                      )}
                      ...
                      {activeTab === "das" && (
                        <div className="mb-4 p-3 bg-green-50 rounded border border-green-200">
                          ...
                          {selectedAreas.length > 0 && (
                            <div className="mb-2 p-2 bg-yellow-50 border border-yellow-200 rounded">
                              <p className="text-xs text-yellow-700">
                                ⚠️ Filter administrasi aktif. Hapus filter
                                administrasi untuk menggunakan filter DAS.
                              </p>
                            </div>
                          )}
                          ...
                          {selectedDas.length > 0 && (
                            <div className="mb-2 flex flex-wrap gap-1">
                              {selectedDas.map((das, index) => (
                                <div
                                  key={index}
                                  className="bg-white px-2 py-1 rounded text-xs border border-green-300 flex items-center gap-1"
                                >
                                  <span
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) =>
                                handleDasSearchChange(e.target.value)
                              }
                            ...
                            {/* Search Results Dropdown */}
                            {showDasSearchDropdown &&
                              dasSearchResults.length > 0 &&
                              selectedAreas.length === 0 && (
                                <>
                                  <div
                                    className="fixed inset-0 z-40"
            ...
            <section className="h-[178px] shrink-0 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-2">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 overflow-hidden">
                ...
                <div className="mt-3 space-y-2 text-[10px]">
                  {[
                    ["Hutan Alam", "45.321 Ha"],
                    ["Hutan Tanaman", "32.187 Ha"],
                    ["Perkebunan", "12.543 Ha"],
                    ["Lahan Terbuka", "8.921 Ha"],
                  ].map(([label, value], i) => (
                    <div
                      key={label}
                ...
                <button
                  onClick={() => setIsLayerPanelOpen(true)}
                  className="mt-3 text-[9px] text-emerald-700 font-bold hover:underline"
                >
                  Kelola layer →
                </button>
              </div>
